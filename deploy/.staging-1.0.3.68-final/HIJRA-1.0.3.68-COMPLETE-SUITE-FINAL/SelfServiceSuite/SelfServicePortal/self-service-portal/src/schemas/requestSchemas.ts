...
export const staffClaimLineSchema = z.object({
  ...
  coveragePercent: z.coerce.number().min(0).max(100).optional().default(0),
...
export const imprestHeaderSchema = z
  .object({
    ...
    // UAT 23/07/2026: free-text destination on the imprest header (line destination stays a code).
...
export const pettyCashReplenishmentSchema = z.object({
  ...
  payingAccount: z.string().optional(),
...
// Mirrors Hijra Bank's Training Need Assessment SSP template (17-07-2026). The ERP stores the
// purpose as Text[100], hence the hard cap; the remaining fields persist via CuPortalTraining.
export const trainingRequestSchema = z
  .object({
    trainingNeed: z.string().min(1, 'Training need is required'),
    department: z.string().optional().default(''),
    otherTrainingName: z.string().optional().default(''),
    comments: z
      .string()
      .min(3, 'Purpose / expected outcome is required')
      .max(100, 'Purpose must be 100 characters or fewer (ERP limit)'),
    trainingType: z.enum(['Role specific', 'Soft skill', 'Leadership', 'Compliance'], {
      message: 'Select the training type',
    }),
    durationDays: z.coerce.number({ message: 'Enter the duration in days' }).positive('Duration must be more than zero'),
    targetGroup: z.string().min(2, 'Target group is required'),
    participants: z.coerce
      .number({ message: 'Enter the number of participants' })
      .int('Participants must be a whole number')
      .positive('Participants must be at least 1'),
    quarter: z.enum(['QI', 'QII', 'QIII', 'QIV'], { message: 'Select the quarter' }),
    priority: z.enum(['High', 'Medium', 'Low'], { message: 'Select the priority' }),
    vendor: z.string().optional().default(''),
    estimatedBudget: z.string().optional().default(''),
    remark: z.string().optional().default(''),
    additionalTrainingNeeds: z.string().optional().default(''),
    periodStart: dateField,
    periodEnd: dateField,
  })
  .refine((values) => values.trainingNeed !== '__OTHER__' || values.otherTrainingName.trim().length > 1, {
    path: ['otherTrainingName'],
    message: 'Name the training since it is not in the ERP list',
  })
  .refine((values) => !isBefore(parseISO(values.periodEnd), parseISO(values.periodStart)), {
    path: ['periodEnd'],
    message: 'Training period end must be the same as or later than the start date',
  })
...
/** BC Asset Transfer (table 50278) — vehicle/tools handover + temporary transfer. */
export const assetTransferHeaderSchema = z
  .object({
    ...
    assetNo: z.string().min(1, 'Select the asset (vehicle/tool) to transfer'),
    ...
    reason: z.string().min(5, 'Describe the reason for the transfer'),
    assetCondition: z.enum(['Good', 'Fair', 'Damaged']),
    assetConditionDescription: z.string().min(3, 'Describe the asset condition'),
    temporaryExpiryDate: optionalText,
  })
  .superRefine((data, ctx) => {
    if (data.typeOfTransfer === 'Temporary' && !data.temporaryExpiryDate) {
      ctx.addIssue({
        code: 'custom',
        path: ['temporaryExpiryDate'],
        message: 'A temporary transfer needs the expected return date',
      })
    }
    if (data.transferType === 'Internal' && !data.toEmployeeNo) {
      ctx.addIssue({
        code: 'custom',
        path: ['toEmployeeNo'],
        message: 'Select the receiving employee',
      })
    }
    if (data.transferType === 'Internal' && !data.fromEmployeeNo) {
      ctx.addIssue({
        code: 'custom',
        path: ['fromEmployeeNo'],
        message: 'Select the current (From) responsible employee',
      })
    }
    if (data.transferType === 'External') {
      if (!data.partnerName) {
        ctx.addIssue({
          code: 'custom',
          path: ['partnerName'],
          message: 'Partner (receiving organisation) is required',
        })
      }
      if (!data.destinationLocation) {
        ctx.addIssue({
          ...
          message: 'Destination/location is required for external transfers',
        })
      }
    }
  })
