...
async function applyMedicalClaimAmounts(values: FieldValues, form: UseFormReturn<FieldValues>) {
  ...
  try {
    ...
    if (coveragePercent > 0) form.setValue('coveragePercent', coveragePercent, { shouldValidate: true })
    else if (amount > 0 && medicalAmount > 0) {
      form.setValue('coveragePercent', Math.round((amount / medicalAmount) * 10000) / 100, {
        shouldValidate: true,
      })
    }
  } catch {
    ...
    <MultiStepRequestPage
      ...
      line={{
        ...
        columns: [
          ...
          { key: 'coveragePercent', header: 'Coverage %', format: (value) => (Number(value ?? 0) > 0 ? `${value}%` : '—') },
