...
function formatDurationDate(start: string, end: string) {
  const formatBcDisplayDate = (value: string) => {
  ...
  if (erpFieldsChanged && advanceType && destination && noOfDays > 0) {
    ...
    try {
      ...
      if (fetchedAmount > 0) {
        ...
        form.setValue(
          'dailyRate',
          fetchedRate > 0 ? fetchedRate : roundMoney(fetchedAmount / noOfDays),
          { shouldValidate: true },
        )
        imprestPrevValues = {
          ...values,
          amount: fetchedAmount,
          dailyRate: fetchedRate > 0 ? fetchedRate : roundMoney(fetchedAmount / noOfDays),
        }
      }
    } catch {
...
function durationSupplement(values: FieldValues): ReactNode {
  const duration = formatDurationDate(String(values.travelDate ?? ''), String(values.returnDate ?? ''))
...
function payloadAmount(payload: Record<string, unknown> | undefined, keys: string[]) {
  if (!payload) return 0
...
const imprestListColumns: DataTableColumn<PortalRequest>[] = [
  { id: 'requestNo', header: 'No.', cell: (row) => row.requestNo },
  {
    ...
    cell: (row) =>
      formatDurationDate(
        String(row.payload?.TravelStartDate ?? row.payload?.TravelDate ?? ''),
        String(row.payload?.ExpectedReturnDate ?? row.payload?.ReturnDate ?? ''),
      ) || '—',
  },
  { id: 'date', header: 'Date', cell: (row) => formatDate(row.createdAt) },
  { id: 'status', header: 'Status', cell: (row) => <StatusBadge status={row.status} /> },
  { id: 'title', header: 'Purpose', cell: (row) => row.title },
  {
    id: 'remaining',
    header: 'Remaining Unsettled',
    cell: (row) => {
      const amount = payloadAmount(row.payload, [
...
export function ImprestRequest() {
  ...
  const travelDestinations = useLookupOptions('travel-destinations')
  ...
  return (
    <MultiStepRequestPage
      ...
      listStatusFilter
      listColumns={imprestListColumns}
      ...
      headerSupplement={durationSupplement}
      headerFields={[
        {
          name: 'dateRequired',
          label: 'Date Required',
        ...
        { name: 'purpose', label: 'Imprest Purpose', type: 'textarea', valuePaths: ['Purpose'] },
        {
          ...
          type: 'select',
          options: travelDestinations.options,
          placeholder: travelDestinations.isLoading
            ? 'Loading travel destinations…'
            : travelDestinations.isError
              ? 'Could not load destinations'
              : 'Select travel destination',
          valuePaths: ['TravelDestination', 'Travel_Destination', 'Destination', 'DestinationCode'],
        },
        {
          name: 'travelDate',
          label: 'Travel Date',
      ...
      detailFields={[
        ...
        { label: 'Travel Destination', paths: ['payload.TravelDestination', 'payload.Travel_Destination', 'payload.Destination'] },
        {
          label: 'Travel Date',
          paths: [
      ...
      line={{
        ...
        fields: [
          ...
          { name: 'noOfDays', label: 'No. of Days', type: 'number' },
          {
            ...
            label: 'Daily Rate',
            type: 'number',
            placeholder: 'Auto-calculated from ERP — or enter to derive amount',
          },
          {
            name: 'amount',
            label: 'Amount',
