...
export interface MultiStepRequestConfig {
  ...
  /** Optional custom list table columns (defaults to No / Date / Status / Description / Amount). */
...
export function MultiStepRequestPage(config: MultiStepRequestConfig) {
  ...
  const filteredRequests = useMemo(() => {
    ...
    return statusFiltered.filter((row) =>
      [row.requestNo, row.title, row.status, row.createdAt, String(row.amount ?? '')]
        .filter(Boolean)
        .some((field) => String(field).toLowerCase().includes(term)),
    )
  }, [config.listStatusFilter, listSearch, listStatus, requestsQuery.data])
  ...
  const actionsColumn: DataTableColumn<PortalRequest> = {
    id: 'actions',
    header: 'Actions',
    cell: (row) => (
      <div className="flex flex-wrap gap-1">
  ...
  const columns: DataTableColumn<PortalRequest>[] = config.listColumns
    ? [...config.listColumns, actionsColumn]
    : [
    ...
    <PageWrapper title={config.title} actions={<PortalNewButton label={config.newButtonLabel ?? 'New Request'} onClick={() => setMode('create')} />}>
      {requestsQuery.isLoading ? (
        ...
        <>
          ...
          <DataTable rows={filteredRequests} columns={columns} getRowId={(row) => row.id} onRowClick={(row) => setSelectedId(row.id)} compact />
        </>
      )}
    </PageWrapper>
  )
