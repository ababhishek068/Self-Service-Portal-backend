...
export interface MultiStepRequestConfig {
  ...
  /** Module-specific actions shown on a request detail, such as posting an approved transfer. */
  extraDetailActions?: Array<{
    ...
    run: (id: string, request: PortalRequest) => Promise<unknown>
...
function purchaseBudgetActionError(
  module: EndpointConfig['module'],
  ...
  if (
    ...
    !/finalized budget|budget for|available budget|budget.*depleted/i.test(message)
  ) {
    return message
  }
  ...
  const lines = Array.isArray(payload.lines)
    ...
    : []
  const itemLabels = lines
    .map((line) => {
      const itemNo = String(line.itemNo ?? line.ItemNo ?? line.No ?? '').trim()
      const description = String(line.description ?? line.Description ?? '').trim()
  ...
  const itemText = itemLabels.length ? itemLabels.join(', ') : 'the purchase line'
  const departmentText = department ? ` for department ${department}` : ''

  return `Business Central budget check failed for ${itemText}${departmentText}. Delete the invalid line and add an item included in the finalized budget, or ask Procurement/Finance to update the budget in Business Central.`
}

function purchaseApprovalConfirmation(
  module: EndpointConfig['module'],
  request: PortalRequest,
  lineCount: number,
) {
  if (module !== 'purchaseRequisition') {
    return 'Send this request into the approval workflow?'
  }
  const payload = (request.payload ?? {}) as Record<string, unknown>
  const department = String(
    payload.RequestingDepartment ??
  ...
  const departmentText = department ? ` for department ${department}` : ''
  return `Business Central will check ${lineCount} purchase line${lineCount === 1 ? '' : 's'} against the finalized budget${departmentText}. Send this request for approval?`
}
  ...
  return function renderField(field: FieldConfig) {
    ...
    return (
      <div
        key={name}
        className={[
          field.type === 'checkbox' ? 'flex items-center gap-2' : 'space-y-1.5',
  ...
  return (
    <div className="space-y-3 overflow-x-auto">
      <ListSearch
        ...
        ariaLabel={`Search ${line.label.toLowerCase()}`}
      />
          ...
          {filteredRows.length === 0 ? (
            <tr>
              <td colSpan={line.columns.length} className="px-2 py-4 text-center text-slate-500">
                {search ? `No lines match "${search.trim()}".` : (line.emptyText ?? 'No lines.')}
              </td>
            </tr>
          ) : null}
        </tbody>
...
export function MultiStepRequestPage(config: MultiStepRequestConfig) {
  ...
  const refresh = async () => {
    ...
    if (['storeRequisition', 'transferOrder', 'assetTransfer'].includes(config.module.module)) {
  ...
  const upsertRequestInList = (request: PortalRequest) => {
    queryClient.setQueryData<PortalRequest[]>(config.queryKey, (current = []) => [
      request,
  ...
  const headerCreated = (request: PortalRequest) => {
    // Put the BC-created document into the list immediately, then reconcile it
    // with a fresh server read. This avoids an eventual-consistency gap after
    // Business Central returns from the create SOAP call.
    upsertRequestInList(request)
    void queryClient.invalidateQueries({ queryKey: config.queryKey }).finally(() => {
      upsertRequestInList(request)
    })
    if (['storeRequisition', 'transferOrder', 'assetTransfer'].includes(config.module.module)) {
  ...
  const returnToList = () => {
    const currentRequest = detailQuery.data
    setSelectedId(null)
    setEditingLine(null)
    ...
    setLineSearch('')
    setMode('list')
    // Back must not rely on React Query's stale-time decision. Always request a
    // current list, while retaining a just-created row if BC has not exposed it
    // through OData yet.
    void requestsQuery.refetch().finally(() => {
      if (currentRequest) upsertRequestInList(currentRequest)
    })
  }
  ...
  const runAction = async (
    ...
    try {
      ...
      toast.error(
        purchaseBudgetActionError(config.module.module, detailQuery.data, err) || errorLabel,
        'Action failed',
      )
    } finally {
      setActionId(null)
              ...
              {selected ? (
                <div className="flex flex-wrap gap-2">
                    ...
                    <Button
                      ...
                      onClick={() => {
                        ...
                        void confirm({
                          ...
                          message: purchaseApprovalConfirmation(
                  ...
                  {config.extraDetailActions
                    ...
                    .map((action) => {
                      const actionKey = `${action.id}-${selected.id}`
                      return (
                        <Button
                          key={action.id}
                          type="button"
                          ...
                          onClick={() => {
                            const execute = () =>
                              runAction(
                                actionKey,
                                () => action.run(selected.id, selected),
                                `${action.label} failed`,
                    ...
                    {!config.line.editableFields?.length ? (
                      <div className="mb-3">
                        <ListSearch
                          ...
                          placeholder={`Search ${config.line.label.toLowerCase()}...`}
                          ariaLabel={`Search ${config.line.label.toLowerCase()}`}
                        />
                      </div>
                    ) : null}
                    {config.line.editableFields?.length ? (
                            ...
                            {filteredLines.length === 0 ? (
                              <tr>
                                <td colSpan={config.line.columns.length + (editable || canReceiveStoreLines ? 1 : 0)} className="px-2 py-4 text-center text-slate-500">
                                  {lineSearch
                                    ? `No lines match "${lineSearch.trim()}".`
