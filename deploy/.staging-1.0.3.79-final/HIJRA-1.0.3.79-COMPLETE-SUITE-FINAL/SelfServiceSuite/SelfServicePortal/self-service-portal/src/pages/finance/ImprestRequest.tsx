...
async function applyImprestLineAmount(
  ...
  try {
    ...
    if (amount > 0) {
      form.setValue('dailyRate', amount / noOfDays, { shouldValidate: true })
  ...
  return (
    <MultiStepRequestPage
      ...
      detailFields={[
        ...
        { label: 'Travel Duration', paths: ['payload.DurationDate', 'payload.Duration_Date'] },
        ...
        { label: 'Total Net Amount', paths: ['payload.TotalNetAmount', 'request.amount'], format: 'currency' },
        {
          ...
          paths: ['payload.RemainingUnsettledAmount', 'payload.RemainingNotSettledAmount', 'payload.OutstandingBalance', 'payload.Balance'],
          format: 'currency',
        },
      ...
      line={{
        ...
        columns: [
          ...
          { key: 'dailyRate', header: 'Daily Rate', format: (value) => formatCurrency(Number(value ?? 0)) },
