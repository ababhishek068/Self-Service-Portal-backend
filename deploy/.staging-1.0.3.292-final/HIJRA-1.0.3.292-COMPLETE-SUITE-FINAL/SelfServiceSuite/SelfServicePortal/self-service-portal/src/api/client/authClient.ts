...
// A reset-token request performs BC employee lookup, token update, and email
// sequentially. Approval writes also wait for BC workflow events. Each BC call
// has its own server-side timeout, so the browser must allow the full sequence.
...
/** Drop stale ssp.apiBaseUrl when host/port no longer matches this page (e.g. :4000 vs :4001). */
function storedBaseMatchesPage(stored: string): boolean {
  if (typeof window === 'undefined') return true
  try {
    return new URL(stored).origin === window.location.origin
  } catch {
    return false
  }
}
...
export function resolveApiBaseUrl(preferred?: 'application' | 'bc365'): string {
  ...
  if (stored) {
    if (storedBaseMatchesPage(stored)) return stored.replace(/\/$/, '')
    clearApiBaseUrl()
  }
  return (
    env.AUTH_API_URL ||
    ...
    return Promise.reject(
      new AuthApiError({
        message:
          data?.message ??
          (timedOut
            ? 'Business Central is taking longer than expected. Wait a moment and check whether the action completed before retrying.'
            : error.message ?? 'Request failed'),
        status,
        code: data?.code ?? (timedOut ? 'REQUEST_TIMEOUT' : undefined),
      }),
    )
  },
)
