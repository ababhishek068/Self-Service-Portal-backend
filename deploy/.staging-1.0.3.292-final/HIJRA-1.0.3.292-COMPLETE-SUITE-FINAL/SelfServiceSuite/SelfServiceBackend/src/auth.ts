...
export interface AuthUser {
  ...
  /** District code when the employee sits under District (not Department) in the organogram. */
  district?: string
  branchCode: string
  branchName: string
  jobTitle: string
...
async function isHeadOfDepartment(employeeNo: string) {
  ...
  const filters = [
    ...
    `Staff_No eq '${odataString(employeeNo)}' and Dimension_Code eq 'DEPART/DIST'`,
    `StaffNo eq '${odataString(employeeNo)}' and DimensionCode eq 'DEPART/DIST'`,
  ]
...
async function buildAuthUser(employee: BcEmployee, userSetup: BcUserSetup): Promise<AuthUser> {
  ...
  const districtCode = employeeFieldText(employee as Record<string, unknown>, ['District'])
  const department = employeeFieldText(employee as Record<string, unknown>, [
    ...
    'Department',
    'District',
    // Sector / GD1 last — only if no department/district dimension is set on the card.
  ...
  const genderRaw = String(employee.Gender ?? employee.Sex ?? '').trim()
  const genderLower = genderRaw.toLowerCase()
  const gender =
    genderLower === '1' || /^(m|male|man)/i.test(genderRaw)
      ? 'Male'
      : genderLower === '2' || /^(f|female|woman)/i.test(genderRaw)
        ? 'Female'
        : genderRaw
  ...
  const org = await resolveEmployeeOrgDisplayFields(employee as Record<string, unknown>, {
    departmentName:
      ...
      '',
    branchCode: employee.GlobalDimension3Code ?? '',
  ...
  return {
    ...
    department: org.departmentCode || districtCode || department,
    departmentName:
      org.departmentName ||
      org.district ||
      (employee.DepartmentName ?? department),
    district: org.district || districtCode || undefined,
    branchCode:
      org.branchCode ||
      (employee.GlobalDimension3Code ?? ''),
    branchName:
      org.branchName ||
    ...
    permissionDepartments: [
      ...new Set(
        [org.departmentCode, districtCode, department, org.district].filter(
