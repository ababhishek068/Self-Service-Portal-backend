...
export async function fetchEmployeeSalaryBaseFast(
  ...
  skipDocumentHints = false,
) {
  if (!skipDocumentHints) {
    if (hints.header) {
...
export async function fetchEmployeeSalaryBaseForAdvance(
  ...
  options: { fast?: boolean; skipDocumentHints?: boolean } = {},
) {
  if (options.fast) {
    return fetchEmployeeSalaryBaseFast(employeeNo, hints, options.skipDocumentHints)
  }

  if (!options.skipDocumentHints) {
    if (hints.header) {
      const fromHeader = employeeSalaryBaseFromRecord(hints.header)
      if (fromHeader > 0) return fromHeader
    }
...
/** Parse a Branches List OData row (Division sits under Department/District). */
export function pickDivisionUnderDepartment(
  rows: ODataRecord[],
  ...
  const codeOf = (row: ODataRecord) =>
    dimensionRowText(row, [
      'Division_Branch_Code',
      'DivisionBranchCode',
      'Division_Branch_Code',
    ])
  const nameOf = (row: ODataRecord) =>
    dimensionRowText(row, [
  ...
  const deptOf = (row: ODataRecord) =>
    dimensionRowText(row, [
      'Department_District_Code',
      'DepartmentDistrictCode',
      'Department_District_Code',
      'DepartmentCode',
      'Department_Code',
    ])
  const levelOf = (row: ODataRecord) => dimensionRowText(row, ['level', 'Level'])

  const divisionRows = rows.filter((row) => {
    const level = levelOf(row)
    return !level || /^division$/i.test(level) || level === '0'
  })
  const underDept = dept
    ? divisionRows.filter((row) => deptOf(row).toUpperCase() === dept)
    : divisionRows
  const pool = underDept.length > 0 ? underDept : divisionRows

  const byHint = hintUpper
    ? pool.find((row) => {
        const code = codeOf(row).toUpperCase()
        const name = nameOf(row).toUpperCase()
        return code === hintUpper || name === hintUpper
      })
    : undefined
  const chosen = byHint ?? pool[0]
  if (!chosen) return { code: '', name: '' }
  const code = codeOf(chosen)
  const name = nameOf(chosen)
  return { code, name: name || code }
}

async function fetchBranchesRows(filter: string, top = 50) {
  for (const service of BRANCHES_ODATA_SERVICES) {
    try {
      ...
      // page/query may be unpublished
    }
  }
  return [] as ODataRecord[]
}

async function resolveDivisionUnderDepartment(
  departmentCode: string,
  hint = '',
  sectorCode = '',
  entryNo = '',
  ...
  if (entryNo && /^\d+$/.test(entryNo.trim())) {
    filters.push(`Entry_No eq ${entryNo.trim()}`, `EntryNo eq ${entryNo.trim()}`)
  }
  if (dept) {
    const escaped = odataString(dept)
    const deptClause = `(Department_District_Code eq '${escaped}' or DepartmentDistrictCode eq '${escaped}')`
    const levelClause = `(level eq 'Division' or Level eq 'Division')`
    const sector = sectorCode.trim()
    if (sector) {
      const sectorEsc = odataString(sector)
      filters.push(
        `${deptClause} and ${levelClause} and (Sector_Code eq '${sectorEsc}' or SectorCode eq '${sectorEsc}')`,
      )
    }
    filters.push(`${deptClause} and ${levelClause}`)
    filters.push(deptClause)
  }

  for (const filter of filters) {
    const rows = await fetchBranchesRows(filter)
    const picked = pickDivisionUnderDepartment(rows, dept, hint)
    if (picked.name || picked.code) return picked
  }

  if (hint) {
    const escaped = odataString(hint)
    const rows = await fetchBranchesRows(
      `(Division_Branch_Code eq '${escaped}' or Division_Branch_Name eq '${escaped}') and (level eq 'Division' or Level eq 'Division')`,
    )
    const picked = pickDivisionUnderDepartment(rows, dept, hint)
    if (picked.name || picked.code) return picked
  }
...
const EMPLOYEE_FINANCE_DEPARTMENT_CODE_FIELDS = [
  // Prefer true department codes — never Sector / Global Dimension 1 (that is Sector).
  // Do not include District here: that field is used only to classify district-line staff.
...
/** OData field keys that identify an employee's Department or District org unit. */
export const EMPLOYEE_ORG_SCOPE_CODE_FIELDS = [
  ...EMPLOYEE_FINANCE_DEPARTMENT_CODE_FIELDS,
  'District',
  'GlobalDimension1Code',
  'Global_Dimension_1_Code',
] as const

/** Collect every BC code that scopes an employee to their Department or District team. */
export function employeeOrgScopeCodes(
  employee: ODataRecord | Record<string, unknown>,
  hints: { department?: string; district?: string } = {},
) {
  ...
  for (const key of EMPLOYEE_ORG_SCOPE_CODE_FIELDS) {
    add(firstEmployeeFieldText(employee, [key]))
  }
  add(hints.department)
  add(hints.district)
  return [...codes]
}
...
export async function resolveEmployeeOrgDisplayFields(
  ...
  const sectorCode = firstEmployeeFieldText(employee, [
    'Sector',
    'SectorCode',
    'Sector_Code',
    'GlobalDimension1Code',
  ...
  const divisionEntryNo = firstEmployeeFieldText(employee, [
    'DivisionEntryNo',
    'Division_Entry_No',
    'DivisionEntry_No',
  ])
  ...
  const departmentCode =
    firstEmployeeFieldText(employee, EMPLOYEE_FINANCE_DEPARTMENT_CODE_FIELDS) || ''

  let departmentName = firstEmployeeFieldText(employee, EMPLOYEE_FINANCE_DEPARTMENT_NAME_FIELDS) || ''
  if (departmentCode && (!departmentName || departmentName === departmentCode)) {
    departmentName =
      (await resolveDepartmentListName(departmentCode, 'Department')) ||
      (await resolveDimensionDisplayName(departmentCode)) ||
      departmentName ||
      departmentCode
  }
  ...
  let branchName = firstEmployeeFieldText(employee, EMPLOYEE_BRANCH_NAME_FIELDS) || ''
  let branchCode =
    firstEmployeeFieldText(employee, [
      'BranchCode',
      'Branch_Code',
      'GlobalDimension3Code',
      'Global_Dimension_3_Code',
    ]) || ''
  if (!branchCode && _authUser?.branchCode?.trim()) {
    branchCode = _authUser.branchCode.trim()
  }
  if (!branchName && _authUser?.branchName?.trim()) {
    branchName = _authUser.branchName.trim()
  }
  ...
  const districtOnList = districtCode
    ? Boolean(await resolveDepartmentListName(districtCode, 'District'))
    : false
  const looksLikeDistrict =
    /district/i.test(district) ||
    /district/i.test(districtCode) ||
    /district/i.test(departmentName) ||
    /district/i.test(departmentCode)
  const orgKind: 'department' | 'district' =
    districtOnList || Boolean(districtCode) || looksLikeDistrict ? 'district' : 'department'

  let divisionCode = firstEmployeeFieldText(employee, ['Division', 'DivisionCode', 'Division_Code'])
  let divisionName = firstEmployeeFieldText(employee, ['DivisionName', 'Division_Name'])
  ...
  if (
    orgKind === 'department' &&
    (divisionCode || divisionEntryNo) &&
    (!divisionName || division === divisionCode)
  ) {
    const fromBranches = await resolveDivisionUnderDepartment(
      departmentCode,
      divisionCode,
      sectorCode,
      divisionEntryNo,
    )
    if (fromBranches.name || fromBranches.code) {
      divisionCode = fromBranches.code || divisionCode
      divisionName = fromBranches.name || divisionName
      division = fromBranches.name || fromBranches.code
    }
  }

  return {
    division: orgKind === 'department' ? division : '',
    divisionCode: orgKind === 'department' ? divisionCode : '',
    departmentName: orgKind === 'department' ? departmentName : '',
    departmentCode: orgKind === 'department' ? departmentCode : '',
    district: orgKind === 'district' ? district : '',
    districtCode: orgKind === 'district' ? districtCode : '',
    branchName: orgKind === 'district' ? branchName : '',
    branchCode: orgKind === 'district' ? branchCode : '',
    sectorCode,
    orgKind,
    scopeCode:
      (orgKind === 'district' ? districtCode || departmentCode : departmentCode || districtCode) ||
      '',
  }
}
