Table 50532 "HR Leave Application"
{
    ...
    fields
    {
        ...
        field(3; "Leave Type"; Code[30])
        {
            ...
            begin
                if ("User ID" <> '') or ("Employee No." <> '') then begin
                    HREmp.Reset;
                    if "Employee No." <> '' then begin
                        if not HREmp.Get("Employee No.") then
                            Error('Employee No. %1 was not found.', "Employee No.");
                    end else begin
        ...
        field(4; "Days Applied"; Decimal)
        {
            ...
            begin
                ...
                if HRLeaveCal.FindFirst() then begin
                   ...
                   //HRLeaveAlloc.SetFilter("Posting Type",'%1|%2', HRLeaveAlloc."Posting Type"::Normal, HRLeaveAlloc."Posting Type"::"Carry Forward");
                  if HRLeaveAlloc.FindSet() then begin
                   HRLeaveAlloc.CalcSums("No. Of days");
                   "Current Total Leave Taken" := -HRLeaveAlloc."No. Of days";
                   Modify();
                  end;
                    ...
                    if HRLeaveAlloc.FindSet then begin
                        HRLeaveAlloc.CalcSums("No. Of days");
                        "Carry Forward" := HRLeaveAlloc."No. Of days";
                        Modify;
                    end;
                    ...
                    if HRLeaveAlloc.FindSet then begin
                        HRLeaveAlloc.CalcSums("No. Of days");
                        "Carry Forward Balance" := HRLeaveAlloc."No. Of days";
                        Modify;
                    end;
                    ...
                    if HRLeaveAlloc.FindSet then begin
                        ...
                        "Current Leave Balance" := ("Allocated Days" + "Reimbursed Days") - ("Current Total Leave Taken")+"Carry Forward Balance";
                        Modify;
                    end;
                    ...
                    if Annual = true then begin
                        ...
                        if ("Allocated Days" + "Reimbursed Days") > 0 then begin
                            if noofdaysworked <> 0 then
                                "Earned Leave Days" :=((noofdaysworked / yearend) * ("Allocated Days" + "Reimbursed Days")) -
                        ...
                        if noofdaysworked<>0 then
                         "Earned Leave Days" := ((noofdaysworked * ("Allocated Days"+"Current Total Leave Taken"+("Carry Forward"+"Carry forward Balance"))/yearend)+"Carry forward Balance");
                         rec.modify();
...
if not CurrCalender.FindFirst() then
    Error('No active leave calendar exists.');

CarryForwardBalance := GetCarryForwardBalance("Employee No.", "Leave Type");

RemainingDays := "Days Applied";

//Consume Carry Forward first
if CarryForwardBalance > 0 then begin
    DaysToConsume := RemainingDays;

    if DaysToConsume > CarryForwardBalance then
        DaysToConsume := CarryForwardBalance;

    CreateLeaveAllocationEntry(
        CurrCalender,
        HREmp,
        HRLeaveAlloc."Posting Type"::"Carry Forward",
        DaysToConsume);

    RemainingDays := RemainingDays - DaysToConsume;
end;

//Consume Current Allocation
if RemainingDays > 0 then
    CreateLeaveAllocationEntry(
        CurrCalender,
        HREmp,
        HRLeaveAlloc."Posting Type"::Normal,
        RemainingDays);

//Mark Application as Posted
...
Modify();
        ...
        field(52; "Delivery Date"; Date)
        {
            ...
            begin
                ...
                if leavetype.FindFirst() then begin
                    ...
                    // Stamp Days Applied from leave type (e.g. 90) so portal/BC show duration.
                    if leavetype.Days > 0 then
                        rec."Days Applied" := leavetype.Days;
                    //Validate("Start Date");
        ...
        field(65; "Carry Forward"; Decimal)
        {
            Editable = false;
            caption = 'Carry Forward';
        }
        field(66;"Carry Forward Balance";Decimal){
    ...
    begin
        ...
        // Staff Details — portal passes Employee No.; prefer it over SOAP session UserId
        // so gender is validated against the correct employee card, not the service account.
        HREmp.Reset;
        if "Employee No." <> '' then begin
            if not HREmp.Get("Employee No.") then
                Error('Employee No. %1 was not found.', "Employee No.");
...
begin
    ...
    local procedure CreateLeaveAllocationEntry(
    CurrCalendar: Record "HR Leave Calendar.";
...
begin
    ...
    HRLeaveAlloc."Calendar Code" := CurrCalendar.Code;
    HRLeaveAlloc."No." := Employee."No.";
    HRLeaveAlloc."Staff Name" := Employee."Full Name";

    HRLeaveAlloc."Posting Date" := Today;

    HRLeaveAlloc."Entry Type" :=
        HRLeaveAlloc."Entry Type"::"Negative Adjustment";

    HRLeaveAlloc."Posting Type" := PostingType;

    HRLeaveAlloc."No. Of days" := -Days;

    HRLeaveAlloc."Leave Type" := "Leave Type";

    HRLeaveAlloc."Posting Description" :=
        Format(HRLeaveAlloc."Entry Type") + ' Allocation - ' + Format(Today);

    HRLeaveAlloc."Posted By" := UserId;
    HRLeaveAlloc.Posted := true;

    HRLeaveAlloc."Calendar Start Date" := CurrCalendar."Start Date";
    HRLeaveAlloc."Calendar End Date" := CurrCalendar."End Date";

    HRLeaveAlloc."Document No." := "Application Code";
    HRLeaveAlloc."Posting Source" :=
        HRLeaveAlloc."Posting Source"::Document;
...
local procedure GetCarryForwardBalance(EmployeeNo: Code[50]; LeaveType: Code[50]): Decimal
var
    LeaveAlloc: Record "HR Leave Allocation";
begin
    LeaveAlloc.Reset();
    LeaveAlloc.SetRange("No.", EmployeeNo);
    LeaveAlloc.SetRange("Leave Type", LeaveType);
    LeaveAlloc.SetRange("Posting Type", LeaveAlloc."Posting Type"::"Carry Forward");

    if LeaveAlloc.FindSet() then begin
        LeaveAlloc.CalcSums("No. Of days");
        exit(LeaveAlloc."No. Of days");
    end;
