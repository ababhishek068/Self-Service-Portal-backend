/** BC "Pending"/"Open" and application DB "Draft" are editable; Asset Transfer starts at "New". */
export function isEditableRequestStatus(
  ...
  return (
    status === 'Draft' ||
    status === 'Open' ||
    (module === 'assetTransfer' && status === 'New')
  )
}
...
export function canRequestApproval(
  ...
  if (module === 'assetTransfer') return status === 'New' || status === 'Open'
