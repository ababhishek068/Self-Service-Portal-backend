...
export const workTicketHeaderSchema = z.object({
  ...
  gkNo: z.string().default(''),
  type: z.string().default('Flight Booking'),
  travelerEmployeeNo: z.string().min(1, 'Traveler is required'),
  flightFrom: z.string().min(2, 'Departure location is required'),
  flightTo: z.string().min(2, 'Destination is required'),
  departureDate: dateField,
  returnDate: optionalText,
  ticketClass: z.string().min(1, 'Ticket class is required'),
  airlinePreference: optionalText,
  justification: z.string().min(3, 'Booking justification is required'),
})
...
export const assetTransferHeaderSchema = z
  .object({
    ...
    assetConditionDescription: optionalText,
