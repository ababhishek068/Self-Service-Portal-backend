...
function isMedicalClaim(claimType: unknown) {
  const raw = String(claimType ?? '').trim().toUpperCase()
  const code = raw.split(' - ')[0]?.trim() ?? raw
  return code === 'MEDICAL' || code.startsWith('MED')
}
...
function claimNeedsRefundField(claimType: unknown) {
  return isMedicalClaim(claimType) || isOtherClaim(claimType)
}
...
async function applyMedicalClaimAmounts(values: FieldValues, form: UseFormReturn<FieldValues>) {
  ...
  try {
    ...
    const coverageFromBc = Number(data.CoveragePercent ?? data.coveragePercent ?? 0)
    ...
    if (coverageFromBc > 0) {
      form.setValue('coveragePercent', coverageFromBc, { shouldValidate: true })
    } else if (medicalAmount > 0 && amount > 0) {
      form.setValue('coveragePercent', Math.round((amount / medicalAmount) * 100), { shouldValidate: true })
    }
  } catch {
...
export function StaffClaim({ medicalOnly = false }: { medicalOnly?: boolean }) {
  ...
  const claimTypeChoices = useMemo(() => {
    const base =
      claimTypesQuery.options.length > 0 ? claimTypesQuery.options : claimTypeOptions
    return medicalOnly ? medicalClaimTypeOptions(base) : base
  }, [claimTypesQuery.options, medicalOnly])
  ...
  return (
    <MultiStepRequestPage
      ...
      description={
        medicalOnly
          ? 'Create a medical claim header, then add claim lines with hospital category and medical amount.'
          : 'Create a staff claim header, then add claim lines. Choose the claim type from Business Central — medical claims need hospital category; other claim types use a manual amount.'
      }
      module={module}
      ...
      line={{
        ...
        fields: [
          ...
          },
          {
            ...
            type: 'number',
            readOnlyWhen: (values) => isMedicalClaim(values.claimType),
            visibleWhen: (values) => claimNeedsRefundField(values.claimType),
          },
        ...
        columns: [
          ...
          { key: 'hospitalCategory', header: 'Hospital Category', format: (value) => hospitalCategoryLabel(value) },
          { key: 'medicalAmount', header: 'Medical Amount', format: (value) => formatCurrency(Number(value ?? 0)) },
          {
            key: 'coveragePercent',
            header: 'Coverage %',
            format: (value) => (Number(value ?? 0) > 0 ? `${Number(value)}%` : '—'),
          },
