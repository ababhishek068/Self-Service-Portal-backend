    ...
    <PageWrapper title="Dashboard">
      ...
      {summary.isLoading || (canFetchSummary && !summary.data && !summary.isError) ? (
        ...
        <div className="space-y-6">
          ...
          {typeof tileValues.leaveBalance === 'number' ? (
            <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-4 shadow-sm">
              <div className="flex items-center justify-between gap-3">
                <div>
                  ...
                  <p className="mt-1 text-2xl font-bold text-[var(--portal-navy)]">
                    {Number.isInteger(tileValues.leaveBalance)
                      ? tileValues.leaveBalance
                      : tileValues.leaveBalance.toFixed(2).replace(/\.?0+$/, '')}{' '}
                    <span className="text-sm font-medium text-slate-600">day(s)</span>
                  </p>
                </div>
                <Button asChild variant="outline" size="sm">
                  <Link to="/hr/leave-request">Apply leave</Link>
                </Button>
              </div>
            </div>
          ) : null}
