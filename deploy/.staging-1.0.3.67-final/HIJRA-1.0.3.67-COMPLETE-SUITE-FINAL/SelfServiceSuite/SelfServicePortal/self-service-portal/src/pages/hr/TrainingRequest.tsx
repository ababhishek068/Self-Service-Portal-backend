...
const trainingColumns: DataTableColumn<PortalRequest>[] = [
  ...
  { id: 'trainingNeed', header: 'Training Need', cell: (row) => row.title },
  {
    id: 'department',
    header: 'Department',
    cell: (row) => String(row.payload?.department ?? row.departmentName ?? '—'),
  },
  {
    id: 'period',
    header: 'Training Period',
    cell: (row) => {
      const start = row.payload?.periodStart
      const end = row.payload?.periodEnd
  ...
  },
  {
    id: 'vendor',
    header: 'Provider',
    cell: (row) => String(row.payload?.vendor ?? '—'),
  },
  {
    id: 'cost',
    header: 'Est. Cost',
    cell: (row) => String(row.payload?.estimatedBudget ?? '—'),
  },
...
export function TrainingRequest() {
  ...
  const defaultDepartment = employee?.departmentCode || employee?.departmentName || ''
    ...
    <RequestFormPage
        ...
        createModuleRequest(module, {
          ...
          title: String(
            values.trainingNeed === '__OTHER__'
              ...
              : values.trainingNeed || 'Training Request',
          ),
        })
      }
      moduleConfig={module}
      ...
      fields={[
        ...
        { name: 'remark', label: 'Remark (optional)', type: 'textarea' },
        {
          ...
          label: 'Additional training needs (optional)',
          type: 'textarea',
          placeholder: 'One training course per line — each creates a separate ERP training application',
        },
      ]}
    />
  )
