...
interface DataTableProps<T> {
  ...
  emptyTitle?: string
  /** Set to `null` to hide the default “Create a new request…” subtitle. */
...
export function DataTable<T>({
  ...
  getRowId,
  emptyTitle,
  emptyHint = 'Create a new request to get started',
  emptyAction,
  selectedRowId,
  compact = false,
  onRowClick,
      ...
      <Table>
        ...
        <TableBody key={page} className="portal-row-stagger">
          {pagedRows.length > 0 ? (
            ...
            <TableRow className="hover:bg-transparent">
              <TableCell colSpan={columns.length} className="py-10 text-center align-middle">
                <div className="mx-auto flex max-w-md flex-col items-center gap-1">
                  <p className="text-sm font-medium text-slate-500">{emptyTitle ?? '*** No records Found ***'}</p>
