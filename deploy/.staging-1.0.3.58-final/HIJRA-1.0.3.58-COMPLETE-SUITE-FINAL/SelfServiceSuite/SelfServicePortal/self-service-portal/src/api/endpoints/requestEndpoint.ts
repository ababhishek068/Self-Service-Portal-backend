...
/** UAT E (R54): post an approved asset transfer (updates the FA + transfer ledger in BC). */
export async function postAssetTransfer(id: string) {
  requireAuthApiUrl()
  return authPost<PortalRequest>(`/api/requests/${encodeURIComponent(id)}/post-asset-transfer`, {})
}

/** UAT Maintenance R39: assign an internal technician. */
export async function assignMaintenanceTechnician(id: string, technicianNo: string) {
  requireAuthApiUrl()
  return authPost<PortalRequest>(`/api/requests/${encodeURIComponent(id)}/maintenance/assign-technician`, {
    technicianNo,
  })
}

/** UAT Maintenance R40: requestor confirms receipt of the maintained FA. */
export async function confirmMaintenanceReceipt(id: string, remarks = '') {
  requireAuthApiUrl()
  return authPost<PortalRequest>(`/api/requests/${encodeURIComponent(id)}/maintenance/confirm-receipt`, {
    remarks,
  })
}

/** UAT Maintenance R41/R42: save odometer + next service KM. */
export async function saveMaintenanceOdometer(
  id: string,
  payload: { currentOdometer: number; lastServiceOdometer?: number },
) {
  requireAuthApiUrl()
  return authPost<PortalRequest>(`/api/requests/${encodeURIComponent(id)}/maintenance/odometer`, payload)
}
