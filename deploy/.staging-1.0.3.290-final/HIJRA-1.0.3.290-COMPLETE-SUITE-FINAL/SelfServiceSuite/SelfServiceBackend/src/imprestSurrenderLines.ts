...
export function prepareImprestSurrenderLinesForSave(
  ...
  for (const line of incomingLines) {
    const lineNo = imprestSurrenderLineNo(line)
    const peers = incomingByLineNo.get(lineNo) ?? []
    peers.push(line)
    incomingByLineNo.set(lineNo, peers)
  ...
  const resolveIncoming = (
    ...
    // Unique G/L account always wins. Colliding Line No (both 10000 when Entry No
    // is missing) used to copy Per Diem spend onto Transport.
    if (accountNo) {
      const sameAccountIncoming = incomingLines.filter(
        (line) => imprestSurrenderAccountNo(line) === accountNo,
      )
      if (sameAccountIncoming.length === 1) return sameAccountIncoming[0]
    }
    const byLine = incomingByLineNo.get(lineNo) ?? []
    if (accountNo) {
      const sameAccount = byLine.find((line) => imprestSurrenderAccountNo(line) === accountNo)
      if (sameAccount) return sameAccount
    }
    if (byLine.length === 1) return byLine[0]
    if (!accountNo) return undefined
    const sameAccountExisting = existingLines.filter(
      (line) =>
        imprestSurrenderAccountNo(line) === accountNo && imprestSurrenderLineAmount(line) > 0,
    )
    if (sameAccountExisting.length === 1) return incomingByAccount.get(accountNo)
    return undefined
  }
