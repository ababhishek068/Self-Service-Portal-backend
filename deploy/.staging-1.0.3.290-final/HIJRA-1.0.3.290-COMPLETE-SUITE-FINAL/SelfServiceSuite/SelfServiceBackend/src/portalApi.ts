...
import { enrichSalaryAdvanceLines, resolveSalaryAdvanceQueueAmount, salaryAdvanceLinesTotal } from './salaryAdvanceAmount.js'
import {
...
import {
  ...
  assertNoDuplicateTransportRequest,
  ...
  prepareMaintenanceFixedAssetForBc,
  prepareMaintenanceVehicleAssetForBc,
...
/** Normalize HTML date input / dd/mm/yyyy to ISO yyyy-mm-dd for BC SOAP/OData. */
function normalizePortalIsoDate(value: unknown) {
...
function trainingPeriodFields(row: ODataRecord, saved: ODataRecord) {
  const fromSavedStart = filterBcDate(text(saved, ['periodStart']))
  const fromSavedEnd = filterBcDate(text(saved, ['periodEnd']))
  if (fromSavedStart) {
    return {
      periodStart: fromSavedStart,
      periodEnd: fromSavedEnd || fromSavedStart,
    }
  }
  const appDate = filterBcDate(text(row, ['ApplicationDate', 'Application_Date']))
  const fromHeader = filterBcDate(text(row, ['FromDate', 'From_Date', 'StartDate']))
  const toHeader = filterBcDate(text(row, ['ToDate', 'To_Date', 'EndDate']))
  if (fromHeader && (!appDate || fromHeader !== appDate)) {
    return { periodStart: fromHeader, periodEnd: toHeader || fromHeader }
  }
  return { periodStart: '', periodEnd: '' }
}

function buildTrainingAssessmentDetails(body: Record<string, unknown>, trainingCourseCode: string) {
  return {
    trainingNeed: trainingCourseCode,
    trainingCourseTitle: body.trainingCourseTitle ?? body.title ?? '',
    purpose: body.purpose ?? body.comments ?? body.justification ?? '',
    otherTrainingName: body.otherTrainingName ?? body.additionalTrainingNeeds ?? '',
    ...
    vendor: body.vendor ?? body.provider ?? '',
    estimatedBudget: body.estimatedBudget ?? body.estimatedCost ?? '',
    remark: body.remark ?? '',
    department: body.department ?? '',
    periodStart: normalizePortalIsoDate(body.periodStart),
    periodEnd: normalizePortalIsoDate(body.periodEnd),
  }
}

/** Stamp HR Training Applications From/To Date when SOAP assessment save omits header sync. */
async function syncTrainingHeaderPeriodViaOData(
  applicationNo: string,
  details: ReturnType<typeof buildTrainingAssessmentDetails>,
) {
  const no = applicationNo.trim()
  const start = details.periodStart
  const end = details.periodEnd || start
  if (!no || !start) return false
  const bases = [config.BC_ODATA_BASE_URL].filter(Boolean)
  const bodies: Array<Record<string, unknown>> = [
    { From_Date: start, To_Date: end },
    { FromDate: start, ToDate: end },
    { From_Date: start, To_Date: end, Duration: details.durationDays || undefined },
  ]
  for (const base of bases) {
    for (const body of bodies) {
      ...
      try {
        await patchODataRecord(base, 'QyTrainingApplicationHeader', no, payload)
        return true
      } catch {
...
function soapReturnedTrue(value: unknown) {
  return String(value ?? '').toLowerCase() === 'true'
}

async function resolveProcurementPlanOrg(authUser: AuthUser) {
  const employee = await fetchMergedEmployeeRecord(authUser.employeeNo).catch(() => null)
  const org = employee
    ? await resolveEmployeeOrgDisplayFields(employee, authUser)
    : {
        orgKind: (/district/i.test(String(authUser.district ?? '')) ? 'district' : 'department') as
          | 'department'
          | 'district',
        departmentCode: String(authUser.department ?? ''),
        districtCode: String(authUser.district ?? ''),
        sectorCode: '',
        scopeCode: String(authUser.department ?? ''),
      }
  return {
    orgKind: org.orgKind,
    globalDim1: org.sectorCode || '',
    globalDim2: org.scopeCode || org.departmentCode || org.districtCode || String(authUser.department ?? ''),
  }
}

function mapProcurementPlanHeader(row: ODataRecord) {
  const dim2 = text(row, ['GlobalDimension2', 'Global_Dimension_2', 'Department'])
  const dim2Name = text(row, ['GlobalDimension2Name', 'DepartmentName', 'Department_Name'], dim2)
  const kind =
    /district/i.test(dim2Name) || /district/i.test(dim2) ? ('district' as const) : ('department' as const)
  return {
    BudgetName: text(row, ['BudgetName', 'Budget_Name']),
    GlobalDimension1: text(row, ['GlobalDimension1', 'Global_Dimension_1', 'Sector']),
    GlobalDimension2: dim2,
    ProcurementPlanPeriod: text(row, ['ProcurementPlanPeriod', 'PlanPeriod', 'Plan_Period']),
    Status: text(row, ['Status'], 'Open'),
    FinanceOrgKind: kind,
  }
}

function mapProcurementPlanLine(row: ODataRecord) {
  return {
    BudgetName: text(row, ['BudgetName', 'Budget_Name']),
    Department: text(row, ['Department', 'GlobalDimension2', 'Global_Dimension_2']),
    Type: text(row, ['Type']),
    TypeNo: text(row, ['TypeNo', 'Type_No']),
    Description: text(row, ['Description']),
    Quantity: number(row, ['Quantity']),
    UnitCost: number(row, ['UnitCost', 'Unit_Cost']),
    Amount: number(row, ['Amount']),
    PlanDate: text(row, ['PlanDate', 'Plan_Date']),
    ProcurementPlanPeriod: text(row, ['ProcurementPlanPeriod', 'PlanPeriod', 'Plan_Period']),
    GlobalDimension1: text(row, ['GlobalDimension1', 'Global_Dimension_1']),
  }
}

function procurementPlanAllowedOrgCodes(authUser: AuthUser, org: { globalDim2: string }) {
  return new Set(
    [org.globalDim2, authUser.department, ...(authUser.permissionDepartments ?? [])]
      .map((value) => String(value ?? '').trim().toLowerCase())
      .filter(Boolean),
  )
}

function procurementSoapMessage(action: string, error: unknown) {
  const message = error instanceof Error ? error.message : String(error)
  if (/404|not found|does not exist|unpublished/i.test(message)) {
    return `Business Central ${action} is not published. Ask Felix to publish CuPortalFacility and the procurement plan queries.`
  }
  return message
}

async function persistFuelFacilityCompanion(
  authUser: AuthUser,
  requisitionNo: string,
  body: Record<string, unknown>,
) {
  const standard = await callSoapMethod(
    'SaveFuelRequestStandard',
    {
      requisitionNo,
      employeeNo: authUser.employeeNo,
      vehicleNo: body.vehicleNo ?? '',
      currentOdometer: Number(body.odometer ?? 0),
      requestedLitres: Number(body.quantity ?? 0),
    },
    facilitySoapEndpoint,
  )
  if (!soapReturnedTrue(standard.returnValue)) {
    throw portalError('Business Central did not save the fuel standard/odometer details', 502)
  }
  const remark = String(body.consumptionRemark ?? '').trim()
  if (!remark) return
  try {
    await callSoapMethod(
      'SaveFuelConsumptionRemark',
      { requisitionNo, consumptionRemark: remark },
      facilitySoapEndpoint,
    )
  } catch {
    // Companion SOAP is additive; purpose/extra Issue Description still carries the remark when unpublished.
  }
}
...
function enrichTrainingRow(
  ...
  const period = trainingPeriodFields(row, saved)
  return {
    ...row,
    ...saved,
    trainingNeed,
...
export const PORTAL_API_BUILD =
  'v1.0.3.290 — 2026-08-19 (all undeployed 16–19 Aug fixes: finance org, salary advance, imprest surrender, petty cash, procurement budget)'
...
const LOOKUP_SPECS: Record<string, LookupSpec> = {
  ...
  'maintenance-technicians': {
    service: 'QyHREmployee',
    valueKeys: ['No'],
    labelKeys: ['FullName', 'Full_Name', 'Name', 'No'],
    meta: { jobTitle: ['JobTitle', 'Job_Title'] },
  },
  vehicles: {
  ...
  departments: {
    // Purchase Header."Requesting Department" TableRelations to Departments (AL comment),
    // codes like FACILTY / FIN. Organogram level is Department OR District (same DEPART/DIST tier).
    // Dimension DEPART/DIST is only a fallback when Departments List is empty/unpublished.
...
/** Fixed Asset list: pick by FA No / description. Untagged assets are stamped before SOAP. */
export function fixedAssetLookupOption(row: ODataRecord, value: string) {
  ...
  const label = tag
    ? `${description} · ${value} · Tag ${tag}`
    : description && description !== value
      ? `${description} · ${value} · (no tag yet — will use asset no.)`
      : `${value} · (no tag yet — will use asset no.)`
  return {
    value,
    label,
    meta: {
...
async function fixedAssetTagIndex() {
  return cacheGetOrLoad('fixed-asset-tag-index', FA_TAG_INDEX_TTL_MS, async () => {
    const rows = (await fetchOData('QyFixedAssets', { $top: 2000 }).catch(() => [])) as
      | ODataRecord[]
      | null
...
const MAINTENANCE_TECHNICIAN_JOB_PATTERN =
  /technician|mechanic|artisan|electrician|auto\s*electric|plumber|carpenter|mason|welder|facilit|propert|maintenance|plant\s*operat|estate|building\s*service|works\s*(officer|supervisor|assistant)|generator|hvac|handyman|foreman|fitter/i

function maintenanceTechnicianJobTitle(row: ODataRecord) {
  return [
    text(row, ['JobTitle', 'Job_Title', 'JobTitleDescription', 'Job_Title_Description']),
    text(row, ['TemporaryJob', 'Temporary_Job']),
    text(row, ['Actingjob', 'Acting_Job', 'ActingJob', 'Acting_Job_ID']),
    text(row, ['JobID', 'Job_ID', 'JobCode', 'Job_Code']),
  ]
    .filter(Boolean)
    .join(' ')
}

function isActiveHrEmployee(row: ODataRecord) {
  const status = text(row, ['Status']).trim().toLowerCase()
  return !status || status === 'active'
}

function maintenanceTechnicianAllowlist() {
  return new Set(
    (config.BC_MAINTENANCE_TECHNICIAN_EMPNOS ?? []).map((no) => String(no).trim().toUpperCase()).filter(Boolean),
  )
}

export function isMaintenanceTechnicianCandidate(row: ODataRecord) {
  if (!isActiveHrEmployee(row)) return false
  const empNo = text(row, ['No', 'No_']).trim().toUpperCase()
  if (empNo && maintenanceTechnicianAllowlist().has(empNo)) return true
  return MAINTENANCE_TECHNICIAN_JOB_PATTERN.test(maintenanceTechnicianJobTitle(row))
}

/** Prefer job-title / allow-list technicians; if none exist in HR, fall back to all active staff (UAT). */
export function filterMaintenanceTechnicianRows(rows: ODataRecord[]) {
  const active = rows.filter(isActiveHrEmployee)
  const matched = active.filter(isMaintenanceTechnicianCandidate)
  return {
    rows: matched.length > 0 ? matched : active,
    usedFallback: matched.length === 0 && active.length > 0,
  }
}

async function assertMaintenanceTechnicianNo(technicianNo: string) {
  const rows = (await fetchOData('QyHREmployee', {
    $filter: `No eq '${odataString(technicianNo)}'`,
    $top: 1,
  }).catch(() => [])) as ODataRecord[] | null
  const row = Array.isArray(rows) ? rows[0] : undefined
  if (!row || !isActiveHrEmployee(row)) {
    throw portalError(
      'Select an internal technician from the technician list.',
      422,
      'TECHNICIAN_INVALID',
    )
  }
  if (isMaintenanceTechnicianCandidate(row)) return row

  // Same fallback as the lookup: when HR has no titled technicians, any active employee is allowed.
  const all = (await fetchOData('QyHREmployee', { $top: 5000 }).catch(() => [])) as ODataRecord[] | null
  const anyTitled = Array.isArray(all) ? all.some(isMaintenanceTechnicianCandidate) : false
  if (!anyTitled) return row
...
async function approverIdCandidates(authUser: AuthUser): Promise<string[]> {
  ...
  const add = (value: unknown) => {
    ...
    if (!trimmed) return
    ids.add(trimmed)
    // BC workflows sometimes stamp DOMAIN\USER or USER@domain while the portal
    // login / User Setup only has the bare account — include both forms.
    const bare = trimmed.includes('\\')
      ? trimmed.split('\\').pop()
      : trimmed.includes('@')
        ? trimmed.split('@')[0]
        : ''
    if (bare && bare !== trimmed) ids.add(bare)
  }
  add(authUser.userID)
  ...
  if (authUser.employeeNo) {
    ...
    // Workflow Approver ID can be the HR Employee User ID even when that value is
    // not present on User Setup — without this, history shows the person (name
    // resolved via HR) but their Pending Approval queue never lists the document.
    ...
    for (const row of Array.isArray(employeeRows) ? employeeRows : []) {
      add(text(row, ['UserID', 'User_ID', 'EmployeeUserID', 'Employee_User_ID']))
      add(text(row, ['No', 'EmployeeNo', 'Employee_No']))
      add(text(row, ['CompanyEMail', 'Company_E_Mail', 'EMail', 'Email']))
    }
  }

  // Also reverse-lookup User Setup by the login User ID in case EmployeeNo on the
  // auth session is blank / stale but User Setup still links the account.
  if (authUser.userID) {
    const byUser = (await fetchOData('QyUserSetup', {
      $filter: `UserID eq '${odataString(authUser.userID)}'`,
      $top: 5,
    }).catch(() => [])) as ODataRecord[] | null
    for (const row of Array.isArray(byUser) ? byUser : []) {
      add(text(row, ['UserID', 'User_ID']))
      add(text(row, ['EmployeeNo', 'Employee_No']))
      add(text(row, ['SalespersonCode', 'Salespers_Purch_Code', 'SalespersPurchCode', 'ApproverID']))
      const linkedEmployee = text(row, ['EmployeeNo', 'Employee_No'])
      if (linkedEmployee && linkedEmployee !== authUser.employeeNo) {
        const siblingUsers = (await fetchOData('QyUserSetup', {
          $filter: `EmployeeNo eq '${odataString(linkedEmployee)}'`,
          $top: 50,
        }).catch(() => [])) as ODataRecord[] | null
        for (const sibling of Array.isArray(siblingUsers) ? siblingUsers : []) {
          add(text(sibling, ['UserID', 'User_ID']))
          add(text(sibling, ['SalespersonCode', 'Salespers_Purch_Code', 'SalespersPurchCode', 'ApproverID']))
        }
      }
    }
  }
...
function approvalEntrySentAt(row: ODataRecord) {
  const raw = text(row, [
...
/** BC QyApprovalEntry is a Query — OData OrderBy is not supported; sort in memory. */
function sortApprovalEntriesNewestFirst(rows: ODataRecord[]) {
  return [...rows].sort((left, right) => {
    const sentDiff = approvalEntrySentAt(right) - approvalEntrySentAt(left)
    if (sentDiff !== 0) return sentDiff
    const leftEntry = Number(text(left, ['EntryNo', 'Entry_No']) || 0)
    const rightEntry = Number(text(right, ['EntryNo', 'Entry_No']) || 0)
...
async function fetchApproverEntriesForDocument(no: string, authUser: AuthUser) {
  ...
  return sortApprovalEntriesNewestFirst(Array.isArray(rows) ? rows : [])
}

async function fetchAllApprovalEntriesForDocument(no: string) {
  const rows = (await fetchOData('QyApprovalEntry', {
...
function approvalDisplayStatus(
  ...
  if (entryRows.some((row) => text(row, ['Status']) === 'Open')) return 'Pending Approval'
  const latest = entryRows[0]
...
async function approvalQueueItems(rows: ODataRecord[]) {
  ...
  // Only probe when TableID / Document Type is ambiguous. Known table IDs
  // (petty cash 50887, interbank 50883, etc.) must not hit BC again — that made
  // the approvals queue hang for minutes under load.
  const ambiguousRows = rows.filter((row) => {
    ...
    if (
      ...
      tableId === APPROVAL_TABLE_IDS.imprest ||
      tableId === APPROVAL_TABLE_IDS.imprestSurrender ||
      tableId === APPROVAL_TABLE_IDS.staffClaim ||
      tableId === APPROVAL_TABLE_IDS.salaryAdvance ||
      tableId === APPROVAL_TABLE_IDS.leave ||
      tableId === APPROVAL_TABLE_IDS.transport ||
      tableId === APPROVAL_TABLE_IDS.gatePass ||
      tableId === APPROVAL_TABLE_IDS.assetTransfer ||
      tableId === APPROVAL_TABLE_IDS.transferOrder ||
      tableId === APPROVAL_TABLE_IDS.workTicket
    ) {
      return false
    }
    return (
      syncModule === 'purchaseRequisition' ||
      approvalTableIdsFor('storeRequisition').includes(tableId) ||
  ...
  // Cap concurrent probes so one slow BC lookup cannot stall the whole queue.
  const uniqueAmbiguous = new Map<string, ODataRecord>()
  for (const row of ambiguousRows) {
    const documentNo = text(row, ['DocumentNo', 'Document_No'])
    if (!documentNo) continue
    const key = normalizedApprovalDocKey(documentNo)
    if (!uniqueAmbiguous.has(key) && !sharedModules.has(key)) {
      uniqueAmbiguous.set(key, row)
    }
  }
  await mapInBatches([...uniqueAmbiguous.entries()], 4, async ([key, row]) => {
    const documentNo = text(row, ['DocumentNo', 'Document_No'])
    if (!documentNo) return
    try {
      const module = (await Promise.race([
        resolveApprovalModuleFromEntry(row, documentNo, approvalModuleFromEntry),
        new Promise<string>((resolve) =>
          setTimeout(() => resolve(approvalModuleFromEntry(row)), 2500),
        ),
      ])) as SupportedFrontendModule
      if (isSupportedModule(module)) ambiguousModules.set(key, module)
    } catch {
      // keep sync fallback from approvalModuleFromEntry
    }
  })
...
async function fetchApprovalQueueSourceHeader(
  module: SupportedFrontendModule,
  documentNo: string,
) {
  const spec = findFrontendModuleSpec(module)
  if (!spec || !documentNo) return null

  const filterKeys =
    module === 'fuelRequest' || module === 'maintenance'
      ...
      : module === 'transport'
        ...
        : module === 'purchaseRequisition'
          ...
          : module === 'storeRequisition'
            ? ['No', 'RequistionNo', 'RequisitionNo']
            : module === 'imprestSurrender'
              ? ['No', 'Imprest_No', 'ImprestNo']
              : ['No', 'DocumentNo', 'Document_No']

  for (const key of filterKeys) {
    try {
      const filterParts = [`${key} eq '${odataString(documentNo)}'`]
      if (module === 'purchaseRequisition' && spec.extraListFilter) {
        filterParts.push(`(${spec.extraListFilter})`)
      }
      if (module === 'fuelRequest') {
        filterParts.push(`(Type eq 'Fuel' or Type eq '0')`)
      }
      if (module === 'maintenance') {
        filterParts.push(`(Type eq 'Maintenance' or Type eq '1')`)
      }
      const rows = (await fetchOData(spec.headerService, {
        $filter: filterParts.join(' and '),
        $top: 1,
      })) as ODataRecord[] | null
      if (Array.isArray(rows) && rows[0]) return { spec, header: rows[0] }
    } catch {
      // try next key / unfiltered fallback below
    }
  }
  ...
  if (module === 'transport') {
    ...
    const wanted = documentNo.trim().toUpperCase()
    const match = fetched.find((row) =>
      filterKeys.some(
        (key) => text(row, [key]).trim().toUpperCase() === wanted,
      ),
    )
    if (match) return { spec, header: match }
  }

  return null
}

function financeHeaderAmount(header: ODataRecord) {
...
async function enrichApprovalQueueAmounts(
  ...
  return mapInBatches(items, 6, async (item) => {
    try {
      if (item.module === 'salaryAdvance') {
        const sourced = await fetchApprovalQueueSourceHeader(item.module, item.requestNo)
        if (!sourced) return item
        let lines: ODataRecord[] = []
        if (resolveSalaryAdvancePercentage({}, sourced.header) <= 0) {
          lines = await listPortalModuleLines(sourced.spec, sourced.header, item.requestNo).catch(
            () => [] as ODataRecord[],
          )
        }
        const amount = await resolveSalaryAdvanceQueueAmount(sourced.header, lines)
      ...
      if (item.module === 'pettyCash') {
        ...
        const amount = financeHeaderAmount(header)
        return amount > 0 ? { ...item, amount } : item
      }
      if (item.module === 'pettyCashReplenishment') {
      ...
      const sourced = await fetchApprovalQueueSourceHeader(item.module, item.requestNo)
      if (!sourced) return item
      const { spec, header } = sourced

      if (
        item.module === 'storeRequisition' ||
        item.module === 'purchaseRequisition' ||
        item.module === 'transferOrder' ||
        item.module === 'fuelRequest'
      ) {
        // Approvals list: header totals only — per-doc line fetches made the
        // queue multi-second when many store/purchase items were pending.
        const summary = facilityListSummary(item.module, header, [])
        if (summary.amount && summary.amount > 0) {
          return { ...item, amount: summary.amount }
        }
        return item
      }
      ...
      if (
        item.module === 'imprest' ||
        item.module === 'imprestSurrender' ||
        item.module === 'staffClaim'
      ) {
        const amount = financeHeaderAmount(header)
        return amount > 0 ? { ...item, amount } : item
      }
...
async function filterApprovalEntriesWithExistingSource(rows: ODataRecord[]) {
  ...
  // Only leave benefits from source-existence filtering. For every other module
  // approvalSourceDocumentExists fails open, so the BC header probes were pure
  // latency on the approvals queue (often 10–60s with many pending items).
  const leaveNos = rows
    .map((row) => {
      const item = approvalQueueItem(row)
      return item.module === 'leave' ? item.requestNo : ''
    })
    .filter(Boolean)

  if (!leaveNos.length) return rows

  const existingLeave = await fetchExistingLeaveApplicationNos(leaveNos)
  const existingByModule = new Map<string, Set<string>>([['leave', existingLeave]])

  return rows.filter((row) => {
    const item = approvalQueueItem(row)
    if (!item.requestNo) return false
    if (item.module !== 'leave') return true
...
function mapStoreLine(row: ODataRecord, index: number) {
  ...
  const quantityIssuedReported = number(row, [
    'quantityIssued',
    'QuantityIssued',
    'Quantity_Issued',
    'QtyIssued',
  ...
  const lastQuantityIssued = number(row, [
    'lastQuantityIssued',
    'LastQuantityIssued',
    'Last_Quantity_Issued',
  ])
  // Some BC tenants expose issued qty only on Last Quantity Issued after store post.
  const quantityIssued =
    quantityIssuedReported > 0 ? quantityIssuedReported : lastQuantityIssued
...
export function facilityListSummary(
  ...
  if (module === 'fuelRequest') {
    ...
    return {
      ...
      ...(amount > 0 ? { TotalPriceofFuel: amount } : {}),
    }
  }
...
async function mappedModuleRows(
  ...
  if (
    ...
    // List view: use header totals only. Per-row line fetches were making
    // /requests and dashboard multi-second. Detail screens still load lines.
    return rows.map((row) => {
      const mapped = mapRequest(row, module as PortalModuleKey)
...
async function requestDetail(
  ...
  if (
    ...
    if (module === 'fuelRequest') {
      const consumptionRemark =
        text(payloadRow, ['ConsumptionRemark', 'Consumption_Remark', 'consumptionRemark']) ||
        text(payloadRow, ['IssueDescription', 'Issue_Description'])
      if (consumptionRemark) payloadRow = { ...payloadRow, consumptionRemark }
    }
  }
  if (module === 'maintenance') {
    payloadRow = {
  ...
  if (isFinanceDetailModule(module)) {
    ...
    if (module === 'pettyCashReplenishment') {
      const requesterNo =
        text(row, ['EmployeeNo', 'Employee_No', 'StaffNo', 'Staff_No']) ||
        authUser.employeeNo
      const departmentLimit = await getPortalPettyCashDepartmentLimit(authUser, undefined, {
        employeeNo: requesterNo,
      })
      let branchLine = departmentLimit.limitSource === 'branch'
      let branchLabel =
        ('branchName' in departmentLimit ? departmentLimit.branchName : '') ||
        ('branchCode' in departmentLimit ? departmentLimit.branchCode : '') ||
        ''
      try {
        const emp = await fetchMergedEmployeeRecord(requesterNo)
        if (emp) {
          const org = await resolveEmployeeOrgDisplayFields(emp, authUser)
          if (org.orgKind === 'district' && (org.branchName || org.branchCode)) {
            branchLine = true
            branchLabel = org.branchName || org.branchCode || branchLabel
          }
        }
      } catch {
        // keep limit lookup labels
      }
      const limitSourceLabel = branchLine
        ? branchLabel || 'Branch'
        : departmentLimit.limitSource === 'branch'
          ? ('branchName' in departmentLimit ? departmentLimit.branchName : '') ||
            ('branchCode' in departmentLimit ? departmentLimit.branchCode : '') ||
            'Branch'
          : departmentLimit.departmentName || departmentLimit.departmentCode
      payloadRow = {
        ...payloadRow,
        PettyCashDepartmentLimit: departmentLimit.limit,
        PettyCashLimitConfigured: departmentLimit.configured,
        PettyCashLimitDepartment: limitSourceLabel,
        PettyCashLimitSource: branchLine
          ? 'Branch'
          : departmentLimit.limitSource === 'branch'
            ? 'Branch'
            : departmentLimit.limitSource === 'department'
              ? 'Department'
              : '',
        ...(branchLine && branchLabel
          ? {
              ResponsibilityCenter: branchLabel,
              Responsibility_Center: branchLabel,
            }
          : {}),
      }
    }
  ...
  if (module === 'salaryAdvance') {
    const staffNo =
      text(row, ['StaffNo', 'Staff_No', 'EmployeeNo', 'Employee_No']) ||
      (options.forApproval ? '' : authUser.employeeNo)
    const customerNo = text(row, ['CustomerNo', 'Customer_No']) || ''
    const rawLines = Array.isArray(lines) ? lines : []
    // Approvers must use the requester's salary base — never the approver's session salary.
    const sessionSalary =
      !options.forApproval &&
      staffNo.trim().toLowerCase() === authUser.employeeNo.trim().toLowerCase()
        ? authUser.monthlySalaryBase
        : undefined
    const enriched = await enrichSalaryAdvanceLines(row, rawLines, {
      employeeNo: staffNo || authUser.employeeNo,
      customerNo: customerNo || authUser.imprestNo || authUser.accountNumber || '',
      docNo: no,
      monthlySalaryBase: sessionSalary,
      fast: !options.forApproval,
      // Allow SOAP amount fallback on approval detail when line Amount is still 0.
      skipSoap: !options.forApproval,
    })
  ...
  const fuelSummary =
    module === 'fuelRequest'
      ? facilityListSummary('fuelRequest', payloadRow, mappedLines as ODataRecord[])
      : null
  const displayAmount =
    module === 'salaryAdvance'
      ? salaryAdvanceAmount > 0
        ...
        : mapped.amount
      : module === 'fuelRequest'
        ? Number(fuelSummary?.amount ?? 0) > 0
          ? Number(fuelSummary?.amount)
          : mapped.amount
        : salaryAdvanceAmount
  ...
  const storeRequesterName = storeRequestOwnedByCurrentUser
    ...
    : ''
  const fuelTotalFromBc = firstNonZeroNumber(payloadRow, [
    'TotalPriceofFuel',
    'Total_Price_of_Fuel',
    'TotalCost',
    'Total_Cost',
  ...
  const fuelComputedTotal = Number(fuelSummary?.amount ?? 0)
  return {
    ...mapped,
    ...
    payload: {
      ...payloadRow,
      ...(module === 'fuelRequest' && fuelComputedTotal > 0
        ? {
            // BC often leaves TotalPriceofFuel at 0 — expose price × litres for detail/approvals.
            ...(fuelTotalFromBc > 0 ? {} : { TotalPriceofFuel: fuelComputedTotal }),
            totalQuantity: Number(fuelSummary?.totalQuantity ?? 0) || undefined,
          }
        : {}),
...
async function fetchEmployeesByOrgScope(scopeCodes: string[], excludeEmployeeNo: string) {
  ...
  const exclude = odataString(excludeEmployeeNo)
  const base = `No ne '${exclude}' and Status eq 'Active'`
  for (const code of scopeCodes) {
    const escaped = odataString(code)
    const filters = [
      `${base} and GlobalDimension2Code eq '${escaped}'`,
      `${base} and GlobalDimension1Code eq '${escaped}'`,
      `${base} and District eq '${escaped}'`,
      `${base} and Department eq '${escaped}'`,
      `${base} and DepartmentCode eq '${escaped}'`,
    ]
    for (const filter of filters) {
      const rows = (await fetchOData('QyHREmployee', { $filter: filter }).catch(
        () => [],
      )) as ODataRecord[]
      for (const row of Array.isArray(rows) ? rows : []) {
...
async function fetchHodDepartmentStaff(authUser: AuthUser) {
  const scopeCodes = hodOrgScopeCodes(authUser)
  if (scopeCodes.length === 0) return [] as ODataRecord[]
  return fetchEmployeesByOrgScope(scopeCodes, authUser.employeeNo)
}

async function hodEmployeeInScope(authUser: AuthUser, employeeNo: string) {
  const wanted = employeeNo.trim().toUpperCase()
  if (!wanted) return null
  const rows = await fetchEmployeesByOrgScope(hodOrgScopeCodes(authUser), authUser.employeeNo)
  return rows.find((row) => text(row, ['No', 'EmployeeNo']).trim().toUpperCase() === wanted) ?? null
}
...
export function buildPortalApiRouter() {
  ...
  router.get(
    ...
    safe(async (req, res) => {
      ...
      const cacheKey = `lookup:${catalog}:${assetNoFilter || 'all'}:${spec.service}`
      let rows: unknown
      try {
        const odataFilter =
        ...
        const loadRows = async () => {
          let loaded = await fetchOData(spec.service, {
            ...(odataFilter ? { $filter: odataFilter } : {}),
            ...(!odataFilter && !spec.filter ? { $top: 2000 } : {}),
          })
          if (catalog === 'vehicle-tools' && assetNoFilter) {
            ...
            if (tagNo) {
              followUps.push(
                fetchOData(spec.service, { $filter: `TagNo eq '${odataString(tagNo)}'` }).catch(
                  () => [],
                ),
              )
            }
            ...
            for (const list of await Promise.all(followUps)) addRows(list)
            loaded = [...merged.values()]
          }
          return loaded
        }

        rows = await cacheGetOrLoad(cacheKey, LOOKUP_TTL_MS, async () => {
          try {
            const loaded = await loadRows()
            if (spec.fallback && (!Array.isArray(loaded) || loaded.length === 0) && !assetNoFilter) {
            ...
            spec = spec.fallback
            return fetchOData(spec.service, {
              ...(spec.filter ? { $filter: spec.filter } : {}),
              ...(!spec.filter ? { $top: 2000 } : {}),
            })
          }
        })
      } catch (error) {
      ...
      if (catalog === 'gate-pass-store-issue') {
        gatePassRowsForStoreIssue = await cacheGetOrLoad(
          'lookup:gate-pass-store-issue-rows',
          LOOKUP_TTL_MS,
          async () =>
            (await fetchOData('QyGatePass', {
              $filter: `Linkto eq 'Store Issue'`,
              $top: 500,
            }).catch(() => [])) as ODataRecord[],
        )
      }
      ...
      if (catalog === 'maintenance-technicians') {
        const filtered = filterMaintenanceTechnicianRows(lookupRows)
        lookupRows = filtered.rows
      }
            ...
            return {
              ...
              label:
                catalog === 'vehicle-tools'
                  ...
                  : catalog === 'petty-cash-types' || catalog === 'departments'
                    ? (() => {
                        const description = text(row, spec.labelKeys, value)
  ...
  router.post(
    ...
    safe(async (req, res) => {
      ...
      if (module === 'transport') {
        // Excel VR_16 / Aug 7: same user + same route + same trip date.
        await assertNoDuplicateTransportRequest(authUser, {
          origin: String(
            req.body?.commencement ?? req.body?.commenceFrom ?? req.body?.startingFrom ?? '',
          ),
          destination: String(req.body?.destination ?? ''),
          tripDate: String(req.body?.dateOfTrip ?? req.body?.tripDate ?? ''),
          tripTime: String(
      ...
      if (module === 'maintenance') {
        ...
        let resolvedFaReference = await resolveMaintenanceFaReference(
          maintenanceRequestType,
          String(req.body?.faTagNumber ?? ''),
          String(req.body?.vehicleNo ?? ''),
        )
        // CuPortalFacility.MarkAsMaintenanceRequest still calls EnsureFixedAssetTag on
        // UAT. Stamp the FA tag via OData first so even the old Error path exits early.
        if (maintenanceRequestType === 1) {
          resolvedFaReference = await prepareMaintenanceFixedAssetForBc(resolvedFaReference)
        } else if (maintenanceRequestType === 2) {
          await prepareMaintenanceVehicleAssetForBc(String(req.body?.vehicleNo ?? ''))
          if (resolvedFaReference) {
            resolvedFaReference = await prepareMaintenanceFixedAssetForBc(resolvedFaReference)
          }
        }
        const result = await callSoapMethod(
      ...
      if (module === 'fuelRequest') {
        await persistFuelFacilityCompanion(authUser, no, (req.body ?? {}) as Record<string, unknown>)
      }
      if (module === 'training') {
        const assessmentDetails = buildTrainingAssessmentDetails(
          { ...(req.body ?? {}), department: req.body?.department ?? authUser.department ?? '' },
          trainingCourseCode,
        )
        const result = await callSoapMethod(
          'SaveTrainingAssessment',
          {
            ...
            employeeNo: authUser.employeeNo,
            requesterUserId: authUser.userID,
            detailsJson: JSON.stringify(assessmentDetails),
          },
          trainingSoapEndpoint,
        )
        if (String(result.returnValue).toLowerCase() !== 'true') {
        ...
        await syncTrainingHeaderPeriodViaOData(no, assessmentDetails)
      }
  ...
  router.post(
    ...
    safe(async (req, res) => {
      ...
      const authUser = user(req)
      if (module === 'transport') {
        const header = await getPortalModuleDocument(spec, authUser, no, false)
        if (header) {
          await assertNoDuplicateTransportRequest(authUser, {
            origin: text(header, [
              'Commencement',
            ...
            tripDate: text(header, ['Date_of_Trip', 'DateOfTrip', 'DateofTrip', 'TripDate']),
            tripTime: text(header, ['Time_of_Trip', 'TimeOfTrip', 'DepartureTime']),
            currentDocNo: no,
          })
        }
      }
  ...
  router.patch(
    ...
    safe(async (req, res) => {
      const requestId = String(req.params.id)
      const { module, no, spec } = requireMutableModule(requestId)
      const authUser = user(req)
  ...
  router.post(
    ...
    safe(async (req, res) => {
      ...
      if (!authUser.canApprove) {
        throw portalError(
          'Only an authorized maintenance approver can assign a technician',
          403,
      ...
      await assertMaintenanceTechnicianNo(technicianNo)
      const detail = await requestDetail(requestId, authUser)
      ...
      const receivedDate = String(
        payload.FAReceivedDate ?? payload.FA_Received_Date ?? payload.faReceivedDate ?? '',
      ).trim()
      if (receivedDate && !receivedDate.startsWith('0001-01-01')) {
        throw portalError(
          'Technician cannot be changed after maintenance receipt has been confirmed',
          409,
          'MAINTENANCE_RECEIPT_ALREADY_CONFIRMED',
        )
      }
      const currentTechnician = String(
        payload.AssignedTechnician ??
          payload.Assigned_Technician ??
          payload.assignedTechnician ??
          '',
      ).trim()
      if (currentTechnician && currentTechnician.toLocaleLowerCase() === technicianNo.toLocaleLowerCase()) {
        res.json(detail)
        return
      }
      const result = await callSoapMethod(
  ...
  router.get(
    ...
    safe(async (req, res) => {
      ...
      const clause = approverIdFilterClause(await approverIdCandidates(authUser))
      // Newest first — sorted in memory because BC Query pages reject OData OrderBy.
      const rows = sortApprovalEntriesNewestFirst(
        ((await fetchOData('QyApprovalEntry', {
          $filter: `Status eq '${status}' and ${clause}`,
          $top: 500,
        })) as ODataRecord[] | null) ?? [],
      )
      const liveRows = await filterApprovalEntriesWithExistingSource(rows)
      res.json({
  ...
  router.get(
    ...
    safe(async (req, res) => {
      ...
      let timelineEntries = await fetchAllApprovalEntriesForDocument(no)
      if (module === 'purchaseRequisition' && source?.payload) {
        const prSpec = findFrontendModuleSpec(module)
        if (prSpec) {
          timelineEntries = filterCurrentPortalApprovalEntries(
            prSpec,
            source.payload as ODataRecord,
            timelineEntries,
          )
        }
      }
      ...
      const approvalSteps = await enrichApprovalStepsWithCommentLines(
        mapApprovalStepsWithSequence(timelineEntries.length ? timelineEntries : entryRows),
        no,
        findFrontendModuleSpec(module)?.headerTableId,
      )
      const displayStatus = approvalDisplayStatus(
        entryRows,
        source?.status || queueItem.status,
      )
      if (source) {
        res.json({
          ...source,
          ...
          submittedAt: source.submittedAt || source.createdAt || queueItem.submittedAt,
  ...
  router.post(
    ...
    safe(async (req, res) => {
      ...
      const source = await requestDetail(requestId, authUser, { forApproval: true }).catch(() => null)
      if (module === 'purchaseRequisition' && source?.payload) {
        const prSpec = findFrontendModuleSpec(module)
        if (prSpec) {
          entries = filterCurrentPortalApprovalEntries(
            prSpec,
            source.payload as ODataRecord,
            entries,
          )
  ...
  router.post(
    ...
    safe(async (req, res) => {
      ...
      try {
        ...
        const message = error instanceof Error ? error.message : String(error)
        // Manual-rate types, missing Job Group/Grade (Emp 002), or missing Job Grade
        // Advance Rates → requester enters Daily Rate on the portal; BC stamps it on save.
        if (
          /please enter daily rate/i.test(message) ||
          /no Job Group\/Grade/i.test(message) ||
          /no active Job Grade rate/i.test(message) ||
          /Job Group\/Grade for advance-rate/i.test(message)
        ) {
  ...
  router.get(
    ...
    safe(async (req, res) => {
      ...
      res.json({
        ...
        sector: text(employee, ['SectorName', 'Sector_Name', 'Sector', 'GlobalDimension1Code']),
        sectorCode: org.sectorCode,
        division: org.division,
        divisionCode: org.divisionCode,
        district: org.district,
        districtCode: org.districtCode,
        scopeCode: org.scopeCode,
  ...
  router.get(
    ...
    safe(async (req, res) => {
      ...
      const mine = authUser.employeeNo.trim().toUpperCase()
      const visible = (Array.isArray(rows) ? rows : []).filter((row) => {
        if (authUser.canApprove || authUser.HOD || authUser.CEO) return true
        const owner = text(row, ['EmployeeNo', 'Employee_No', 'RaisedBy', 'Raised_By', 'RequesterID'])
        if (!owner) return true
        return owner.trim().toUpperCase() === mine
      })
      res.json({
  ...
  router.post(
    ...
    safe(async (req, res) => {
      ...
      const ticketNo = String(req.params.ticketNo)
      const tickets = (await fetchOData('QyWorkTickets', {
        $filter: `TicketNo eq '${odataString(ticketNo)}'`,
        $top: 1,
      }).catch(() => [])) as ODataRecord[] | null
      const ticket = Array.isArray(tickets) ? tickets[0] : undefined
      const status = ticket
        ? resolveModuleRequestStatus(ticket, 'workTickets')
        : ''
      if (status && !['Approved', 'Posted', 'Released'].includes(status)) {
        throw portalError(
          'Confirm the flight booking only after the work ticket is approved.',
          422,
          'WORK_TICKET_NOT_APPROVED',
        )
      }
      const result = await callSoapMethod(
  ...
  router.get(
    ...
    safe(async (req, res) => {
      const authUser = user(req)
      if (!authUser.HOD) throw portalError('HOD access required', 403)
      const employees = await fetchEmployeesByOrgScope(
        hodOrgScopeCodes(authUser),
        authUser.employeeNo,
      )
  ...
  router.get(
    ...
    safe(async (req, res) => {
      const authUser = user(req)
      const org = await resolveProcurementPlanOrg(authUser)
      const allowed = procurementPlanAllowedOrgCodes(authUser, org)
      const budgetName = String(req.query.budgetName ?? '').trim()
      const planPeriod = String(req.query.planPeriod ?? '').trim()
      let headerRows: ODataRecord[] = []
      let lineRows: ODataRecord[] = []
      try {
        headerRows = (await fetchOData('QyProcurementPlanHeader')) as ODataRecord[]
      } catch (error) {
        throw portalError(
          procurementSoapMessage('QyProcurementPlanHeader', error),
          502,
        )
      }
      try {
        lineRows = (await fetchOData('QyProcurementPlanLines')) as ODataRecord[]
      } catch {
        lineRows = []
      }
      const matchHeader = (row: ReturnType<typeof mapProcurementPlanHeader>) => {
        if (budgetName && row.BudgetName !== budgetName) return false
        if (planPeriod && row.ProcurementPlanPeriod !== planPeriod) return false
        if (!allowed.size) return false
        return allowed.has(row.GlobalDimension2.trim().toLowerCase())
      }
      const matchLine = (row: ReturnType<typeof mapProcurementPlanLine>) => {
        if (budgetName && row.BudgetName !== budgetName) return false
        if (planPeriod && row.ProcurementPlanPeriod !== planPeriod) return false
        if (!allowed.size) return false
        return allowed.has(row.Department.trim().toLowerCase())
      }
      res.json({
        orgKind: org.orgKind,
        globalDim1: org.globalDim1,
        globalDim2: org.globalDim2,
        headers: (Array.isArray(headerRows) ? headerRows : []).map(mapProcurementPlanHeader).filter(matchHeader),
        lines: (Array.isArray(lineRows) ? lineRows : []).map(mapProcurementPlanLine).filter(matchLine),
      })
    }),
  )
  ...
  router.post(
    ...
    safe(async (req, res) => {
      const authUser = user(req)
      const org = await resolveProcurementPlanOrg(authUser)
      const globalDim1 = String(req.body?.globalDim1 ?? '').trim() || org.globalDim1
      const globalDim2 = String(req.body?.globalDim2 ?? '').trim() || org.globalDim2
      if (!globalDim2) {
        throw portalError(
          'Your Department or District is required to open a budget template. Ask HR to set it on the employee card.',
          422,
          'MISSING_ORG_UNIT',
        )
      }
      try {
        const result = await callSoapMethod(
          ...
          {
            budgetName: String(req.body?.budgetName ?? ''),
            globalDim1,
            globalDim2,
            planPeriod: String(req.body?.planPeriod ?? ''),
          },
          facilitySoapEndpoint,
        )
        if (!soapReturnedTrue(result.returnValue)) {
          throw portalError('Business Central did not save the procurement plan header', 502)
        }
      } catch (error) {
        if ((error as { status?: number }).status) throw error
        throw portalError(procurementSoapMessage('SaveProcurementPlanHeader', error), 502)
      }
      res.status(201).json({ ok: true, globalDim1, globalDim2, orgKind: org.orgKind })
    }),
  )

  router.post(
    '/procurement-plans/lines',
    safe(async (req, res) => {
      const authUser = user(req)
      const org = await resolveProcurementPlanOrg(authUser)
      const globalDim1 = String(req.body?.globalDim1 ?? '').trim() || org.globalDim1
      const department = String(req.body?.department ?? '').trim() || org.globalDim2
      const lineTypeRaw = Number(req.body?.lineType ?? 2)
      const lineType = lineTypeRaw === 1 || lineTypeRaw === 3 ? lineTypeRaw : 2
      if (!department) {
        throw portalError('Department or District is required on each budget line.', 422)
      }
      try {
        const result = await callSoapMethod(
          'SaveProcurementPlanLine',
          {
            ...
            department,
            lineType,
            typeNo: String(req.body?.typeNo ?? ''),
            globalDim1,
            planPeriod: String(req.body?.planPeriod ?? ''),
            quantity: Number(req.body?.quantity ?? 0),
            unitCost: Number(req.body?.unitCost ?? 0),
            planDate: String(req.body?.planDate ?? ''),
          },
          facilitySoapEndpoint,
        )
        if (!soapReturnedTrue(result.returnValue)) {
          throw portalError('Business Central did not save the procurement plan line', 502)
        }
      } catch (error) {
        if ((error as { status?: number }).status) throw error
        throw portalError(procurementSoapMessage('SaveProcurementPlanLine', error), 502)
      }
      res.json({ ok: true })
    }),
  )
  ...
  router.post(
    '/procurement-plans/lines/delete',
    safe(async (req, res) => {
      const authUser = user(req)
      const org = await resolveProcurementPlanOrg(authUser)
      try {
        const result = await callSoapMethod(
          ...
          {
            budgetName: String(req.body?.budgetName ?? ''),
            department: String(req.body?.department ?? '').trim() || org.globalDim2,
            lineType: Number(req.body?.lineType ?? 2),
            typeNo: String(req.body?.typeNo ?? ''),
            globalDim1: String(req.body?.globalDim1 ?? '').trim() || org.globalDim1,
            planPeriod: String(req.body?.planPeriod ?? ''),
          },
          facilitySoapEndpoint,
        )
        if (!soapReturnedTrue(result.returnValue)) {
          throw portalError('Business Central did not delete the procurement plan line', 502)
        }
      } catch (error) {
        if ((error as { status?: number }).status) throw error
        throw portalError(procurementSoapMessage('DeleteProcurementPlanLine', error), 502)
      }
      res.json({ ok: true })
    }),
  )
  ...
  router.post(
    ...
    safe(async (req, res) => {
      ...
      try {
        ...
        if (!soapReturnedTrue(result.returnValue)) {
          throw portalError('Business Central did not submit the procurement plan', 502)
        }
      } catch (error) {
        if ((error as { status?: number }).status) throw error
        throw portalError(procurementSoapMessage('SubmitProcurementPlan', error), 502)
      }
      res.json({ ok: true })
    }),
  )
