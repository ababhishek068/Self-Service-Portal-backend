...
export async function postAssetTransfer(id: string) {
  requireAuthApiUrl()
  return authPost<PortalRequest>(
    `/api/requests/${encodeURIComponent(id)}/post-asset-transfer`,
    {},
  )
}
...
export async function assignMaintenanceTechnician(id: string, technicianNo: string) {
  requireAuthApiUrl()
  return authPost<PortalRequest>(
    `/api/requests/${encodeURIComponent(id)}/assign-technician`,
    { technicianNo },
  )
}

export async function confirmMaintenanceReceipt(id: string, remarks: string) {
  requireAuthApiUrl()
  return authPost<PortalRequest>(
    `/api/requests/${encodeURIComponent(id)}/confirm-maintenance-receipt`,
    { remarks },
  )
}
