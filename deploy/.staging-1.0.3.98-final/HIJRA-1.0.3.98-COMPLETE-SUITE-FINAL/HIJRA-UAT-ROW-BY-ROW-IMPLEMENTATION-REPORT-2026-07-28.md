...
Date: 28 July 2026  
Release: `1.0.3.98` (supersedes `1.0.3.79` baseline)  
AL patch: `1.0.5.75` safe patch

## v1.0.3.98 spreadsheet audit follow-ups (28 Jul 2026)

Additional portal fixes after row-by-row audit and live UAT feedback:

- **Imprest** — list search covers destination, purpose, department, and employee account; line destination only pre-fills when it matches a BC travel-destination code (header free text no longer breaks line lookup).
- **Imprest surrender** — create preview shows Place of Duty and Employee Account; list search covers linked imprest and trip fields; cash-receipt dropdown no longer clips in the line editor.
- **Salary advance** — create form previews monthly salary base and computed advance amount; detail shows amount and salary base from ERP enrichment.
- **Purchase requisition** — specification is mandatory on every line (UAT row 4).
- **Approvals (all modules)** — approver steps resolve employee display names and job titles from BC HR, not raw user IDs.
- **Prior session fixes retained:** staff-claim ERP account mapping, finance org dimensions from employee, gate-pass transferNo + approval, work-ticket ERP create flow (no forced flight booking).
