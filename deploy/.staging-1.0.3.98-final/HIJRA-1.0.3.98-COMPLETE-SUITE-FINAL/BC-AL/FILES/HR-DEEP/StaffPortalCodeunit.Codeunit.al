codeunit 50049 "Staff Portal Codeunit"
{
    ...
    begin
        // UAT 28/07/2026: Imprest Holder must come from the *requisition* header (IR…),
        // not Imprest Header (IMP…). Using the wrong table left Holder blank so
        ...
        TbImprestRequisitionHeader.Reset();
        TbImprestRequisitionHeader.SetRange("No.", headerNo);
    ...
    begin
        ...
        if action = 'create' then begin
            ...
            // UAT 28/07/2026: portal amount/daily rate must be applied before Validate days
            // for types like PETTY CASH that require Daily Rate(Amount) to be present.
            if amount > 0 then begin
                TbImprestRequisitionLines.Amount := amount;
                if noOfDays > 0 then
                    TbImprestRequisitionLines."Daily Rate(Amount)" := Round(amount / noOfDays, 0.01);
            end else
                TbImprestRequisitionLines.Validate("No of Days", noOfDays);
    ...
    begin
        ...
        if TbGatePass.FindFirst() then begin
            ...
            if CuCustomApprovals.CheckApprovalsWorkflowEnabled(VarVariant) then begin
                ...
                Error('No approval workflow is enabled for gate pass %1 (link: %2). Ask the BC administrator to enable the workflow for table 50296.', gatePassNo, TbGatePass."Link to");
        end else begin
            Error('Requisition is no longer editable or it does not exist');
        end;
    end;
