...
export function LeaveStatement() {
  ...
  const leaveTypesQuery = useQuery({
    ...
    queryFn: async () => (await fetchLeaveTypes()).types,
  })
          ...
          {leaveType && balanceQuery.data ? (
            <div
              className={`grid gap-3 rounded-lg border border-slate-200 bg-slate-50 p-4 ${
                balanceQuery.data.isAnnual ? 'sm:grid-cols-3' : 'sm:grid-cols-2'
              }`}
            >
              <div>
              ...
              {/* Earned Leave Days is the employee's ANNUAL leave accrual — it
                  has no meaning for Sick/Special/other non-annual leave types,
                  so only show it when the selected type actually IS annual. */}
              {balanceQuery.data.isAnnual ? (
                <div>
