...
export const staffClaimLineSchema = z
  .object({
    ...
    expenditureDate: optionalText,
    expenditureEndDate: optionalText,
    expenditureDescription: z.string().min(3, 'Expenditure description is required'),
  })
  .superRefine((data, ctx) => {
    const raw = String(data.claimType ?? '').trim()
    const code = raw.split(' - ')[0]?.trim().toUpperCase() ?? ''
    const medical = code === 'MEDICAL' || code.startsWith('MED')
    const other = code === 'OTHER'
    const perDiem =
      /PER\s*&\s*ACC|PER\s*AND\s*ACC|PER\s*DIEM|PERDIEM|ACCOMMOD/.test(code) ||
      /PER\s*&\s*ACC|PER\s*AND\s*ACC|PER\s*DIEM|PERDIEM|ACCOMMOD/.test(raw.toUpperCase())

    if (perDiem) {
      const start = String(data.expenditureDate ?? '').trim()
      const end = String(data.expenditureEndDate ?? '').trim()
      if (!start) {
        ctx.addIssue({
          code: 'custom',
          path: ['expenditureDate'],
          message: 'Start date is required for per diem (e.g. 2026-08-14)',
        })
      } else if (!isValidDateString(start)) {
        ctx.addIssue({
          code: 'custom',
          path: ['expenditureDate'],
      ...
      if (!end) {
        ctx.addIssue({
          code: 'custom',
          path: ['expenditureEndDate'],
          message: 'End date is required for per diem (e.g. 2026-08-19)',
        })
      } else if (!isValidDateString(end)) {
        ctx.addIssue({
          code: 'custom',
          path: ['expenditureEndDate'],
    ...
    if (!String(data.accountNo ?? '').trim()) {
      ctx.addIssue({
        code: 'custom',
        path: ['claimType'],
        message: 'Choose a claim type that has a G/L account mapped in Business Central',
      })
    }

    if (medical) {
      if (!String(data.hospitalCategory ?? '').trim()) {
        ctx.addIssue({
          code: 'custom',
          path: ['hospitalCategory'],
          message: 'Hospital category is required for medical claims',
      ...
      if (Number(data.medicalAmount ?? 0) <= 0) {
        ctx.addIssue({
          code: 'custom',
          path: ['medicalAmount'],
          message: 'Medical amount is required',
        })
      }
      if (Number(data.amount ?? 0) <= 0) {
        ctx.addIssue({
          code: 'custom',
          path: ['amount'],
          message: 'Amount is calculated from Business Central — set hospital category and medical amount first',
        })
      }
      return
    }
    ...
    if (other) {
      const refund = Number(data.amountToRefund ?? 0)
      const amount = Number(data.amount ?? 0)
      if (refund <= 0 && amount <= 0) {
        ctx.addIssue({
          code: 'custom',
          path: ['amountToRefund'],
          message: 'Amount to refund is required for Other claims',
        })
      }
      return
    }
...
export const imprestHeaderSchema = z
  .object({
    ...
    travelDestination: z.string().min(2, 'Travel destination is required'),
    travelDate: dateField,
    returnDate: dateField,
  })
  .superRefine((data, ctx) => {
    const travel = parseISO(data.travelDate)
    const ret = parseISO(data.returnDate)
    const todayIso = today().toISOString().slice(0, 10)
    // Excel R5 / Word Aug 7: no back-dated travel start or return.
    if (data.travelDate < todayIso) {
      ctx.addIssue({
        code: 'custom',
        path: ['travelDate'],
        message: 'Travel start date cannot be earlier than today',
      })
    }
    if (data.returnDate < todayIso) {
      ctx.addIssue({
        code: 'custom',
        path: ['returnDate'],
    ...
    if (isBefore(ret, travel)) {
      ctx.addIssue({
        code: 'custom',
        path: ['returnDate'],
        message: 'Return date must be on or after the travel start date',
      })
    }
    // HB 05/08/2026: start and return must not be the same calendar day for duration display.
    if (data.travelDate === data.returnDate) {
      ctx.addIssue({
        code: 'custom',
        path: ['returnDate'],
        message: 'Return date must be after the travel start date (duration cannot be the same day)',
      })
    }
  })
...
export const imprestLineHeaderSchema = z.object({
  ...
  // Word Aug 7: destination entered on header — line uses ERP rate location optionally.
...
export const storeHeaderSchema = z.object({
  ...
  description: z
    .string()
    .min(5, 'Describe what you need in a few words (at least 5 characters)'),
  // Optional until AL 1.0.2.360 is published; the backend omits it when blank
  // so older AL builds keep working. Prefer selecting a store when the list loads.
  issuingStore: optionalText,
})

export const storeLineSchema = z
  .object({
    type: z.string().min(1, 'Choose Item or Fixed asset'),
    issuingStore: z.string().min(1, 'Select the issuing store for this line'),
    itemNo: z.string().min(1, 'Select an item or fixed asset from the list'),
  ...
  .superRefine((data, ctx) => {
    if (data.type === '1' && Number(data.quantity) <= 0) {
      ctx.addIssue({
        code: 'custom',
        message: 'Enter how many units you need',
        path: ['quantity'],
      })
    }
  })
...
export const workTicketHeaderSchema = z.object({
  ...
  type: z.string().max(10, 'Type must be 10 characters or less for Business Central').default(''),
})
...
export const transportHeaderSchema = z.object({
  ...
  commencement: z.string().min(2, 'Trip origin / starting location is required'),
  destination: z.string().min(1, 'Destination is required'),
...
export const assetTransferHeaderSchema = z
  .object({
    transferType: z.enum(['Internal', 'External']),
    typeOfTransfer: z.enum(['Permanent', 'Temporary']),
    assetType: z.enum(['Fixed Asset', 'Item']),
    assetNo: z.string().min(1, 'Asset, vehicle, or tool is required'),
    fromLocation: z.string().min(1, 'From location is required'),
    fromEmployeeNo: optionalText,
    toEmployeeNo: optionalText,
    toLocation: optionalText,
    destinationLocation: optionalText,
    partnerName: optionalText,
    reasonForTransfer: z.enum(['Lost', 'Damaged', 'Resignation', 'Other']),
    reason: z.string().min(3, 'Comments are required'),
    assetCondition: z.enum(['Good', 'Fair', 'Damaged']),
    assetConditionDescription: z
      .string()
      .trim()
      .min(1, 'Condition description is required'),
    temporaryExpiryDate: optionalText,
  })
  .superRefine((data, ctx) => {
    if (data.transferType === 'Internal' && !data.toEmployeeNo && !data.toLocation) {
      ctx.addIssue({
        code: 'custom',
        path: ['toEmployeeNo'],
        message: 'Select a receiving employee or destination location',
      })
    }
    if (data.transferType === 'External' && !data.partnerName) {
      ctx.addIssue({
        code: 'custom',
        path: ['partnerName'],
        message: 'Receiving organisation is required for an external transfer',
      })
    }
    if (data.transferType === 'External' && !data.destinationLocation) {
      ctx.addIssue({
        code: 'custom',
        path: ['destinationLocation'],
        message: 'Destination is required for an external transfer',
      })
    }
    if (data.typeOfTransfer === 'Temporary') {
      if (!data.temporaryExpiryDate) {
        ctx.addIssue({
          code: 'custom',
          path: ['temporaryExpiryDate'],
          message: 'Expiry date is required for a temporary transfer',
        })
      } else if (!isValidDateString(data.temporaryExpiryDate)) {
        ctx.addIssue({
          code: 'custom',
          path: ['temporaryExpiryDate'],
...
export const assetTransferToolLineSchema = z
  .object({
    accessoryEntryNo: optionalText,
    toolCode: optionalText,
    toolName: optionalText,
    quantity: z.coerce.number().int().positive('Quantity must be greater than zero'),
    remarks: optionalText,
  })
  .superRefine((data, ctx) => {
    const hasSelected = Boolean(String(data.accessoryEntryNo ?? '').trim())
    const hasRegistered =
      Boolean(String(data.toolCode ?? '').trim()) || Boolean(String(data.toolName ?? '').trim())
    if (!hasSelected && !hasRegistered) {
      ctx.addIssue({
        code: 'custom',
        path: ['accessoryEntryNo'],
        message: 'Select or register a vehicle tool or accessory',
      })
    }
  })
...
export const pettyCashReplenishmentSchema = z.object({
  ...
  // Paying bank account is Finance-side in BC — not required from the requester (HB 05/08/2026).
  payingAccount: optionalText,
...
export const fuelHeaderSchema = z
  .object({
    ...
    // Provided when actual km/L falls below the vehicle's rated standard.
    // Captures the operating condition justification (terrain, load, traffic, etc.).
    consumptionRemark: z.string().optional().default(''),
  })
  .superRefine((data, ctx) => {
    ...
    if (data.odometer <= 0) {
      ctx.addIssue({ code: 'custom', path: ['odometer'], message: 'Current odometer reading is required' })
    }
  })
...
export const trainingRequestSchema = z
  .object({
    ...
    otherTrainingName: optionalText,
    comments: z.string().min(3, 'Purpose / expected outcome is required'),
    trainingType: z.string().min(1, 'Training type is required'),
    periodStart: dateField,
    periodEnd: dateField,
    durationDays: z.coerce.number().positive('Duration is required'),
    targetGroup: z.string().min(2, 'Target group is required'),
    participants: z.coerce.number().int().positive('No. of participants is required'),
  ...
  .superRefine((data, ctx) => {
    if (data.trainingNeed === '__OTHER__' && !String(data.otherTrainingName ?? '').trim()) {
      ctx.addIssue({
        code: 'custom',
        path: ['otherTrainingName'],
        message: 'Enter the training name when Others is selected',
      })
    }
    if (isBefore(parseISO(data.periodEnd), parseISO(data.periodStart))) {
      ctx.addIssue({
        code: 'custom',
        path: ['periodEnd'],
        message: 'Training period end must be on or after the start date',
      })
    }
  })
...
export const trainingNeedsSchema = z.object({
  trainingNeed: z.string().min(1, 'Primary training course is required'),
  additionalTrainingNeeds: optionalText,
  purpose: z.string().min(10, 'Purpose / expected outcome is required'),
  trainingType: z.string().min(1, 'Training type is required'),
  durationDays: z.coerce.number().positive('Duration is required'),
  targetGroup: z.string().min(2, 'Target group is required'),
  participants: z.coerce.number().int().positive('No. of participants is required'),
  ...
  priority: z.string().min(1, 'Priority is required'),
  vendor: optionalText,
  estimatedBudget: z.coerce.number().min(0, 'Estimated budget cannot be negative').default(0),
  remark: optionalText,
  department: z.string().min(1, 'Department is required'),
  periodStart: dateField,
  periodEnd: dateField,
...
export const maintenanceRequestSchema = z
  .object({
    requestDate: dateField,
    requestorName: optionalText,
    requestType: z.enum(['1', '2']),
    faTagNumber: optionalText,
    vehicleNo: optionalText,
    item: z.string().min(3, 'Describe the work or spare part (at least 3 characters)'),
    quantity: z.coerce.number().int().positive('Quantity must be at least 1').default(1),
    priority: z.enum(['Low', 'Medium', 'High', 'Critical']),
    location: z.string().min(2, 'Where is the asset / vehicle located?'),
    odometer: z.coerce.number().min(0).default(0),
    lastServiceOdometer: z.coerce.number().min(0).default(0),
    issueDescription: z
      .string()
      .min(20, 'Please describe the issue in at least 20 characters so facilities can act'),
    attachments: z.array(attachmentSchema).default([]),
  })
  .superRefine((data, ctx) => {
    if (data.requestType === '1') {
      if (!data.faTagNumber) {
        ctx.addIssue({
          code: 'custom',
          path: ['faTagNumber'],
          message: 'Select the fixed asset that needs maintenance',
        })
      }
      if (data.vehicleNo) {
        ctx.addIssue({
          code: 'custom',
          path: ['vehicleNo'],
          message: 'Switch to “Vehicle service” above, or clear this vehicle selection',
        })
      }
    }
    if (data.requestType === '2') {
      if (!data.vehicleNo) {
        ctx.addIssue({
          code: 'custom',
          path: ['vehicleNo'],
          message: 'Select the vehicle registration from the list',
        })
      }
      if (data.odometer <= 0) {
        ctx.addIssue({
          code: 'custom',
          path: ['odometer'],
          message: 'Enter the current odometer reading (km)',
        })
      }
      if (data.lastServiceOdometer > 0 && data.odometer < data.lastServiceOdometer) {
        ctx.addIssue({
          code: 'custom',
          path: ['odometer'],
          message: 'Current odometer cannot be lower than the last service reading',
        })
      }
      // 5,000 km is a recommended interval — warn via message only when clearly overdue
      // by blocking nothing for early service (urgent faults are allowed).
    }
  })
