...
export type WorkTicketFlightInput = {
  ...
  departureDate: string
  returnDate?: string
  ticketClass?: 'Economy' | 'Business' | '1' | '2'
  airlinePreference?: string
  justification?: string
}
...
/** Optional flight booking details — saved separately after the work ticket header exists. */
...
export async function submitWorkTicket(ticketNo: string) {
  requireAuthApiUrl()
  return authPost<{ ok: boolean }>(`/api/work-tickets/${encodeURIComponent(ticketNo)}/submit`, {})
}

export async function cancelWorkTicketApproval(ticketNo: string) {
  requireAuthApiUrl()
  return authPost<{ ok: boolean }>(`/api/work-tickets/${encodeURIComponent(ticketNo)}/cancel`, {})
}
