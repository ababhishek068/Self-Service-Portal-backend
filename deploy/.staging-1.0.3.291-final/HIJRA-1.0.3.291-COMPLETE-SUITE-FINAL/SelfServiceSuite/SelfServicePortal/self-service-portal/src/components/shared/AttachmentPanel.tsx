...
import { AttachmentFileCard } from './attachmentUi'
...
export function AttachmentPanel({
  ...
  const filteredUploading = useMemo(
    () => uploading.filter((attachment) => matchesSearchQuery(attachment, search)),
    [search, uploading],
  )
  const filteredAttachments = useMemo(
    () => attachments.filter((attachment) => matchesSearchQuery(attachment, search)),
    [attachments, search],
  )
  const filteredCount = filteredUploading.length + filteredAttachments.length
    ...
    <section className="border-t border-slate-200 pt-4">
      ...
      {showEmpty ? (
        ...
        <div className="portal-attachment-grid grid gap-3 sm:grid-cols-2">
          {filteredUploading.map((file, index) => (
            <div
              key={file.id}
          ...
          {filteredAttachments.map((attachment, index) => (
            <div
              key={attachment.id}
              className="portal-attachment-enter"
              style={{ animationDelay: `${(filteredUploading.length + index) * 60}ms` }}
            >
              <AttachmentFileCard
        ...
        <div className="rounded-xl border border-dashed border-slate-200 bg-slate-50/80 px-4 py-8 text-center">
          <p className="text-sm font-medium text-slate-600">
            No attachments match &quot;{search.trim()}&quot;.
          </p>
        </div>
      ) : null}
    </section>
