...
function normalizeAuthGender(gender: string | undefined | null): 'Male' | 'Female' | '' {
  ...
  if (!g || g === '0' || g === 'both' || g === ' ') return ''
  // HR-Employee / Leave Application OptionMembers " ",Male,Female and Leave Types Both,Male,Female.
  if (g === '1') return 'Male'
...
function leaveTypeGenderRestriction(row: ODataRecord): 'Male' | 'Female' | '' {
  ...
  // BC "Both"/blank Gender → infer from the type's name/code.
  const name = `${row.Description ?? (row as Record<string, unknown>).description ?? ''} ${
    row.Code ?? (row as Record<string, unknown>).code ?? ''
  }`.toLowerCase()
  if (/patern|father|paternal|\bpat\b/.test(name)) return 'Male'
  if (/matern|pre-?natal|ante-?natal|mother|\bmat\b/.test(name)) return 'Female'
  return ''
}

/**
 * Females only see Maternity/Prenatal, males only see Paternity; gender-neutral
 * types show to everyone. When employee Gender is blank in BC we hide
 * gender-restricted types (instead of showing all) so users cannot pick
 * Paternity/Maternity and then hit BC's hard gender Error on submit.
 */
...
/** Always prefer the live HR Employee card over a stale session gender. */
function resolveEmployeeGender(
  employeeRow: ODataRecord | null | undefined,
  sessionGender?: string | null,
  sessionGenderAlt?: string | null,
): 'Male' | 'Female' | '' {
  const fromCard = normalizeAuthGender(
    String(
      (employeeRow as Record<string, unknown> | null | undefined)?.Gender ??
        (employeeRow as Record<string, unknown> | null | undefined)?.Sex ??
        '',
    ),
  )
  if (fromCard) return fromCard
  return normalizeAuthGender(sessionGender || sessionGenderAlt || '')
}

function leaveTypeRestrictionFromCodeOrName(leaveType: string): 'Male' | 'Female' | '' {
...
/** Same HR-Employee row Business Central uses when validating leave gender. */
async function fetchHrEmployeeRowForLeave(employeeNo: string) {
  const rows = (await fetchOData('QyHREmployee', {
    $filter: `No eq '${odataString(employeeNo)}'`,
    $top: 1,
  }).catch(() => [])) as ODataRecord[] | null
...
async function buildLeavePortalDetail(
  ...
  return {
    ...
    payload: {
      ...
      StartDate: filterBcLeaveDate(fieldText(row, ['StartDate', 'Start_Date'])),
      EndDate: filterBcLeaveDate(fieldText(row, ['EndDate', 'End_Date'])),
...
export function formatBcSoapDate(value: string) {
  ...
  if (slash) {
    ...
    if (first > 12) {
      ...
      // Ambiguous d/m/y — portal display is day-first (HB / ET).
      day = slash[1]!
      month = slash[2]!
    }
  ...
  const slashShort = /^(\d{1,2})\/(\d{1,2})\/(\d{2})$/.exec(normalized)
  if (slashShort) {
    const first = Number(slashShort[1])
    const second = Number(slashShort[2])
    const shortYear = slashShort[3]!
...
export function buildStaffRouter() {
  ...
  router.get(
    ...
    safe(async (req, res) => {
      ...
      const [rows, employeeRow] = await Promise.all([
        fetchOData('QyHRLeaveType'),
        fetchHrEmployeeRowForLeave(user.employeeNo),
      ])
      ...
      // Live HR card wins — a stale session Gender (e.g. Male) was letting women
      // pick Paternity and then fail in Business Central on submit.
      const userGender = resolveEmployeeGender(employeeRow, user.gender, user.Gender)
      if (userGender) {
        user.gender = userGender
        user.Gender = userGender
      } else if (employeeGenderRaw) {
        // Card has a non-empty value we could not normalize — treat as missing for
        // filtering so gender-restricted types stay hidden.
        user.gender = ''
        user.Gender = ''
      }
      res.json({
        rows: (Array.isArray(rows) ? rows : [])
          ...
          return {
            ...
            Annual: annual,
            GenderRestriction: leaveTypeGenderRestriction(row) || 'Both',
          }
        }),
        employeeGender: userGender || null,
        genderMissing:
          !userGender &&
          (Array.isArray(rows) ? rows : []).some((row) => Boolean(leaveTypeGenderRestriction(row))),
      })
    }),
  )
  ...
  router.get(
    ...
    safe(async (req, res) => {
      ...
      res.json({
        ...
        currentLeaveBalance: Math.max(0, balance ?? 0),
        ...
        // Floor at 0 — negative earned/available must not show or allow apply.
        earnedLeaveDays:
          isAnnual && earnedLeaveDays !== null && earnedLeaveDays !== undefined
            ? Math.max(0, Number(earnedLeaveDays) || 0)
            : null,
        isAnnual,
        unlimitedDays,
  ...
  router.post(
    ...
    safe(async (req, res) => {
      ...
      // Block gender-mismatched leave types before BC SOAP (BC error text is opaque).
      {
        const [typeRows, employeeRow] = await Promise.all([
          fetchOData('QyHRLeaveType', {
        ...
        const leaveTypeRow = Array.isArray(typeRows) && typeRows.length > 0 ? typeRows[0]! : null
        const liveGender = resolveEmployeeGender(employeeRow, user.gender, user.Gender)
        if (liveGender) {
          user.gender = liveGender
          user.Gender = liveGender
        } else {
          user.gender = ''
          user.Gender = ''
        ...
        const restriction =
          (leaveTypeRow ? leaveTypeGenderRestriction(leaveTypeRow) : '') ||
          leaveTypeRestrictionFromCodeOrName(body.leaveType)
        if (restriction && !liveGender) {
          res.status(422).json({
            ok: false,
            message:
              `Your Business Central employee profile has no Gender set. Ask HR to set Gender to ${restriction} before applying for this leave type.`,
          })
          return
        }
        if (restriction && liveGender && restriction !== liveGender) {
          res.status(422).json({
            ok: false,
            message: `This leave type (${body.leaveType}) can be applied only by ${restriction.toLowerCase()} employees. Your Business Central profile gender is ${liveGender}. Ask HR to correct Gender on your employee card if that is wrong.`,
          })
          return
        }
      }
      ...
      // Block create when available balance is 0 (or would go negative). Maternity uses delivery date.
      if (!deliveryDate && action === 'create') {
        const [typeRows, employeeRow, summarySoap] = await Promise.all([
          fetchOData('QyHRLeaveType', {
            $filter: `Code eq '${odataString(body.leaveType)}'`,
            $top: 1,
          }) as Promise<ODataRecord[] | null>,
          ...
          callSoapMethod(
            'GetLeaveBalance',
            { employeeNo: user.employeeNo, leaveType: body.leaveType },
            employeeDataSoapEndpoint,
          ).catch(() => null),
        ])
        const leaveTypeRow = Array.isArray(typeRows) && typeRows.length > 0 ? typeRows[0]! : null
        ...
        if (!unlimitedDays) {
          ...
          const available = Math.max(
            ...
            roundLeaveValue(
              resolveBcLeaveBalance({
                isAnnual,
                leaveTypeDays,
                leaveTypeUnlimitedDays: false,
                maximumApplicationDays,
                summary,
                cardAnnualBalance: employeeAnnualLeaveBalance(employeeRow),
                earnedLeaveDays: metrics.earnedLeaveDays,
                hasCurrentPeriodEntries: Boolean(summary?.hasCurrentPeriodEntries),
                currentPeriodNet: Number(summary?.currentPeriodNet ?? 0),
                hasOpenEntries: Boolean(summary?.hasOpenEntries),
                openNet: Number(summary?.openNet ?? 0),
                ledgerNetDays: 0,
              }),
            ),
          )
          if (available <= 0) {
            res.status(422).json({
              ok: false,
              code: 'LEAVE_BALANCE_ZERO',
              message:
                'You have no available leave balance for this type. Leave cannot be created when the balance is 0.',
            })
            return
          }
          if (body.appliedDays > available) {
            res.status(422).json({
              ok: false,
              code: 'LEAVE_BALANCE_INSUFFICIENT',
              message: `Insufficient leave balance. Available: ${available} day(s). Requested: ${body.appliedDays} day(s).`,
            })
            return
          }
        } else if (
          maximumApplicationDays !== null &&
          maximumApplicationDays !== undefined &&
          maximumApplicationDays > 0 &&
          body.appliedDays > maximumApplicationDays
        ) {
          res.status(422).json({
            ok: false,
            message: `The maximum number of days you can apply for is ${maximumApplicationDays}.`,
          })
          return
        }
      }
...
async function waitForLeavePendingInBc(
  ...
  router.post(
    ...
    safe(async (req, res) => {
      ...
      if (!soap.ok) {
        const bcStatus = fieldText(row, ['Status', 'ApprovalStatus', 'Approval_Status']) || currentStatus
        const bcEmployee = fieldText(row, ['EmployeeNo', 'Employee_No']) || user.employeeNo
        res.json({
          ok: false,
          message:
            soap.message ||
          ...
          diagnostic: {
            documentNo: body.no,
            bcStatus,
            employeeNo: bcEmployee,
            hint:
              'Confirm in BC that the leave is Open, Employee No matches your login, and Approvals are enabled for HR Leave Application (table 50532).',
          },
        })
        return
      }
