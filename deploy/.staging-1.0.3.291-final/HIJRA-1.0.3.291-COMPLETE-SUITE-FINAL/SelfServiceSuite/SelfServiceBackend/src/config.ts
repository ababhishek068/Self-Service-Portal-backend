...
const envSchema = z.object({
  ...
  // 2-hour hard token expiry; the portal also enforces 10-min idle auto-logout.
  JWT_TTL_SECONDS: z.coerce.number().int().positive().default(60 * 60 * 2),
  LOG_API_REQUESTS: z
    .string()
    .optional()
    .default('true')
  ...
  /**
   * Optional allow-list of HR employee Nos who can be assigned as maintenance technicians
   * even when Job Title does not match technician/mechanic/facilities patterns.
   * Example: 018,002,E0083
   */
  BC_MAINTENANCE_TECHNICIAN_EMPNOS: csvList,
