...
export interface FieldConfig {
  ...
  /** Span the full form grid width (use for textareas so they do not sit beside short selects). */
  fullWidth?: boolean
}
...
interface RequestFormPageProps {
  ...
  /** Optional panel rendered under the detail field grid (e.g. technician assign). */
  detailExtras?: (request: PortalRequest, refresh: () => void) => ReactNode
}
...
function firstPathValue(source: unknown, paths: string[], format: DetailFieldConfig['format'] = 'text') {
  for (const path of paths) {
    ...
    // BC returns 0001-01-01 for unset dates — keep searching the fallback chain.
