...
import { getLeaveBalance } from '@/api/endpoints/leave'
...
function formatLeaveType(payload: Record<string, unknown>) {
  const code = payloadValue(payload, [
    'LeaveTypeCode',
    'Leave_Type_Code',
    'LeaveType',
    'Leave_Type',
  ...
  if (descriptionText && codeText && descriptionText.toLowerCase() !== codeText.toLowerCase()) {
    return `${descriptionText} (${codeText})`
  }
  return descriptionText || codeText || '—'
}
...
export function ApprovalDetail() {
  ...
  const leaveType = formatLeaveType(payload)
  const leaveTypeCode = payloadValue(payload, [
    'LeaveType',
    'Leave_Type',
    'leaveType',
    'LeaveTypeCode',
  ...
  const leaveBalanceQuery = useQuery({
    queryKey: ['hr', 'leave-balance', leaveTypeCode],
    queryFn: () => getLeaveBalance(String(leaveTypeCode)),
    enabled: request?.requestType === 'leave' && Boolean(leaveTypeCode) && leaveTypeCode !== '—',
  })
          ...
          <Card className="min-w-0">
            ...
            <CardContent className="space-y-5">
              <div className={`grid gap-4 ${request.requestType === 'leave' ? 'sm:grid-cols-2' : 'md:grid-cols-3'}`}>
                ...
                {request.requestType === 'leave' && leaveBalanceQuery.data ? (
                  <div className="rounded-md bg-emerald-50 p-3">
                    <p className="text-xs uppercase text-emerald-700">Available balance</p>
                    <p className="font-medium text-emerald-900">
                      {leaveBalanceQuery.data.currentLeaveBalance ?? leaveBalanceQuery.data.balance ?? 0}{' '}
                      day(s)
                    </p>
                  </div>
                ) : null}
