...
import {
  ...
  fetchHrServiceLetterFormContext,
...
// Per the bank's SSP template flow: user submits -> HR approves/rejects with a reason -> the
// user gets the status update. In Progress stays presented as Submitted (an internal HR step).
function employeeVisibleStatus(status: HrServiceLetterRequest['status']) {
  return status === 'In Progress' ? 'Submitted' : status
}
...
function formatSalaryAmount(value: number | null | undefined) {
...
function RequestLettersPage({
  ...
  const formContextQuery = useQuery({
    queryKey: ['hr', 'service-letter-form-context'],
    queryFn: () => fetchHrServiceLetterFormContext(),
    enabled:
      mode === 'create' &&
      (letterType === 'emergency-staff-loan' || letterType === 'experience'),
    staleTime: 60_000,
  })
  ...
  const submitMutation = useMutation({
    ...
    onSuccess: (request) => {
      ...
      toast.success(
        ...
        `${request.requestNo} was submitted. HR will approve or reject it with a reason, and you will see the status here.`,
      )
  ...
  const emergencyLoanContext = formContextQuery.data
  const monthlySalaryBase =
    emergencyLoanContext?.monthlySalaryBase ??
    employee?.monthlySalaryBase ??
    null
  const monthlySalaryLabel = formatSalaryAmount(monthlySalaryBase)
  const employeeId = emergencyLoanContext?.employeeId ?? employee?.employeeNo ?? ''
  const employeeName = emergencyLoanContext?.employeeName ?? employee?.displayName ?? ''
  const employeeDepartment =
    emergencyLoanContext?.employeeDepartment ??
                ...
                <div className="flex items-start gap-2">
                  ...
                  <p>
                    {letterType === 'emergency-staff-loan'
                      ? 'Submit the Emergency Staff Loan request directly to HR. HR will approve or reject it with a mandatory reason, and the latest status will appear automatically in My HR Service Requests.'
                      : 'Select the required letter and submit it to HR. HR will approve or reject it with a reason, and the latest status will appear automatically in My HR Service Requests.'}
                  </p>
                </div>
              </div>
              ...
              {letterType ? (
                <div className="grid gap-4 border-t border-slate-100 pt-4 sm:grid-cols-2">
                  {letterType === 'emergency-staff-loan' || letterType === 'experience' ? (
                    <>
                      {[
                        { name: 'employeeId', label: 'Employee ID', value: employeeId },
                        { name: 'employeeName', label: 'Employee Name', value: employeeName },
                        {
                          ...
                          ? [
                              {
                                ...
                                value: monthlySalaryLabel,
                                loading: formContextQuery.isLoading,
                              },
                            ]
                          : [
                              {
                                name: 'jobTitle',
                                label: 'Job Title',
                                value: formContextQuery.data?.jobTitle ?? employee?.jobTitle ?? '',
                                loading: formContextQuery.isLoading,
                              },
                              {
                                name: 'dateOfJoin',
                                label: 'Date of Join',
                                value: formContextQuery.data?.dateOfJoin ?? '',
                                loading: formContextQuery.isLoading,
                              },
                              {
                                name: 'yearsOfService',
                                label: 'Years of Service',
                                value: formContextQuery.data?.yearsOfService ?? '',
                                loading: formContextQuery.isLoading,
                              },
                            ]),
                      ].map((field) => (
                            ...
                            <Input
                              ...
                              placeholder={
                                field.name === 'monthlyBasicSalary'
                                  ? 'Salary not available in Business Central'
                                  : ''
                              }
                      ...
                      {!formContextQuery.isLoading &&
                      letterType === 'emergency-staff-loan' &&
                      !monthlySalaryLabel ? (
                        <div className="sm:col-span-2 flex items-start gap-2 rounded border-l-4 border-amber-500 bg-amber-50 px-3 py-2 text-sm text-amber-800">
                          <Info className="mt-0.5 h-4 w-4 shrink-0" />
                          <span>
                            Monthly basic salary could not be loaded from Business Central. You can
                            still submit the request — HR will verify your salary during processing.
                          </span>
                        </div>
                      ) : null}
                    </>
