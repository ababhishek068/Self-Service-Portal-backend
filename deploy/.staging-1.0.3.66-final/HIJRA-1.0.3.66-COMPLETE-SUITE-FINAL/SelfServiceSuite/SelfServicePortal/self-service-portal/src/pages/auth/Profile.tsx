...
export function Profile() {
  ...
  const trainingColumns: DataTableColumn<EmployeeTraining>[] = [
    ...
    { id: 'from', header: 'From', cell: (row) => (row.fromDate ? formatDate(row.fromDate) : '—') },
    { id: 'to', header: 'To', cell: (row) => (row.toDate ? formatDate(row.toDate) : '—') },
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
          ...
          {activeTab === 'bio' ? (
            <section>
              ...
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                ...
                <InfoField label="Department" value={profile.departmentName ?? employee?.departmentName ?? employee?.departmentCode} icon={Building2} />
          ...
          {activeTab === 'contract' ? (
            <section>
              ...
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                ...
                <InfoField label="Years of Service" value={profile.yearsOfService || '—'} icon={CalendarDays} />
          ...
          {activeTab === 'kin' ? (
            <section>
              ...
              <DataTable
                rows={profile.nextOfKin}
                columns={kinColumns}
                getRowId={(row) => row.name || `${row.relationship}-${row.phone}` || 'kin-row'}
                compact
              />
            </section>
          ) : null}
