...
function ImprestSurrenderPreview({
  ...
  return (
    <div className="grid gap-x-8 gap-y-5 border-y border-slate-200 py-4 sm:grid-cols-2 lg:grid-cols-3">
      ...
      />
      <Detail label="Duration Date" value={durationDate ? String(durationDate) : formatDuration(travelStart, travelEnd)} />
      ...
      <Detail
        label="Division"
        value={String(firstFromSources(sources, ['Division', 'DivisionName', 'Division_Name']) ?? '')}
      />
      <Detail
        label="Department"
        value={String(
      ...
      <Detail
        label="District"
        value={String(firstFromSources(sources, ['DistrictName', 'District_Name', 'District']) ?? '')}
      />
      <Detail
        label="Branch"
        value={String(
          firstFromSources(sources, ['BranchName', 'Branch_Name', 'BranchCode', 'Branch_Code'])
            ?? (fallback as { branchName?: string; branchCode?: string } | undefined)?.branchName
            ?? (fallback as { branchName?: string; branchCode?: string } | undefined)?.branchCode
            ?? '',
        )}
      />
      <Detail
