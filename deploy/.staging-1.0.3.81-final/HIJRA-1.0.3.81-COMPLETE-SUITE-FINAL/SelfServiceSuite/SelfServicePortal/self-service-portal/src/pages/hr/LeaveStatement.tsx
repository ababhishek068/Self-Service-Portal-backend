...
export function LeaveStatement() {
  ...
  const columns: DataTableColumn<StatementRow>[] = [
    ...
    { id: 'balance', header: 'Current Balance', cell: (row) => row.balance },
    { id: 'status', header: 'Status', cell: (row) => <StatusBadge status={row.status} /> },
  ]

  const selectedBalance =
    balanceQuery.data?.currentLeaveBalance ??
        ...
        <div className="space-y-4">
          ...
          {leaveType ? (
            <p className="text-sm text-slate-600">
              Current balance for selected type:{' '}
              <span className="font-semibold text-slate-900">{selectedBalance}</span>
            </p>
          ) : null}
