...
export const PORTAL_API_BUILD =
  'v1.0.3.44 — 2026-07-25 (UAT: finance+HR profile, leave, training, claims, exit)'
...
const APPROVAL_MODULE_TITLES: Partial<Record<SupportedFrontendModule, string>> = {
...
function approvalQueueItem(row: ODataRecord) {
  ...
  const documentType = text(row, ['DocumentType', 'Document_Type', 'Description'])
  const moduleTitle = APPROVAL_MODULE_TITLES[module]
  let title = documentType
  if (
    !title ||
    title === 'Order' ||
    title.toLowerCase() === module.toLowerCase() ||
    (moduleTitle && title.toLowerCase() === moduleTitle.toLowerCase())
  ) {
    title = moduleTitle ?? `${module} approval`
  }
  if (documentNo && !title.includes(documentNo)) {
    title = `${title} — ${documentNo}`
  }
  return {
  ...
  if (module === 'training') {
    return Promise.all(
      rows.map(async (row) => {
        ...
        let enriched: ODataRecord = { ...row }
        if (applicationNo) {
          try {
            const assessment = await fetchPortalTrainingAssessment(applicationNo)
            if (assessment) {
              enriched = {
                ...enriched,
                department: assessment.department || enriched.department,
            ...
            // Assessment is additive — list still works from the ERP header alone.
          }
        }
        const mapped = mapRequest(enriched, 'training')
        return {
          ...mapped,
          status: resolveModuleRequestStatus(row, 'training'),
          payload: enriched,
        }
      }),
    )
...
async function resolveRequestApprovalSteps(
  ...
  return enrichApprovalStepsWithCommentLines(resolved, docNo, tableId > 0 ? tableId : undefined)
}
...
export function buildPortalApiRouter() {
  ...
  router.get(
    ...
    safe(async (req, res) => {
      const authUser = user(req)
      const merged =
        (await fetchMergedEmployeeRecord(authUser.employeeNo)) ??
        ((await fetchOData('QyHREmployee', {
          $filter: `No eq '${odataString(authUser.employeeNo)}'`,
          $top: 1,
        }).catch(() => [])) as ODataRecord[] | null)?.[0] ??
        {}
      const employee = merged as ODataRecord
      const [kin, history, qualifications, assets] = await Promise.all([
    ...
    safe(async (req, res) => {
      ...
      const enriched = await Promise.all(
        (Array.isArray(rows) ? rows : []).map(async (row) => {
          ...
          let assessment: Awaited<ReturnType<typeof fetchPortalTrainingAssessment>> | null = null
          if (applicationNo) {
            try {
              assessment = await fetchPortalTrainingAssessment(applicationNo)
            } catch {
              assessment = null
            }
          }
          const periodStart = assessment?.periodStart || ''
          const periodEnd = assessment?.periodEnd || ''
          return {
            applicationNo,
            course: assessment?.otherTrainingName?.trim() || assessment?.trainingNeed || courseTitle,
            fromDate: periodStart,
            toDate: periodEnd,
            trainer: assessment?.vendor || '',
            location: assessment?.department || authUser.departmentName || '',
            status: text(row, ['Status', 'ApprovalStatus']),
            result: assessment?.remark || '',
            purpose: assessment?.purpose || text(row, ['PurposeofTraining', 'Purpose_of_Training']),
            year: periodStart ? periodStart.slice(0, 4) : text(row, ['ApplicationDate', 'Application_Date']).slice(0, 4),
          }
        }),
      )
