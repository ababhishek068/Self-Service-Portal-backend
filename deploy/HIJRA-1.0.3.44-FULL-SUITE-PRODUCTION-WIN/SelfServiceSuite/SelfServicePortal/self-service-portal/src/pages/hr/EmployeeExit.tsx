...
function WorkflowProgress({ request }: { request: EmployeeExitRequest }) {
  ...
  // Per the bank's SSP template the Employee Exit form goes directly to HR for information only.
...
function EmployeeExitPage({ initialRequestType }: { initialRequestType: EmployeeExitRequestType | '' }) {
  ...
  const renderField = (field: EmployeeExitFieldConfig) => {
    ...
    const selectOptions =
      field.optionsSource === 'employees' ? employeesQuery.data ?? [] : field.options ?? []
    return (
            ...
            <div className="space-y-5">
              ...
              {requestType === 'exit-interview' ? (
                <p className="rounded-lg border border-blue-200 bg-blue-50 px-3 py-2 text-sm text-blue-800">
                  This form follows Hijra Bank’s Employee Exit SSP template and is sent directly to HR in Business Central for information purposes only.
                </p>
              ) : null}
