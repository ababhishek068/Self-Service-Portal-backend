...
codeunit 52121 "Portal Training Mgt."
{
    ...
    begin
        ...
        // Mirror portal training period onto the BC training header so approvers
        // see From/To Date on HR Training Applications (not just Application Date).
        SyncTrainingHeaderFromAssessment(TrainingHeader, Assessment);
    ...
    local procedure SyncTrainingHeaderFromAssessment(var TrainingHeader: Record "HR Training Applications"; Assessment: Record "Portal Training Assessment")
    begin
        // Sync the portal-selected period onto the BC training header so From/To Date
        // show the actual training dates (not Application Date = Today).
        if Assessment."Training Start Date" <> 0D then
            TrainingHeader."From Date" := Assessment."Training Start Date";
        if Assessment."Training End Date" <> 0D then
            TrainingHeader."To Date" := Assessment."Training End Date";
        if Assessment."Duration Days" > 0 then
            TrainingHeader.Duration := Assessment."Duration Days";
        if Assessment."No. of Participants" > 0 then
            TrainingHeader."No of Participants" := Assessment."No. of Participants";
        // Quarter Offered is an Option field — cannot assign Code[10] directly.
        // Quarter is retained on the portal assessment; skip it here.
        if Assessment."Recommended Vendor" <> '' then
            TrainingHeader.Trainer := CopyStr(Assessment."Recommended Vendor", 1, MaxStrLen(TrainingHeader.Trainer));
        TrainingHeader.Modify(true);
    end;
    ...
    begin
        ...
        if Value = '' then
            exit(0D);
        // Portal sends ISO yyyy-mm-dd (format 9). Also accept local dd/mm/yyyy.
        if Evaluate(Parsed, Value, 9) then
            exit(Parsed);
        if Evaluate(Parsed, Value) then
            exit(Parsed);
        exit(0D);
    end;
