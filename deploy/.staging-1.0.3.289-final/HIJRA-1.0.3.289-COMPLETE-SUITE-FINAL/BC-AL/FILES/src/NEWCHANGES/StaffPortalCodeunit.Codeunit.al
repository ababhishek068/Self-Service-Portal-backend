codeunit 50049 "Staff Portal Codeunit"
{
    ...
    procedure LeaveApplication(leaveNo: Code[100]; employeeNo: Code[100]; leaveType: Code[30]; reason: Text[250]; daysApplied: Decimal; startDate: DateTime; reliever: Code[30]; isRequestLeaveAllowance: Boolean; "action": Text; myUserID: Code[30]; endDate: DateTime; whetherIsHalfDay: Integer; familyMember: Text[30]; deliveryDate: DateTime) return_value: Code[30]
    var
        LeaveTypesForDays: Record "Leave Types";
    begin
        return_value := '';
        ...
        if action = 'create' then begin
            ...
            if Dt2Date(deliveryDate) <> 0D then begin
                // Maternity / postnatal: Delivery Date OnValidate sets Start/End/Return from
                // leave-type Days + Days before Delivery. Also stamp Days Applied from the
                // leave type — portal sends 1 as a placeholder and must not overwrite dates.
                TbHRLeaveRequisition."Delivery Date" := Dt2Date(deliveryDate);
                TbHRLeaveRequisition.Validate("Delivery Date");
                TbHRLeaveRequisition."Select Whether Half Day" :=
                    TbHRLeaveRequisition."Select Whether Half Day"::Normal;
                LeaveTypesForDays.Reset();
                LeaveTypesForDays.SetRange(Code, leaveType);
                if LeaveTypesForDays.FindFirst() then
                    if LeaveTypesForDays.Days > 0 then
                        TbHRLeaveRequisition."Days Applied" := LeaveTypesForDays.Days;
                // If leave type is not flagged Maternity?, Delivery Date OnValidate is a no-op —
                // still stamp Start Date from delivery so portal/BC are not left blank.
                if TbHRLeaveRequisition."Start Date" = 0D then
                    TbHRLeaveRequisition."Start Date" := Dt2Date(deliveryDate);
            end else begin
                TbHRLeaveRequisition."Start Date" := Dt2Date(startDate);
                TbHRLeaveRequisition."End Date" := Dt2Date(endDate);

                // Excel HR HB: return date must follow half-day type (0=Normal, 1=Morning, 2=Evening).
                case whetherIsHalfDay of
                    ...
                    2:
                        begin
                            TbHRLeaveRequisition."Select Whether Half Day" :=
                                TbHRLeaveRequisition."Select Whether Half Day"::"Half Day Evening";
                            TbHRLeaveRequisition."Days Applied" := 0.5;
                            TbHRLeaveRequisition."End Date" := Dt2Date(startDate);
                            TbHRLeaveRequisition."Return Date" := CalcDate('<+1D>', Dt2Date(startDate));
                            if Date2DWY(TbHRLeaveRequisition."Return Date", 1) = 6 then
                                TbHRLeaveRequisition."Return Date" := CalcDate('<+2D>', Dt2Date(startDate));
                            if Date2DWY(TbHRLeaveRequisition."Return Date", 1) = 7 then
                                TbHRLeaveRequisition."Return Date" := CalcDate('<+1D>', TbHRLeaveRequisition."Return Date");
                        end;
                    else begin
                        TbHRLeaveRequisition."Select Whether Half Day" :=
                            TbHRLeaveRequisition."Select Whether Half Day"::Normal;
            ...
            if TbHRLeaveRequisition.Insert(true) then begin
                // Maternity: Delivery Date already calculated Start/End/Return + Days Applied.
                // Do NOT re-Validate with portal placeholder daysApplied=1.
                // If End/Return still blank (leave type missing Maternity?), compute from Start + Days.
                if Dt2Date(deliveryDate) <> 0D then begin
                    if (TbHRLeaveRequisition."Start Date" <> 0D) and
                       (TbHRLeaveRequisition."Days Applied" > 0) and
                       ((TbHRLeaveRequisition."End Date" = 0D) or (TbHRLeaveRequisition."Return Date" = 0D))
                    then begin
                        TbHRLeaveRequisition.Validate("Start Date");
                        TbHRLeaveRequisition.Modify(true);
                    end;
                end else if whetherIsHalfDay = 0 then begin
            ...
            // Portal "Discard Application" for Open drafts (not Pending Approval).
            ...
            if TbHRLeaveRequisition.FindFirst() then begin
                ...
                return_value := leaveNo;
            end else begin
                Error('Leave draft cannot be discarded or was not found');
            end;
        end else begin
            TbHRLeaveRequisition.SetRange(TbHRLeaveRequisition."Application Code", leaveNo);
            TbHRLeaveRequisition.SetRange("Employee No.", employeeNo);
    ...
    begin
        ...
        if TbHRLeaveRequisition.FindFirst() then begin
            ...
            // Also allow discarding Open drafts from the same SOAP entry point.
            TbHRLeaveRequisition.Reset;
            TbHRLeaveRequisition.SetRange(TbHRLeaveRequisition."Application Code", requisitionNo);
            TbHRLeaveRequisition.SetRange("Employee No.", employeeNo);
            TbHRLeaveRequisition.SetRange(Status, TbHRLeaveRequisition.Status::Open);
    ...
    begin
        ...
        if TbHRLeaveRequisition.FindFirst() then begin
            ...
            if not CuCustomApprovals.CheckApprovalsWorkflowEnabled(VarVariant) then
                Error(
                    'Leave approval workflow is not enabled for %1. Ask the BC administrator to configure Approvals for HR Leave Application.',
                    requisitionNo);
    ...
    begin
        ...
        if action = 'create' then begin
            ...
            // The table OnInsert uses the SOAP service UserId and can therefore stamp the
            // service account instead of the portal employee's imprest/customer account.
            // Restore the account belonging to EmployeeNo so ERP daily-rate calculation
            // resolves the correct employee and job group.
    ...
    begin
        // Prefer the explicit portal employee. Existing drafts may contain the SOAP
        // service account because Imprest Header.OnInsert derives it from UserId.
        if EmployeeNo <> '' then
            if Employee.Get(EmployeeNo) then
                if Employee."Customer No" <> '' then
                    exit(Employee."Customer No");
    ...
    begin
        ...
        if ImprestHolder = '' then
            Error(
                'No imprest/customer account is configured for employee %1. Update the employee card before creating an imprest.',
                EmployeeNo);
    ...
    // UAT HB 06/08/2026: Actual Return Date is required so Actual Travel Days
    // are calculated before Actual Spent (per-diem / taxable travel).
    local procedure EnsureImprestSurrenderTravelStartDate(var SurrenderHeader: Record "Imprest Surrender Header"; FallbackDate: Date)
    begin
        // PrepareSurrender / source imprest sometimes leave Travel Start Date blank.
        // Actual Return Date validate and Actual Spent both require it.
        if SurrenderHeader."Travel Start Date" <> 0D then
            exit;
        if SurrenderHeader."Imprest Issue Doc. No" <> '' then begin
            TbImprestRequisitionHeader.Reset;
            TbImprestRequisitionHeader.SetRange("No.", SurrenderHeader."Imprest Issue Doc. No");
            if TbImprestRequisitionHeader.FindFirst() then begin
                if TbImprestRequisitionHeader."Travel Start Date" <> 0D then
                    SurrenderHeader."Travel Start Date" := TbImprestRequisitionHeader."Travel Start Date"
                else if TbImprestRequisitionHeader."Date Required" <> 0D then
                    SurrenderHeader."Travel Start Date" := TbImprestRequisitionHeader."Date Required"
                else if TbImprestRequisitionHeader.Date <> 0D then
                    SurrenderHeader."Travel Start Date" := TbImprestRequisitionHeader.Date;
            end;
        end;
        // UAT 10/08/2026: portal drafts must not hard-block when the source imprest
        // never stored Travel Start Date — use Actual Return Date so Validate succeeds.
        if (SurrenderHeader."Travel Start Date" = 0D) and (FallbackDate <> 0D) then
            SurrenderHeader."Travel Start Date" := FallbackDate;
    end;

    // UAT 10/08/2026: Validate("Imprest Issue Doc. No") and PrepareSurrender both insert
    // surrender detail lines. Calling both (or PrepareSurrender again on update) duplicates
    // accounts in the portal (e.g. staff debetor x3). Prepare once when empty only.
    local procedure EnsureImprestSurrenderLinesPrepared(var SurrenderHeader: Record "Imprest Surrender Header")
    begin
        TbImprestSurrenderLines.Reset();
        TbImprestSurrenderLines.SetRange("Surrender Doc No.", SurrenderHeader.No);
        if TbImprestSurrenderLines.FindFirst() then
            exit;
        SurrenderHeader.PrepareSurrender();
    end;

    // UAT 10/08/2026: remove untouched duplicate account rows left by earlier Validate+Prepare.
    // Keeps the lowest Entry No per Account No:; deletes later peers with same Amount and no spend.
    local procedure DeduplicateImprestSurrenderLines(docNo: Code[50])
    var
        KeepLine: Record "Imprest Surrender Details";
        PeerLine: Record "Imprest Surrender Details";
    begin
        KeepLine.Reset();
        KeepLine.SetRange("Surrender Doc No.", docNo);
        if not KeepLine.FindSet() then
            exit;
        repeat
            if (KeepLine.Amount > 0) and (KeepLine."Account No:" <> '') then begin
                PeerLine.Reset();
                PeerLine.SetRange("Surrender Doc No.", docNo);
                PeerLine.SetRange("Account No:", KeepLine."Account No:");
                PeerLine.SetFilter("Entry No", '>%1', KeepLine."Entry No");
                while PeerLine.FindFirst() do begin
                    if (PeerLine.Amount = KeepLine.Amount) and
                       (PeerLine."Actual Spent" = 0) and
                       (PeerLine."Cash Receipt Amount" = 0)
                    then
                        PeerLine.Delete(true)
                    else
                        PeerLine.SetFilter("Entry No", '>%1', PeerLine."Entry No");
                end;
            end;
        until KeepLine.Next() = 0;
    end;

    procedure ImprestSurrenderHeader(myUserID: Code[30]; employeeNo: Code[30]; imprestNo: Code[30]; myAction: Text[100]; docNo: Code[30]; imprestIssueDocNo: Code[30]; receivedFrom: Code[50]; PVNo: Code[30]; actualReturnDate: Date) return_value: Code[50]
    begin
        return_value := '';
        if myAction = 'create' then begin
            if actualReturnDate = 0D then
                Error('Actual Return Date is required to calculate Actual Travel Days before entering expenditure.');
            if imprestIssueDocNo = '' then
                Error('Imprest Issue Doc. No is required to create an imprest surrender.');
            TbCashOfficeSetup.Get;
            ...
            // Assign only — do NOT Validate Imprest Issue Doc. No (duplicates lines with PrepareSurrender).
            ...
            if TbImprestSurrenderHeader.Insert(true) then begin
                ...
                if TbImprestSurrenderHeader.FindFirst then begin
                    EnsureImprestSurrenderLinesPrepared(TbImprestSurrenderHeader);
                    DeduplicateImprestSurrenderLines(NextNo);
                    TbImprestSurrenderHeader.Validate("PV No");
                    EnsureImprestSurrenderTravelStartDate(TbImprestSurrenderHeader, actualReturnDate);
                    // Stamp return date AFTER lines exist so CalculateSurrender sets Actual Travel Days.
                    TbImprestSurrenderHeader.Validate("Actual Return Date", actualReturnDate);
                    TbImprestSurrenderHeader.Modify(true);
                    RecalculateImprestSurrenderDocument(NextNo);
                end;
            end;
            return_value := NextNo;
        end else begin
            ...
            if TbImprestSurrenderHeader.FindFirst then begin
                // Update: stamp return date / travel start only. Never re-Validate Imprest Issue
                // Doc or PrepareSurrender — that appends duplicate accounts on every Save.
                if (imprestNo <> '') and (TbImprestSurrenderHeader."Account No." = '') then
                    TbImprestSurrenderHeader."Account No." := imprestNo;
                if (imprestIssueDocNo <> '') and (TbImprestSurrenderHeader."Imprest Issue Doc. No" = '') then
                    TbImprestSurrenderHeader."Imprest Issue Doc. No" := imprestIssueDocNo;
                EnsureImprestSurrenderLinesPrepared(TbImprestSurrenderHeader);
                DeduplicateImprestSurrenderLines(docNo);
                EnsureImprestSurrenderTravelStartDate(TbImprestSurrenderHeader, actualReturnDate);
                TbImprestSurrenderHeader.Validate("Actual Return Date", actualReturnDate);
                TbImprestSurrenderHeader.Modify(true);
                RecalculateImprestSurrenderDocument(docNo);
                return_value := docNo;
            end;
        end;
    end;
    ...
    // UAT HB 06/08/2026: ensure Actual Travel Days are calculated from Actual Return Date
    // before Validate("Actual Spent") — otherwise BC rejects with the travel-days error.
    // UAT HB 07/08/2026: portal SOAP saves Actual Spent but BC left Refund/Outstanding at full advance.
    local procedure ApplyImprestSurrenderLineSettlement(var SurrenderLine: Record "Imprest Surrender Details")
    var
        advanceAmount: Decimal;
        remaining: Decimal;
    begin
        advanceAmount := SurrenderLine.Amount;
        remaining := advanceAmount - SurrenderLine."Actual Spent" - SurrenderLine."Cash Receipt Amount";
        if SurrenderLine."Actual Spent" > advanceAmount then begin
            SurrenderLine."Additional Payment" := SurrenderLine."Actual Spent" - advanceAmount;
            SurrenderLine."Refund Amount" := 0;
            SurrenderLine."Outstanding Amount" := 0;
        end else begin
            SurrenderLine."Additional Payment" := 0;
            if remaining < 0 then
                remaining := 0;
            SurrenderLine."Outstanding Amount" := remaining;
            SurrenderLine."Refund Amount" := remaining;
        end;
    end;

    local procedure RecalculateImprestSurrenderDocument(docNo: Code[50])
    begin
        TbImprestSurrenderLines.Reset();
        TbImprestSurrenderLines.SetRange("Surrender Doc No.", docNo);
        if TbImprestSurrenderLines.FindSet() then
            repeat
                if TbImprestSurrenderLines.Amount > 0 then
                    TbImprestSurrenderLines.CalculateSurrender();
            until TbImprestSurrenderLines.Next() = 0;
    end;

    procedure RecalculateImprestSurrenderSettlement(docNo: Code[50]) return_value: Boolean
    begin
        return_value := false;
        TbImprestSurrenderHeader.Reset();
        TbImprestSurrenderHeader.SetRange(No, docNo);
    ...
    local procedure FindImprestSurrenderLine(docNo: Code[50]; lineNo: Integer; accountNo: Code[30]; var SurrenderLine: Record "Imprest Surrender Details"): Boolean
    begin
        // Prefer Entry No so duplicate Account No: rows (from earlier AL bug) update the right line.
        SurrenderLine.Reset();
        SurrenderLine.SetRange("Surrender Doc No.", docNo);
        if lineNo <> 0 then begin
            SurrenderLine.SetRange("Entry No", lineNo);
            if SurrenderLine.FindFirst() then
                exit(true);
            SurrenderLine.SetRange("Entry No");
        end;
        if accountNo <> '' then begin
            SurrenderLine.SetRange("Account No:", accountNo);
            if SurrenderLine.FindFirst() then
                exit(true);
        end;
        exit(false);
    ...
    begin
        ...
        if not TbImprestSurrenderHeader.FindFirst() then
            Error('Imprest surrender %1 was not found.', docNo);
        if TbImprestSurrenderHeader."Actual Return Date" = 0D then
            Error(
                'Actual Return Date is required on surrender %1 before entering actual expenditure. Set Actual Return Date when creating the surrender.',
                docNo);
        DeduplicateImprestSurrenderLines(docNo);
        travelStartBefore := TbImprestSurrenderHeader."Travel Start Date";
        EnsureImprestSurrenderTravelStartDate(
            TbImprestSurrenderHeader,
            TbImprestSurrenderHeader."Actual Return Date");
        if TbImprestSurrenderHeader."Travel Start Date" = 0D then
            Error(
                'Travel Start Date is missing on surrender %1. Re-create the surrender from the posted imprest.',
                docNo);
        if TbImprestSurrenderHeader."Travel Start Date" <> travelStartBefore then
            TbImprestSurrenderHeader.Modify(true);

        if not FindImprestSurrenderLine(docNo, lineNo, accountNo, TbImprestSurrenderLines) then
            Error('Imprest surrender line is no longer editable or it does not exist.');
        // Recalculate travel days only when BC has not computed them yet.
        if TbImprestSurrenderHeader."Actual Travel Days" = 0 then begin
            TbImprestSurrenderHeader.Validate("Actual Return Date", TbImprestSurrenderHeader."Actual Return Date");
            TbImprestSurrenderHeader.Modify(true);
            if not FindImprestSurrenderLine(docNo, lineNo, accountNo, TbImprestSurrenderLines) then
                Error('Imprest surrender line is no longer editable or it does not exist.');
        end;
        ...
        TbImprestSurrenderLines.Modify(true);
        RecalculateImprestSurrenderDocument(docNo);
        return_value := true;
    end;
        ...
        case myAction of
            'create':
                begin
                    ...
                    // Match Petty Cash Payment Card OnInsertRecord — Post TestField("Pay Mode").
                    PettyCashHeader."Pay Mode" := PettyCashHeader."Pay Mode"::Cash;
                    // Posting uses Payment Release Date as Gen. Journal Posting Date.
                    if requiredDate <> 0D then
                        PettyCashHeader."Payment Release Date" := requiredDate
                    else
                        PettyCashHeader."Payment Release Date" := Today;
                    PettyCashHeader."Employee No" := staffNo;
                    PettyCashHeader.VALIDATE("Employee No");
                    // UAT 10/08/2026: do NOT Validate dimensions before Insert.
                    // DimensionManagement.Get on Payments Header fails with
                    // "The Payments Header does not exist. No.='xxxx'" and burns
                    // the Petty Cash Payments No. series (2070, 2073, ...).
                    // Employee OnValidate already stamps dimension codes; validate
                    // names only after the header row exists.
                    TbEmployee.Reset();
                    ...
                    if PettyCashHeader.Insert(true) then begin
                        if PettyCashHeader."Global Dimension 1 Code" <> '' then
                            PettyCashHeader.VALIDATE("Global Dimension 1 Code");
                        if PettyCashHeader."Shortcut Dimension 2 Code" <> '' then
                            PettyCashHeader.VALIDATE("Shortcut Dimension 2 Code");
                        if PettyCashHeader."Shortcut Dimension 3 Code" <> '' then
                            PettyCashHeader.VALIDATE("Shortcut Dimension 3 Code");
            ...
            'edit':
                begin
                    ...
                    if PettyCashHeader.FindFirst() then begin
                        PettyCashHeader."Payment Type" := PettyCashHeader."Payment Type"::"Petty Cash";
                        if PettyCashHeader."Pay Mode" = PettyCashHeader."Pay Mode"::" " then
                            PettyCashHeader."Pay Mode" := PettyCashHeader."Pay Mode"::Cash;
                        PettyCashHeader.Date := requiredDate;
                        if PettyCashHeader."Payment Release Date" = 0D then begin
                            if requiredDate <> 0D then
                                PettyCashHeader."Payment Release Date" := requiredDate
                            else
                                PettyCashHeader."Payment Release Date" := Today;
        ...
        case myAction of
            'create':
                begin
                    ...
                    TbRec.Type := type;
                    TbRec.VALIDATE(Type);
                    if TbRec."Account No." = '' then
                        Error('Petty cash type %1 has no G/L account. Choose a Direct Posting payment type.', type);
    ...
    begin
        ...
        if PettyCashHeaderTbl.FindFirst() then begin
            if not (PettyCashHeaderTbl.Status in
                [PettyCashHeaderTbl.Status::Pending, PettyCashHeaderTbl.Status::"Pending Approval"])
            then
                Error('Only a pending petty cash request can be cancelled.');

            if PettyCashHeaderTbl.Status = PettyCashHeaderTbl.Status::"Pending Approval" then begin
                VarVariant := PettyCashHeaderTbl;
                CuCustomApprovals.OnCancelDocApprovalRequest(VarVariant);
                PettyCashHeaderTbl.Get(requisitionNo);
            end;

            PettyCashHeaderTbl.Validate(Status, PettyCashHeaderTbl.Status::Cancelled);
            PettyCashHeaderTbl.Modify(true);
            return_value := true;
        end else begin
    ...
    begin
        ...
        if action = 'create' then begin
            ...
            // The portal dropdown is sourced from the custom Departments table.
            // Do not validate its code (e.g. FACILTY) as a Dimension Value: those
            // are separate BC tables and valid departments need not exist in both.
            if requestingDepartment <> '' then
                TbPurchaseHeader.Validate("Requesting Department", requestingDepartment);
            ...
            if TbPurchaseHeader.FindFirst() then begin
                ...
                // Requesting Department relates to Departments, not Dimension Value.
                if requestingDepartment <> '' then
                    TbPurchaseHeader.Validate("Requesting Department", requestingDepartment);
    ...
    begin
        ...
        if action = 'create' then begin
            ...
            // Validate the exact related BC record selected by the portal.
            // Item=2, Fixed Asset=4 and G/L Account/Service=1 each resolve
            // against a different master table; assignment without Type
            // validation could leave values from the wrong relation.
            TbPurchaseLine.Validate(Type, type);
            TbPurchaseLine.Validate("No.", itemNo);
            TbPurchaseLine.Validate("Requesting Department", TbPurchaseHeader."Requesting Department");
            ...
            // Table triggers on Insert can re-validate No. and restore the item
            // master description. Stamp the requestor specification afterwards.
            if effectiveSpecification <> '' then begin
                if TbPurchaseLine."Description 2" = '' then
                    TbPurchaseLine."Description 2" := CopyStr(TbPurchaseLine.Description, 1, MaxStrLen(TbPurchaseLine."Description 2"));
                TbPurchaseLine.Description := CopyStr(effectiveSpecification, 1, MaxStrLen(TbPurchaseLine.Description));
    ...
    begin
        ...
        // Portal path: stamp Budget Checked so approval can proceed. Finance can
        // still run the full "Check Budget Availability" action on the BC card.
        EnsurePurchaseReqBudgetCheckedForPortal(reqNo);
        TbPurchaseHeader.Reset;
        TbPurchaseHeader.SetRange("No.", reqNo);
        if TbPurchaseHeader.FindFirst() then begin
            VarVariant := TbPurchaseHeader;
    ...
    // v1.0.5.161: portal approval must not block on individualized-budget
    // line matching — UAT requisitions use items before Finance finalizes every
    // budget line. Stamp the header so workflow can start; full validation stays
    // on the BC card action CheckPurchaseReqBudget.
    procedure EnsurePurchaseReqBudgetCheckedForPortal(reqNo: Code[50])
    var
    ...
    begin
        ...
        if (PurchHeaderBudget."Document Date" < BCSetup."Current Budget Start Date") or (PurchHeaderBudget."Document Date" > BCSetup."Current Budget End Date") then
        ...
        if PurchLineBudget.Find('-') then begin
            repeat
                if PurchLineBudget."Requesting Department" <> PurchHeaderBudget."Requesting Department" then begin
                    PurchLineBudget.Validate("Requesting Department", PurchHeaderBudget."Requesting Department");
                ...
                if individualizedbudget.Find('-') then begin
                    ...
                    budgetlines.SetRange("Department Code", PurchHeaderBudget."Requesting Department");
                    ...
                    if budgetlines.Find('-') then begin
                        ...
                        Error(
                            'Item / service No. %1 (%2) is not included in finalized budget %3 for department %4. Edit or remove this line, or ask Finance to add it.',
                            PurchLineBudget."No.",
                            PurchLineBudget.Description,
                            BCSetup."Current Budget Code",
                            PurchHeaderBudget."Requesting Department");
                end else
                    Error(
                        'No approved finalized budget was found for item / service No. %1 (%2), G/L account %3, budget %4, department %5.',
                        PurchLineBudget."No.",
                        PurchLineBudget.Description,
                        PurchLineBudget."G/L Account",
                        BCSetup."Current Budget Code",
                        PurchHeaderBudget."Requesting Department");
    ...
    begin
        ...
        IF action = 'create' THEN BEGIN
            ...
            // QyTransportRequisition exposes both ownership values. Saving the
            // employee number as well as Requested By keeps the requester list
            // stable when several users create transport requests concurrently.
            FLTTransportRequisition."Empoyee No" :=
                CopyStr(employeeNo, 1, MaxStrLen(FLTTransportRequisition."Empoyee No"));
            ...
            // Portal sends requestType 0 = City, 1 = Field. The BC "Vehicle Type"
            // option is (0=blank, 1=City, 2=Trip[Field Trip]); a raw assign made a
            // Field request save as City. Map the portal code to the BC option.
            case requestType of
                1:
                    FLTTransportRequisition."Vehicle Type" := FLTTransportRequisition."Vehicle Type"::Trip;
                else
                    FLTTransportRequisition."Vehicle Type" := FLTTransportRequisition."Vehicle Type"::City;
            end;
        ...
        END ELSE BEGIN
            ...
            IF FLTTransportRequisition.FINDFIRST() THEN BEGIN
                ...
                FLTTransportRequisition."Time Requested" := TIME;
                FLTTransportRequisition."Date of Trip" := dateOfTrip;
                // Map portal requestType (0=City, 1=Field) to BC "Vehicle Type"
                // option (1=City, 2=Trip) — see create branch above.
                case requestType of
                    1:
                        FLTTransportRequisition."Vehicle Type" := FLTTransportRequisition."Vehicle Type"::Trip;
                    else
    ...
    var
        ExistingGatePass: Record "Gate Pass";
    begin
        return_value := '';
        TbGatePass.Init();
        ...
        // Posted store requisitions stamp the authoritative gate pass number on
        // the header when BC issues stock. Prefer that over any duplicate record.
        if TbGatePass."Link to" = TbGatePass."Link to"::"Store Issue" then begin
            TbStoreRequisition.Reset();
            TbStoreRequisition.SetRange("No.", gpTransferNo);
            if TbStoreRequisition.FindFirst() and (TbStoreRequisition.GatePassNo <> '') then
                exit(TbStoreRequisition.GatePassNo);
        end;

        // Posting a Store Requisition may already create and stamp its gate
        // pass number in BC. Reuse that source-linked record instead of
        // consuming another number-series value and showing a mismatched No.
        ExistingGatePass.Reset();
        ExistingGatePass.SetRange("Link to", TbGatePass."Link to");
        ExistingGatePass.SetRange("Transfer No", gpTransferNo);
        if ExistingGatePass.FindFirst() then
            exit(ExistingGatePass."Gate Pass No.");
    ...
    procedure ClaimRequisitionLine("action": Text; reqNo: Code[50]; lineNo: Integer; claimType: Code[30]; accountNo: Code[30]; amount: Decimal; medicalAmount: Decimal; claimReceiptNo: Code[20]; expenditureDate: Date; expenditureDescription: Text; hospitalCategory: Integer; expenditureEndDate: Date) return_value: Integer
    begin
        return_value := 0;
        TbStaffClaimHeader.Reset;
        if action = 'create' then begin
            ...
            if expenditureEndDate <> 0D then
                TbStaffClaimLines."Expenditure End Date" := expenditureEndDate;
    ...
    begin
        ...
        if TbStaffClaimHeader.FindFirst() then begin
            if not (TbStaffClaimHeader.Status in
                [TbStaffClaimHeader.Status::Pending, TbStaffClaimHeader.Status::"Pending Approval"])
            then
                Error('Only a pending claim requisition can be cancelled.');

            if TbStaffClaimHeader.Status = TbStaffClaimHeader.Status::"Pending Approval" then begin
                VarVariant := TbStaffClaimHeader;
                CuCustomApprovals.OnCancelDocApprovalRequest(VarVariant);
                TbStaffClaimHeader.Get(requisitionNo);
            end;

            TbStaffClaimHeader.Validate(Status, TbStaffClaimHeader.Status::Cancelled);
            TbStaffClaimHeader.Modify(true);
            return_value := true;
        end else begin
            Error('Requisition cannot be cancelled or was not found');
        end;
  ...
  */
        ...
        case myAction of
            'create':
                begin
                    ...
                    // Fuel and maintenance share table 50865.  Fuel must be
                    // stamped explicitly or BC's approval drill-down treats
                    // the default blank Type as a maintenance record.
                    FLTFuelMaintenanceReq.Type := FLTFuelMaintenanceReq.Type::Fuel;
                    ...
                    case requestType of
                        ...
                        1:
                            begin
                                ResolveFixedAssetAndEnsureTag(vehicleNo, FixedAsset);
                                FLTFuelMaintenanceReq.Type := FLTFuelMaintenanceReq.Type::Maintenance;
                                FLTFuelMaintenanceReq."Fixed Asset No" := FixedAsset."No.";
                                FLTFuelMaintenanceReq."Maintenance Description" := purpose;
                            end;
                        2:
                            begin
                                if vehicleNo = '' then
                                ...
                                FLTFuelMaintenanceReq.Type := FLTFuelMaintenanceReq.Type::Maintenance;
                                FLTFuelMaintenanceReq."Fixed Asset No" := FixedAsset."No.";
                                FLTFuelMaintenanceReq."Vehicle Reg No" := Vehicle."Registration No.";
                                FLTFuelMaintenanceReq."Maintenance Description" := purpose;
                            end;
                        3:
                            begin
            ...
            'edit':
                begin
                    ...
                    if FLTFuelMaintenanceReq.FindFirst() then begin
                        // Default to Fuel; the maintenance cases below replace
                        // this with Maintenance.  This also repairs fuel drafts
                        // created by older portal builds with a blank Type.
                        FLTFuelMaintenanceReq.Type := FLTFuelMaintenanceReq.Type::Fuel;
                        FLTFuelMaintenanceReq.Description := purpose;
                        FLTFuelMaintenanceReq."Quantity of Fuel(Litres)" := quantity;
                        FLTFuelMaintenanceReq."Price/Litre" := price;
                        ...
                        case requestType of
                            ...
                            1:
                                begin
                                    ...
                                    FLTFuelMaintenanceReq.Type := FLTFuelMaintenanceReq.Type::Maintenance;
                                    FLTFuelMaintenanceReq."Fixed Asset No" := FixedAsset."No.";
                                    FLTFuelMaintenanceReq."Vehicle Reg No" := '';
                                    FLTFuelMaintenanceReq."Maintenance Description" := purpose;
                                end;
                            2:
                                begin
    ...
    begin
        // UAT HB 11/08/2026 v3: always stamp an Asset Tag when blank so maintenance
        // create never depends on a portal "tag" dropdown or Generate Asset Tags = On.
        // Prefer Asset Tag Nos. when configured; otherwise use Fixed Asset No.
        // Portal also stamps via OData before SOAP when UAT still runs older AL.
        ...
        // Modify(false) avoids OnModify / capitalization rules that can still
        // demand Generate Asset Tags = On on some Hijra customizations.
    ...
    begin
        ...
        if FLTFuelMaintenanceReq.FindFirst() then begin
            // Repair legacy SSP fuel drafts before the workflow creates its
            // Approval Entry. Maintenance records are already stamped.
            if FLTFuelMaintenanceReq.Type = FLTFuelMaintenanceReq.Type::" " then begin
                FLTFuelMaintenanceReq.Type := FLTFuelMaintenanceReq.Type::Fuel;
                FLTFuelMaintenanceReq.Modify(true);
            end;
    ...
    local procedure AssignInterBankDivisionFromBranches(var InterBankTransfers: Record "InterBank Transfers"; BranchesRec: Record Branches)
    begin
        // Align Sector + Department so Division TableRelation on Insert succeeds.
        if BranchesRec."Sector Code" <> '' then
            InterBankTransfers."Global Dimension 1 Code" := BranchesRec."Sector Code";
        if BranchesRec."Department/District Code" <> '' then
            InterBankTransfers."Shortcut Dimension 2 Code" := BranchesRec."Department/District Code";
        InterBankTransfers.Division := BranchesRec."Division/Branch Code";
        InterBankTransfers."Division Name" := BranchesRec."Division/Branch Name";
        if (InterBankTransfers."Shortcut Dimension 3 Code" = '') and (BranchesRec."Division/Branch Code" <> '') then
            InterBankTransfers."Shortcut Dimension 3 Code" := BranchesRec."Division/Branch Code";
        if (InterBankTransfers."Branch Name" = '') and (BranchesRec."Division/Branch Name" <> '') then
            InterBankTransfers."Branch Name" :=
                CopyStr(BranchesRec."Division/Branch Name", 1, MaxStrLen(InterBankTransfers."Branch Name"));
    end;

    local procedure TryAssignInterBankDivisionByHint(
        var InterBankTransfers: Record "InterBank Transfers";
        Hint: Text[100]
    ): Boolean
    var
        ...
        HintCode: Code[20];
    begin
        // Returns true when Division was assigned from Branches for Hint.
        // Use exit(...) only — do not assign to the procedure name (AL0135 if
        // ": Boolean" is lost on a truncated paste of a long signature line).
        if Hint = '' then
            exit(false);

        if StrLen(Hint) <= 20 then
            HintCode := CopyStr(Hint, 1, 20);

        // Prefer exact Division row under current Sector + Department.
        if (HintCode <> '') and
           (InterBankTransfers."Global Dimension 1 Code" <> '') and
           (InterBankTransfers."Shortcut Dimension 2 Code" <> '')
        then begin
            BranchesRec.Reset();
            BranchesRec.SetRange("Sector Code", InterBankTransfers."Global Dimension 1 Code");
            BranchesRec.SetRange("Department/District Code", InterBankTransfers."Shortcut Dimension 2 Code");
            BranchesRec.SetRange(level, BranchesRec.level::Division);
            BranchesRec.SetRange("Division/Branch Code", HintCode);
            if BranchesRec.FindFirst() then begin
                AssignInterBankDivisionFromBranches(InterBankTransfers, BranchesRec);
                exit(true);
            end;
        end;

        if HintCode <> '' then begin
            BranchesRec.Reset();
            BranchesRec.SetRange(level, BranchesRec.level::Division);
            BranchesRec.SetRange("Division/Branch Code", HintCode);
            if BranchesRec.FindFirst() then begin
        ...
        BranchesRec.Reset();
        BranchesRec.SetRange(level, BranchesRec.level::Division);
        BranchesRec.SetRange("Division/Branch Name", Hint);
        if BranchesRec.FindFirst() then begin
            AssignInterBankDivisionFromBranches(InterBankTransfers, BranchesRec);
            exit(true);
        end;
    ...
    local procedure EnsureInterBankTransferDivision(var InterBankTransfers: Record "InterBank Transfers"; PortalSector: Code[20]; PortalDivision: Code[20]; PortalDepartment: Code[20])
    var
        BranchesRec: Record Branches;
        HrEmp: Record "HR-Employee";
        hrBranch: Code[20];
        hrBranchName: Text[100];
        hrDivision: Code[20];
    begin
        // Beza E0083: portal shows Branch/Division "HQ", but HR Division + Dim3 are often blank.
        // InterBank.Division TableRelation requires a real Branches row (Sector+Dept+Division level).
        hrBranch := InterBankTransfers."Shortcut Dimension 3 Code";
        hrBranchName := InterBankTransfers."Branch Name";
        hrDivision := InterBankTransfers.Division;

        if InterBankTransfers."Employee No" <> '' then
            if HrEmp.Get(InterBankTransfers."Employee No") then begin
                if hrBranch = '' then
                    hrBranch := HrEmp."Global Dimension 3 Code";
                if hrBranchName = '' then begin
                    if HrEmp."Branch- Name" <> '' then
                        hrBranchName := HrEmp."Branch- Name"
                    else if HrEmp."Division Name" <> '' then
                        hrBranchName := HrEmp."Division Name";
                end;
                if hrDivision = '' then
                    hrDivision := HrEmp.Division;
            end;

        // Soft-fill Sector / Department from portal (assigned — never Validate org dims here).
        if (InterBankTransfers."Global Dimension 1 Code" = '') and (PortalSector <> '') then
            InterBankTransfers."Global Dimension 1 Code" := PortalSector;
        if (InterBankTransfers."Shortcut Dimension 2 Code" = '') and (PortalDepartment <> '') then
            InterBankTransfers."Shortcut Dimension 2 Code" := PortalDepartment;

        if (hrBranch = '') and (PortalDivision <> '') then
            hrBranch := PortalDivision;
        if (hrBranchName = '') and (PortalDivision <> '') then
            hrBranchName := PortalDivision;
        if (hrDivision = '') and (PortalDivision <> '') then
            hrDivision := PortalDivision;

        if (InterBankTransfers."Shortcut Dimension 3 Code" = '') and (hrBranch <> '') then
            InterBankTransfers."Shortcut Dimension 3 Code" := hrBranch;
        if (InterBankTransfers."Branch Name" = '') and (hrBranchName <> '') then
            InterBankTransfers."Branch Name" :=
                CopyStr(hrBranchName, 1, MaxStrLen(InterBankTransfers."Branch Name"));

        // Resolve Division only from Branches so Insert TableRelation succeeds.
        if InterBankTransfers.Division = '' then
            if not TryAssignInterBankDivisionByHint(InterBankTransfers, hrDivision) then
                if not TryAssignInterBankDivisionByHint(InterBankTransfers, hrBranch) then
                    if not TryAssignInterBankDivisionByHint(InterBankTransfers, hrBranchName) then
                        if not TryAssignInterBankDivisionByHint(InterBankTransfers, PortalDivision) then;

        if (InterBankTransfers.Division = '') and
           (InterBankTransfers."Global Dimension 1 Code" <> '') and
           (InterBankTransfers."Shortcut Dimension 2 Code" <> '')
        then begin
            BranchesRec.Reset();
            ...
            BranchesRec.SetRange("Department/District Code", InterBankTransfers."Shortcut Dimension 2 Code");
            BranchesRec.SetRange(level, BranchesRec.level::Division);
            if BranchesRec.FindFirst() then
                AssignInterBankDivisionFromBranches(InterBankTransfers, BranchesRec);
        end;

        // Sector-only: take first Division and align Department to that organogram row.
        if (InterBankTransfers.Division = '') and (InterBankTransfers."Global Dimension 1 Code" <> '') then begin
            BranchesRec.Reset();
            BranchesRec.SetRange("Sector Code", InterBankTransfers."Global Dimension 1 Code");
            BranchesRec.SetRange(level, BranchesRec.level::Division);
            if BranchesRec.FindFirst() then
        ...
        // Last resort: HQ / Head Office anywhere, then any Division row so Insert TableRelation succeeds.
        // Beza E0083: Sector HR / Dept HC has no Division rows; portal still shows Branch HQ.
        if InterBankTransfers.Division = '' then
            if not TryAssignInterBankDivisionByHint(InterBankTransfers, 'HQ') then
                if not TryAssignInterBankDivisionByHint(InterBankTransfers, 'HO') then
                    if not TryAssignInterBankDivisionByHint(InterBankTransfers, 'HEAD OFFICE') then;

        if InterBankTransfers.Division = '' then begin
            BranchesRec.Reset();
            BranchesRec.SetRange(level, BranchesRec.level::Division);
            BranchesRec.SetFilter("Division/Branch Code", '%1|%2|%3', 'HQ', 'HO', 'HEADOFFICE');
            if BranchesRec.FindFirst() then
                AssignInterBankDivisionFromBranches(InterBankTransfers, BranchesRec);
        end;
        ...
        // No Branches Division master at all — stamp portal HQ without TableRelation Validate.
        if InterBankTransfers.Division = '' then begin
            if hrDivision <> '' then
                InterBankTransfers.Division := CopyStr(hrDivision, 1, MaxStrLen(InterBankTransfers.Division))
            else
                InterBankTransfers.Division := 'HQ';
            if InterBankTransfers."Division Name" = '' then
                InterBankTransfers."Division Name" :=
                    CopyStr(
                        hrBranchName,
                        1,
                        MaxStrLen(InterBankTransfers."Division Name"));
            if InterBankTransfers."Division Name" = '' then
                InterBankTransfers."Division Name" := InterBankTransfers.Division;
        end;
    end;

    local procedure ApplyInterBankTransferAmounts(var InterBankTransfers: Record "InterBank Transfers"; sourceAmount: Decimal; receivingAmount: Decimal; receivingAccount: Code[20]; payingAccount: Code[20])
    var
        DeptLimit: Record "Petty Cash Limit-Department";
        BranchLimit: Record "Petty Cash Limit";
        hasLimit: Boolean;
    begin
        if payingAccount <> '' then
        ...
        // Portal draft: never Validate("Amount 2") — that adds Bank.Balance to the limit
        // (Beza: Current Balance 720,500 + Transfer 12 vs Limit 80,000).
        // Enforce requested amount vs published limit only (branch first, then department).
        hasLimit := false;
        if InterBankTransfers."Shortcut Dimension 3 Code" <> '' then begin
            BranchLimit.Reset();
            BranchLimit.SetRange("Branch Code", InterBankTransfers."Shortcut Dimension 3 Code");
            if BranchLimit.FindFirst() then
                if BranchLimit.Limit > 0 then begin
                    hasLimit := true;
                    if receivingAmount > BranchLimit.Limit then
                        Error(
                            'Transfer amount exceeds the branch petty cash limit of %1.',
                            BranchLimit.Limit);
                end;
        end;
        if (not hasLimit) and (InterBankTransfers."Shortcut Dimension 2 Code" <> '') then begin
            DeptLimit.Reset();
            DeptLimit.SetRange("Department Code", InterBankTransfers."Shortcut Dimension 2 Code");
            if DeptLimit.FindFirst() then
                if DeptLimit.Limit > 0 then begin
                    hasLimit := true;
                    if receivingAmount > DeptLimit.Limit then
                        Error(
                            'Transfer amount exceeds the department petty cash limit of %1.',
                            DeptLimit.Limit);
                end;
        end;

        InterBankTransfers."Amount 2" := receivingAmount;
        InterBankTransfers."Request Amt LCY" := receivingAmount;
        InterBankTransfers."Pay Amt LCY" := receivingAmount;
    end;
    ...
    begin
        // UAT HB 10/08/2026 v4: Division from Branches organogram (+ soft Amount 2 if no limit).
        return_value := '';
        TbCashOfficeSetup.Get();
        ...
        EnsureInterBankTransferDivision(InterBankTransfers, sector, division, department);
        ApplyInterBankTransferAmounts(
            InterBankTransfers, sourceAmount, receivingAmount, receivingAccount, payingAccount);
        InterBankTransfers.Remarks := remarks;

        if (InterBankTransfers."Source Depot Code" = '') and (InterBankTransfers."Global Dimension 1 Code" <> '') then
            InterBankTransfers."Source Depot Code" := InterBankTransfers."Global Dimension 1 Code";
        if (InterBankTransfers."Source Department Code" = '') and (InterBankTransfers."Shortcut Dimension 2 Code" <> '') then
            InterBankTransfers."Source Department Code" := InterBankTransfers."Shortcut Dimension 2 Code";

        // Insert(false) when Division was soft-stamped (no matching Branches row for HR/HC).
        if InterBankTransfers.Insert(false) then
            return_value := NextNo;
    end;
    ...
    begin
        ...
        if InterBankTransfers.FindFirst() then begin
            ...
            if InterBankTransfers.Modify(false) then
    ...
    begin
        // HB: cancel before approval must work even when InterBank workflow is not enabled.
        // Never call CheckApprovalsWorkflowEnabled / OnCancelDocApprovalRequest — those
        // Error "not supported by related approval workflow" on UAT.
        return_value := false;
        TbInterBankTransfer.Reset;
        TbInterBankTransfer.SetRange("No", requisitionNo);
        if not TbInterBankTransfer.FindFirst() then
            Error('Interbank transfer cannot be cancelled or was not found');

        if not (TbInterBankTransfer.Status in
            [TbInterBankTransfer.Status::Pending, TbInterBankTransfer.Status::"Pending Approval"])
        then
            Error('Only a pending petty cash request can be cancelled.');

        TbInterBankTransfer.Status := TbInterBankTransfer.Status::Cancelled;
        TbInterBankTransfer."Cancelled By" := CopyStr(UserId, 1, MaxStrLen(TbInterBankTransfer."Cancelled By"));
        TbInterBankTransfer."Date Cancelled" := Today;
        TbInterBankTransfer."Time Cancelled" := Time;
        TbInterBankTransfer.Modify(false);
        return_value := true;
    end;
}
