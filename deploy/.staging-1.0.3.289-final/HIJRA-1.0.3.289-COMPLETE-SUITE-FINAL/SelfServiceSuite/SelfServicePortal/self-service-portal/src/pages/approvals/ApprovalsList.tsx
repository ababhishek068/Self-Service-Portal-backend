...
export function ApprovalsList({ type, title, emptyTitle }: ApprovalsListProps) {
  ...
  const filters = [
    ...
    // Petty Cash Request = InterBank / replenishment (was mislabelled "PV").
    { value: 'pettyCashReplenishment', label: 'PC Req' },
    { value: 'pettyCash', label: 'PC Set' },
  ...
  const rows = useMemo(() => {
    ...
    return sourceRows.filter((row) => {
      ...
      // Prefer document-no match so searching "2113" does not hit unrelated
      // cards that only share digits in timestamps / entry metadata.
      if (q && /^\d+[A-Za-z0-9-]*$/.test(q)) {
        const no = String(row.requestNo ?? '').toLowerCase()
        const needle = q.toLowerCase()
        if (no === needle || no.includes(needle)) return true
        // Fall through to broad search only when no document-no hit is required
        // for short numeric queries that are clearly document numbers.
        if (q.length >= 3) return false
      }
      return matchesSearchQuery(row, search)
    })
  ...
  const columns: DataTableColumn<ApprovalQueueItem>[] = type === 'pending' ? [
    ...
    { id: 'amount', header: 'Amount', cell: (row) => {
      if (Number(row.amount ?? 0) > 0) return formatCurrency(row.amount)
      // Non-financial facility docs (transport etc.) have no currency total.
      if (
        row.module === 'transport' ||
        row.module === 'maintenance' ||
        row.module === 'gatePass' ||
        row.module === 'workTickets' ||
        row.module === 'assetTransfer' ||
        row.module === 'leave'
      ) {
        return '—'
      }
