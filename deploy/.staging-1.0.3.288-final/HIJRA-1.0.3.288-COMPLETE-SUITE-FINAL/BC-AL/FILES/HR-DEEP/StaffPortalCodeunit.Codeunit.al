codeunit 50049 "Staff Portal Codeunit"
{
    ...
    begin
        ...
        if action = 'create' then begin
            ...
            // Keep the master description for audit, while the visible
            // Description shows the requestor's exact specification (for
            // example "Patch Pannel") in BC and on the approval screen.
                ...
                begin
                    ...
                    case requestType of
                        ...
                        1:
                            begin
                                if vehicleNo = '' then
                                    Error('Fixed Asset Tag Number is required for fixed asset maintenance.');
                                FixedAsset.Reset();
                                FixedAsset.SetRange("Asset Tag", vehicleNo);
                                if not FixedAsset.FindFirst() then
                                    Error('Fixed Asset Tag Number %1 was not found. Select a current asset tag from Business Central.', vehicleNo);
                                FLTFuelMaintenanceReq.Type := FLTFuelMaintenanceReq.Type::Maintenance;
                                // The portal submits the human-facing Asset Tag;
                                // table 50865 must retain the internal Fixed Asset No.
                        ...
                        2:
                            begin
                                ...
                                if FixedAsset."Asset Tag" = '' then
                                    Error('Vehicle %1 is linked to Fixed Asset %2, but that asset has no Asset Tag. Set its Asset Tag in Business Central.', vehicleNo, FixedAsset."No.");
                                FLTFuelMaintenanceReq.Type := FLTFuelMaintenanceReq.Type::Maintenance;
                                FLTFuelMaintenanceReq."Fixed Asset No" := FixedAsset."No.";
