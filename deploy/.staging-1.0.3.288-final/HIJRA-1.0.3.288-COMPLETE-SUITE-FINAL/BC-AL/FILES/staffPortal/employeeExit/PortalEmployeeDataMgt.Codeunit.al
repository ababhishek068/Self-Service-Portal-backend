...
codeunit 52105 "Portal Employee Data Mgt."
{
    ...
    begin
        ...
        // UAT R10 (30/07/2026): the leave-request FORM must display the SAME
        // balance as the printed leave STATEMENT. The statement (Report 50368
        // "Leave statements1") computes its closing balance as the CalcSums of
        // the posted "HR Leave Allocation"."No. Of days" for the employee + leave
        // type — with no calendar/period/date filter. Reproduce that exact figure
        // here (for both annual and non-annual) so the two views never disagree.
        HRLeaveAlloc.Reset();
        HRLeaveAlloc.SetRange("No.", employeeNo);
        HRLeaveAlloc.SetRange("Leave Type", leaveType);
        ...
        if not HRLeaveAlloc.IsEmpty() then begin
            ...
            CardAnnualBalance := HRLeaveAlloc."No. Of days";
        end;
