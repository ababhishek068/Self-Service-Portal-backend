...
import { formatCurrency, formatDate, toHtmlDateInputValue } from '@/utils/formatters'
...
export interface MultiStepLineConfig {
  ...
  emptyText?: string | ((request: PortalRequest) => string)
  /** Some documents may legitimately have no child lines. Defaults to true. */
  required?: boolean | ((request: PortalRequest) => boolean)
  /** Override the toast shown when Request Approval is clicked with zero lines. */
  requiredMessage?: string
  /** Hide the whole lines section (ordinary FA does not use vehicle tools). */
  visibleWhen?: (request: PortalRequest) => boolean
}

function lineIsRequired(line: MultiStepLineConfig | undefined, request?: PortalRequest) {
  if (!line) return false
  if (request && line.visibleWhen && !line.visibleWhen(request)) return false
  if (typeof line.required === 'function') return request ? line.required(request) : false
  return line.required !== false
}

function lineEmptyMessage(line: MultiStepLineConfig, request?: PortalRequest) {
  if (typeof line.emptyText === 'function') {
    return request ? line.emptyText(request) : 'No lines.'
  }
  return line.emptyText ?? 'No lines.'
}
...
export interface MultiStepRequestConfig {
  ...
  /** Optional content rendered above the header fields (e.g. “How this works”). */
  headerPreface?: ReactNode
  /** Optional read-only content derived from the current header form values.
   *  The second argument is `form.setValue` so the supplement can update fields
   *  (e.g. a remark textarea rendered inside the supplement panel). */
  headerSupplement?: (values: FieldValues, setValue: UseFormReturn<FieldValues>['setValue']) => ReactNode
  /** Keep derived header fields in sync (e.g. Expected Return after Travel Start). */
  headerOnValuesChange?: (
    values: FieldValues,
    form: UseFormReturn<FieldValues>,
  ) => void | Promise<void>
  /** Status/next-step guide on the detail screen (module-specific). */
  detailGuide?: (ctx: {
...
function storeRequisitionHasIssuedLines(lines: Record<string, unknown>[]) {
  return lines.some((line) => storeLineIssuedQty(line) > 0)
}

function storeHeaderFullyIssued(payload: Record<string, unknown> | undefined) {
  if (!payload) return false
  const value = payload.FullyIssued ?? payload.Fully_Issued ?? payload.fullyIssued
...
function initialFieldValue(source: Record<string, unknown>, field: FieldConfig, fallback: unknown) {
  ...
  const resolved = mapped(source[key])
  return field.type === 'date' ? toHtmlDateInputValue(resolved) : resolved
}
  ...
  return function renderField(field: FieldConfig) {
    ...
    return (
      ...
      >
        ...
        {!message && field.hint ? <p className="text-xs text-slate-500">{field.hint}</p> : null}
      </div>
    )
  }
}
    ...
    <PageWrapper title={request ? `Edit ${config.title}` : config.headerLabel ?? config.title} showPageHeading={false}>
      ...
      >
        <form className="space-y-4" onSubmit={(event) => event.preventDefault()}>
          ...
          {config.headerSupplement ? config.headerSupplement(watchedValues, form.setValue) : null}
          {mutation.error ? (
          ...
          {displayRows.length === 0 ? (
            <tr>
              <td colSpan={columns.length} className="px-2 py-4 text-center text-slate-500">
                {typeof line.emptyText === 'function' ? 'No lines.' : (line.emptyText ?? 'No lines.')}
              </td>
            </tr>
          ) : null}
        </tbody>
...
export function MultiStepRequestPage(config: MultiStepRequestConfig) {
  ...
  const filteredRequests = useMemo(() => {
    ...
    const statusFiltered =
      !config.listStatusFilter || listStatus === 'all'
        ? rows
        : rows.filter((row) => matchesRequestListStatus(row.status, listStatus))
  ...
  const storeHasIssued =
    storeRequisitionHasIssuedLines(selectedLines) ||
    storeHeaderFullyIssued(selected?.payload as Record<string, unknown> | undefined)
  // Store headers often stay Status=Open after approval. Lock edits once approved/issued
  // so Action shows Receive Items instead of Delete.
  ...
  // Show receive actions whenever store has issued — do not require Approved status label.
  const canReceiveStoreLines =
    config.module.module === 'storeRequisition' && storeHasIssued
  const storeAwaitingIssue =
    config.module.module === 'storeRequisition' &&
    (storeApprovalDone || selected?.status === 'Approved') &&
    !storeHasIssued
  const storeLinesNeedingReceipt = selectedLines.filter((line) => storeLineNeedsReceipt(line))

  const beginReceiveLine = (line: Record<string, unknown>) => {
    setReceiveLine(line)
    const received = Number(line.quantityReceived ?? 0)
    const issued = storeLineIssuedQty(line)
  ...
  const submitReceiveLine = async () => {
    ...
    const issued = storeLineIssuedQty(receiveLine)
    if (issued <= 0) {
      toast.error('Store has not issued this line yet. Wait for store issue in Business Central.', 'Receive failed')
      return
    }
    const alreadyReceived = Number(receiveLine.quantityReceived ?? 0)
    const outstanding = Math.max(0, issued - alreadyReceived)
    if (quantityToReceive > outstanding) {
      toast.error(
        `Quantity to receive cannot exceed outstanding issued quantity (${outstanding}).`,
        'Receive failed',
      )
      return
    }
    if (quantityToReceive < outstanding && !String(receiveReason ?? '').trim()) {
      toast.error('Enter a reason when receiving less than the issued quantity.', 'Receive failed')
      return
    }
    await runAction(
      `receive-${lineId}`,
  ...
  const postStoreReceipt = async () => {
    if (!selected) return
    if (storeLinesNeedingReceipt.length === 0) {
      toast.error('No issued lines are waiting for receipt confirmation.', 'Receive failed')
      return
    }
    const yes = await confirm({
      title: 'Confirm receipt of all lines',
      message: `Confirm you have received all outstanding issued quantities (${storeLinesNeedingReceipt.length} line${storeLinesNeedingReceipt.length === 1 ? '' : 's'})?`,
    ...
    await runAction(
      ...
      'Receive failed',
      'Receipt confirmed for outstanding lines',
    )
  }
  ...
  if (selectedId) {
    ...
    const detailFields = (config.detailFields ?? [])
      .map((field) => ({ field, value: firstPathValue(detailSource, field.paths, field.format) }))
      .filter(({ field, value }) => {
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  {submittable ? (
                    <Button
                      ...
                      onClick={() => {
                        if (lineIsRequired(config.line, selected) && lines.length === 0) {
                          toast.warning(
                            config.line?.requiredMessage ||
                              `Add at least one ${config.line?.label.toLowerCase() ?? 'line'} entry before requesting approval.`,
                            'Lines required',
                          )
                          return
                        }
            ...
            {selected ? (
              <>
                ...
                <RequestProgress
                  ...
                  requiresLines={lineIsRequired(config.line, selected)}
                ...
                />
                {config.detailGuide
                  ? config.detailGuide({ request: selected, lines })
                  : null}
                <section>
                ...
                {config.line && (!config.line.visibleWhen || (selected && config.line.visibleWhen(selected))) ? (
                  <section className="border-t border-slate-200 pt-4">
                    ...
                    {storeAwaitingIssue ? (
                      <div className="mb-4 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-950">
                        <p className="font-semibold">Waiting for store issue</p>
                        <p className="mt-1 text-xs sm:text-sm">
                          This request is approved. Store staff will issue the items in Business Central.
                          When stock is issued, you can confirm receipt here.
                        </p>
                      </div>
                    ) : null}
                    {canReceiveStoreLines ? (
                      <div className="mb-4 rounded-xl border border-sky-200 bg-sky-50 px-4 py-3 text-sm text-sky-950">
                        <p className="font-semibold">Your turn — confirm receipt</p>
                        <p className="mt-1 text-xs sm:text-sm">
                          Store has issued stock. Confirm each line (or use <strong>Receive all</strong> above).
                          If you receive less than issued, enter a short reason.
                        </p>
                      </div>
                    ) : null}
                    {canReceiveStoreLines && receiveLine ? (
                      <div className="mb-4 rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
                        <div className="mb-3 text-sm font-semibold text-[var(--portal-navy)]">
                          Confirm receipt — line {String(receiveLine.lineNo ?? receiveLine.id ?? '')}
                        </div>
                        <p className="mb-3 text-xs text-slate-600">
                          Issued{' '}
                          <strong>{storeLineIssuedQty(receiveLine)}</strong>
                          {Number(receiveLine.quantityReceived ?? 0) > 0
                            ? ` · already received ${Number(receiveLine.quantityReceived ?? 0)}`
                            : ''}
                          {' · '}
                          outstanding{' '}
                          <strong>
                            {Math.max(
                              0,
                              storeLineIssuedQty(receiveLine) - Number(receiveLine.quantityReceived ?? 0),
                            )}
                          </strong>
                        </p>
                        <div className="grid gap-3 md:grid-cols-2">
                          ...
                          <div className="space-y-1.5">
                            <Label htmlFor="receive-reason">
                              Reason if partial{' '}
                              <span className="font-normal text-slate-400">(required when less)</span>
                            </Label>
                            <Input
                              id="receive-reason"
                              value={receiveReason}
                              onChange={(event) => setReceiveReason(event.target.value)}
                              placeholder="e.g. One unit damaged / short-shipped"
                            />
                          </div>
                        </div>
                        ...
                        <table className="w-full text-left text-sm">
                          <thead className="border-b border-slate-200 text-xs text-slate-500">
                            <tr>
                              ...
                              {canReceiveStoreLines || editable ? <th className="px-2 py-2">Action</th> : null}
                            </tr>
                          </thead>
                          <tbody>
                            {lines.map((row, index) => (
                            ...
                            {lines.length === 0 ? (
                              <tr>
                                <td colSpan={lineColumns.length + (canReceiveStoreLines || editable ? 1 : 0)} className="px-2 py-4 text-center text-slate-500">
                                  {lineEmptyMessage(config.line, selected)}
                                </td>
                              </tr>
                            ) : null}
                          </tbody>
