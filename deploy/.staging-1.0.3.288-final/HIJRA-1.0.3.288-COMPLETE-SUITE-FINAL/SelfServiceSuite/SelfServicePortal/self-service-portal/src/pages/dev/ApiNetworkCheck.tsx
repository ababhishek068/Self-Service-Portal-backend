...
export function ApiNetworkCheck() {
  ...
  const filteredResults = useMemo(
    () => results.filter((result) => matchesSearchQuery(result, search)),
    [results, search],
  )
  ...
  const runAll = async () => {
    ...
    const checks: Array<() => Promise<CheckResult>> = [
      ...
      () => runCheck('Leave types', 'GET', '/api/leave/types', async () => (await fetchLeaveTypes()).types),
        ...
        {results.length > 0 ? (
          <div className="space-y-3">
            <ListSearch
              ...
              placeholder="Search API checks..."
              ariaLabel="Search API checks"
            />
                ...
                {filteredResults.length === 0 ? (
                  <tr>
                    <td colSpan={6} className="px-3 py-6 text-center text-slate-500">
                      No API checks match &quot;{search.trim()}&quot;.
                    </td>
                  </tr>
                ) : null}
              </tbody>
