...
export function TransportRequest() {
  ...
  return (
    <MultiStepRequestPage
      ...
      description="Create the transport header, then add passengers before requesting approval. The system blocks a second request from the same user for the same route on the same trip date."
      module={module}
      ...
      listValueColumn={{
        ...
        cell: (row) => {
          const passengers = Number(
            row.payload?.passengerCount ??
            row.payload?.No_Of_Passangers ??
            row.payload?.NoOfPassengers ??
            0
          )
          return passengers > 0 ? String(passengers) : '—'
        },
      }}
      ...
      headerFields={[
        ...
        { name: 'commencement', label: 'Trip Origin / Starting From', type: 'text', valuePaths: ['Commencement', 'CommenceFrom', 'StartingFrom'] },
      ...
      line={{
        ...
        columns: [
          ...
          { key: 'passengerOrganization', header: 'Organization / Position' },
        ],
