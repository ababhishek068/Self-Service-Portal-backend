...
export async function fetchLeaveTypes(): Promise<{
  types: LeaveType[]
  employeeGender: 'Male' | 'Female' | null
  genderMissing: boolean
}> {
  requireAuthApiUrl()
  const { rows, employeeGender, genderMissing } = await authGet<{
    rows: Array<{
      Code: string
      Description: string
      Days: number
  ...
  return {
    employeeGender: employeeGender ?? null,
    genderMissing: Boolean(genderMissing),
    types: rows.map((row) => ({
    code: row.Code,
