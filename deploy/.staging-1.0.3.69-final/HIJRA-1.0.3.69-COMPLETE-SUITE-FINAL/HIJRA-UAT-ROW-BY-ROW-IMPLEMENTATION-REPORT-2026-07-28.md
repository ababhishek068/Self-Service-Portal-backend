...
- Facility workbook: 39 latest-fail rows, including the comments describing the missing templates and wrong integrations.
- Total: 71 rows.
- The workbooks contain ordinary comment/remark columns; no threaded Excel comments were present.
...
| 28 | Employee cannot request own experience letter | Experience Letter is a requestable HR service letter using `CuPortalHrLetters` | Pending live UAT |
...
| 59 | Procurement budget template missing | Procurement Plan header template is included | Pending live UAT |
| 60 | Itemized procurement plan | Multiple procurement plan lines are included | Pending live UAT |
| 61 | Submit itemized procurement plan | Plan submit flow and `CuPortalFacility` integration are included | Pending live UAT |

Primary code: `staffModules.ts`, `portalApi.ts`, `PurchaseRequisition.tsx`, `StoreRequisition.tsx`, `FuelRequest.tsx`, `TransportRequest.tsx`, `MaintenanceRequest.tsx`, `WorkTickets.tsx`, `AssetTransfer.tsx`, `GatePass.tsx`, `ProcurementPlan.tsx`, and the 22-file safe AL patch.

## Verification evidence

- Backend TypeScript build: passed.
- Portal on-prem TypeScript/Vite build: passed.
- Backend tests: 69 passed, 0 failed.
- Local health/start smoke test: passed; build ID is `hijra-portal-1.0.3.69-FINAL-2026-07-28-hr-finance-facility-integrated`.
- Release and Felix ZIP integrity: passed.
- Compiled JavaScript syntax checks: passed.
- AL duplicate checks: one codeunit 52106; zero query 52133; one tableextension 52133; exactly one object for every ID 52160–52170; zero query files in `facilityUat`.
- Mac AL compile: no new patch/query/permission errors. The remaining errors are pre-existing environment-only blockers: a Windows report-layout path and OnPrem DotNet aliases unavailable to the macOS AL compiler. Felix must run the final `AL: Package` on Windows.
