...
import { callSoapMethod, fetchOData, fetchODataCount, odataString, type ODataRecord } from '../infrastructure/bc/client.js'
import { requireAuth, type AuthUser } from '../middleware/auth.js'
import { approvalTableFilter, resolveApprovalModuleFromTableId } from '../domain/approval/tableIds.js'
import { mapItem, mapRequest, type PortalModuleKey } from '../domain/erp/mappings.js'
import {
...
import { asyncHandler as safe } from '../shared/asyncHandler.js'
...
import { portalError } from '../shared/portalError.js'
...
export function approvalModule(row: ODataRecord): SupportedFrontendModule {
  const tableId = Number(row.TableID ?? row.TableId ?? 0)
  const documentType = text(row, ['DocumentType', 'Document_Type'])
  if (
    tableId === 50865 &&
    /maintenance|service/i.test(documentType)
  ) {
    return 'maintenance'
  }
...
async function mappedModuleRows(
  ...
  let rows = await listPortalModuleRows(spec, authUser, {
    gatePassSource: options.gatePassSource,
  })
  if (module === 'fuelRequest' || module === 'maintenance') {
    rows = rows.filter((row) => {
      const raw = text(row, ['RequestType', 'Request_Type']).trim().toLowerCase()
      const numeric = Number(raw)
      const maintenanceType = Number.isFinite(numeric) && raw !== ''
        ? numeric !== 0 && numeric !== 3
        : raw.includes('maintenance') || raw.includes('service')
      return module === 'maintenance' ? maintenanceType : !maintenanceType
    })
  }
  return rows
      ...
      const [
        ...
        Promise.all(
          listModules.map((module) =>
            mappedModuleRows(module, authUser).catch(() => []),
          ),
        ),
      ])
