 ...
 */
export function employeeExitCallError(error: unknown) {
  ...
  if (/HR Approver User ID is blank|No eligible HR approver|More than one eligible HR approver/i.test(message)) {
    return Object.assign(
      new Error(
        'HR approval routing is not ready in Business Central. Ask an authorized HR or Business Central administrator to confirm the HR approver for Employee Transfer and Resignation.',
      ),
      { status: 503, code: 'EMPLOYEE_EXIT_HR_ROUTING_REQUIRED' },
    )
  }
  return error
}
...
const REQUIRED_DETAILS: Record<EmployeeExitRequestType, string[]> = {
  ...
  // Mirrors ABH Partners' Employee Exit SSP template.
...
const FIELD_LABELS: Record<string, string> = {
  ...
  leavingReasons: 'Reason(s) for leaving ABH Partners',
  ...
  wouldReturn: 'Would consider returning to ABH Partners',
...
export function validateEmployeeExitDetails(
  ...
  if (requestType === 'exit-interview') {
    ...
    if (details.wouldReturn && !['Yes', 'No', 'Maybe'].includes(details.wouldReturn)) {
      errors.push('Answer whether you would consider returning to ABH Partners (Yes, No, or Maybe).')
    }
...
export const EMPLOYEE_EXIT_MODULE = 'employeeExit' as const

function employeeExitQueueStatus(status: EmployeeExitRequestStatus) {
  if (status === 'Approved' || status === 'Completed') return 'Approved'
  if (status === 'Rejected' || status === 'Cancelled') return 'Rejected'
  return 'Pending Approval'
}

export function employeeExitDetailFacts(request: EmployeeExitRequest) {
  const facts: Array<{ label: string; value: string }> = [
    { label: 'Request type', value: request.requestTypeLabel || request.requestType },
    { label: 'Department', value: request.departmentName },
  ]
  if (request.pendingStageLabel) {
    facts.push({ label: 'Waiting on', value: request.pendingStageLabel })
  }
  for (const [key, value] of Object.entries(request.details ?? {})) {
    const text = String(value ?? '').trim()
    if (text) facts.push({ label: FIELD_LABELS[key] ?? key, value: text })
  }
  if (request.decisionRemarks?.trim()) {
    facts.push({ label: 'Decision remarks', value: request.decisionRemarks.trim() })
  }
  return facts.filter((fact) => fact.value)
}

export function employeeExitApprovalQueueItem(request: EmployeeExitRequest) {
  const stage = request.pendingStage === 2 ? 2 : 1
  return {
    id: `${EMPLOYEE_EXIT_MODULE}-${request.requestNo}`,
    requestNo: request.requestNo,
    module: EMPLOYEE_EXIT_MODULE,
    title: request.pendingStageLabel
      ? `${request.requestTypeLabel} — ${request.pendingStageLabel}`
      : request.requestTypeLabel || 'Employee Exit',
    makerEmployeeNo: request.employeeNo,
    makerName: request.employeeName,
    amount: 0,
    status: employeeExitQueueStatus(request.status),
    submittedAt: request.submittedAt,
    approverEmployeeNo:
      stage === 2
        ? request.hrApproverUserId || request.hrDecisionBy || ''
        : request.supervisorUserId || request.supervisorDecisionBy || '',
    sourceDocumentNo: request.requestNo,
  }
}

function exitStepStatus(
  done: boolean,
  rejected: boolean,
  pending: boolean,
): 'Submitted' | 'Pending Approval' | 'Approved' | 'Rejected' | 'Draft' {
...
/** Portal Approvals detail payload — supervisor then HR, matching Hijra BC workflow. */
export function employeeExitToPortalRequest(request: EmployeeExitRequest) {
  const queue = employeeExitApprovalQueueItem(request)
  const rejectedAtSupervisor =
    request.status === 'Rejected' && request.rejectedAtStage === 'Immediate Supervisor'
  const rejectedAtHr = request.status === 'Rejected' && request.rejectedAtStage === 'HR'
  const stage = request.pendingStage === 2 ? 2 : request.status === 'Pending HR Approval' ? 2 : 1
  ...
  const supervisorPending = !supervisorDone && stage === 1
  const hrPending = supervisorDone && !hrDone && !rejectedAtSupervisor

  return {
    id: queue.id,
    requestNo: request.requestNo,
    requestType: EMPLOYEE_EXIT_MODULE,
    title: queue.title,
    status: queue.status,
    makerEmployeeNo: request.employeeNo,
    makerName: request.employeeName,
    ...
    responsibleCenter: '',
    amount: 0,
    sourceDocument: { documentNo: request.requestNo, erpEntity: request.requestTypeLabel },
    createdAt: request.submittedAt,
    submittedAt: request.submittedAt,
    approverEmployeeNo: queue.approverEmployeeNo,
    approverName: queue.approverEmployeeNo,
    auditTrail: [],
    approvalSteps: [
      {
        id: 'maker',
        ...
        note: 'Submitted from the self-service portal',
        sequenceNo: 0,
      },
      {
        id: 'supervisor',
        actorEmployeeNo: request.supervisorUserId || request.supervisorDecisionBy || '',
        actorName:
          request.supervisorDecisionBy || request.supervisorUserId || 'Immediate Supervisor',
        role: 'Checker' as const,
        status: exitStepStatus(supervisorDone, rejectedAtSupervisor, supervisorPending),
        timestamp: request.supervisorDecisionAt || request.submittedAt,
        note: rejectedAtSupervisor
          ? request.decisionRemarks
          : supervisorPending
            ? 'Awaiting Immediate Supervisor'
            : supervisorDone
      ...
      {
        id: 'hr',
        actorEmployeeNo: request.hrApproverUserId || request.hrDecisionBy || '',
        actorName: request.hrDecisionBy || request.hrApproverUserId || 'HR',
        role: 'HR' as const,
        status: rejectedAtSupervisor
          ? ('Draft' as const)
          : exitStepStatus(hrDone, rejectedAtHr, hrPending),
        timestamp: request.hrDecisionAt || request.supervisorDecisionAt || request.submittedAt,
        note: rejectedAtSupervisor
          ? 'Not reached'
          : hrPending
            ? 'Awaiting HR after supervisor approval'
            : rejectedAtHr
              ? request.decisionRemarks
              : hrDone
                ? 'Approved by HR'
    ...
    payload: {
      ...
      pendingStage: stage,
      pendingStageLabel: request.pendingStageLabel || (stage === 2 ? 'HR' : 'Immediate Supervisor'),
      bcStatus: request.status,
      reason: request.details?.reason || request.details?.resignationReason || '',
      detailFacts: employeeExitDetailFacts(request),
      lines: [],
    },
  }
}

export function employeeExitActingUserMayView(
  request: EmployeeExitRequest,
  actingUserIds: string[],
) {
  const identities = new Set(actingUserIds.map((id) => id.trim().toLowerCase()).filter(Boolean))
  const candidates = [
    request.supervisorUserId,
    request.hrApproverUserId,
    request.supervisorDecisionBy,
    request.hrDecisionBy,
  ]
  return candidates.some((id) => identities.has(String(id ?? '').trim().toLowerCase()))
}
