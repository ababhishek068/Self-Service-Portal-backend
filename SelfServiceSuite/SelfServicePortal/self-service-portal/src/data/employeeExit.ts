import { ArrowRightLeft, ClipboardCheck, UserMinus, type LucideIcon } from 'lucide-react'
...
export const EMPLOYEE_EXIT_TYPES: EmployeeExitTypeOption[] = [
  ...
  },
  {
    ...
    description:
      'Submit a formal resignation with your proposed last working date — the exit interview is included in the same form (optional).',
    icon: UserMinus,
  },
  {
    value: 'exit-interview',
    label: 'Employee Exit Form',
    shortLabel: 'Exit Form',
    description: 'Complete the employee exit interview form for direct submission to HR.',
    icon: ClipboardCheck,
  },
]
...
export interface EmployeeExitFieldConfig {
  name: string
  label: string
  type: 'text' | 'textarea' | 'date' | 'select' | 'email' | 'tel' | 'multiselect' | 'heading'
  placeholder?: string
  required?: boolean
...
/**
 * Reasons (a)–(o) from ABH Partners' Employee Exit material, plus "Other".
...
export const EMPLOYEE_EXIT_FIELDS: Record<EmployeeExitRequestType, EmployeeExitFieldConfig[]> = {
  transfer: [
    ...
    { name: 'desiredDepartment', label: 'Requested department / district', type: 'select', optionsSource: 'departments', placeholder: 'Select an existing Business Central department or district', required: true },
    { name: 'desiredLocation', label: 'Requested division / branch', type: 'select', optionsSource: 'divisions', placeholder: 'Select an existing Business Central division or branch', required: true },
  ...
  resignation: [
    ...
    // ── Exit Interview (merged into Resignation, 29-07-2026 per HB). Optional: the
    // employee may resign without completing the interview and it can be filled later. ──
    { name: 'exitInterviewHeading', label: 'Exit Interview (optional)', type: 'heading' },
    { name: 'supervisorName', label: 'Name of immediate supervisor at time of termination', type: 'select', optionsSource: 'employees', placeholder: 'Select your immediate supervisor' },
    { name: 'contractTerminationDate', label: 'Date contract is terminated', type: 'date', allowPast: true },
    { name: 'leavingReasons', label: 'Reason for leaving ABH Partners (select all that apply)', type: 'multiselect', options: reasonOptions },
    { name: 'joiningAnotherCompany', label: 'Are you joining another company?', type: 'select', placeholder: 'Select Yes or No', options: yesNoOptions },
    { name: 'startOwnBusiness', label: 'Are you leaving to start your own business?', type: 'select', placeholder: 'Select Yes or No', options: yesNoOptions },
    { name: 'otherPlans', label: 'If other plans, please indicate (optional)', type: 'textarea', placeholder: 'Any other plans after leaving ABH Partners' },
    { name: 'wouldReturn', label: 'Would you consider returning to ABH Partners in the future?', type: 'select', placeholder: 'Select an answer', options: [{ label: 'Yes', value: 'Yes' }, { label: 'No', value: 'No' }, { label: 'Maybe', value: 'Maybe' }] },
    { name: 'mostSatisfying', label: 'What did you find most satisfying during your stay in ABH Partners?', type: 'textarea', placeholder: 'Share what you valued most' },
    { name: 'mostFrustrating', label: 'What did you find most frustrating during your stay in ABH Partners?', type: 'textarea', placeholder: 'Share what frustrated you most' },
    ...
    { name: 'confidentialityAcknowledged', label: 'I confirm that the information provided is accurate and may be reviewed', type: 'select', placeholder: 'Select Yes to confirm', options: yesNoOptions },
  ],
  // Matches ABH Partners' Employee Exit SSP template exactly.
  'exit-interview': [
    ...
    { name: 'leavingReasons', label: 'Reason for leaving ABH Partners (select all that apply)', type: 'multiselect', required: true, options: reasonOptions },
    ...
    {
      name: 'wouldReturn',
      label: 'Would you consider returning to ABH Partners in the future?',
      type: 'select',
    ...
    { name: 'mostSatisfying', label: 'What did you find most satisfying during your stay in ABH Partners?', type: 'textarea', placeholder: 'Share what you valued most', required: true },
    { name: 'mostFrustrating', label: 'What did you find most frustrating during your stay in ABH Partners?', type: 'textarea', placeholder: 'Share what frustrated you most', required: true },
    { name: 'additionalComments', label: 'Additional comments (optional)', type: 'textarea', placeholder: 'Any other feedback you would like HR to consider' },
    {
