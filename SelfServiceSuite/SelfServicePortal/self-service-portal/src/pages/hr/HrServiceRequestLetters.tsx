...
function LetterProgress({ status }: { status: HrServiceLetterRequest['status'] }) {
  const steps = [
    ...
    { label: 'HR review', done: status !== 'Pending Approval' && status !== 'Cancelled' },
    {
      label: 'Letter ready',
...
export function HrServiceRequestLetters() {
  ...
  const listQuery = useQuery({
    queryKey: [...QUERY_KEY, employeeNo],
    queryFn: () => fetchHrServiceLetterRequests(employeeNo),
  ...
  const submitMutation = useMutation({
    ...
    onError: () => toast.error('Submission failed', 'Please complete all required fields and try again.'),
  })
  ...
  const resetMutation = useMutation({
    mutationFn: resetHrServiceLetterMockData,
    onSuccess: () => {
  ...
  const handleSubmit = (event: React.FormEvent) => {
    ...
    if (!validateForm() || !letterType || !employee) return
    submitMutation.mutate({
      letterType,
      employeeNo: employee.employeeNo,
      employeeName: employee.displayName,
      departmentName: employee.departmentName || employee.departmentCode,
      details,
    })
  }
  ...
  const renderField = (field: HrLetterFieldConfig) => {
    ...
    const common = {
      ...
      onChange: (event: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) =>
        updateDetail(field.name, event.target.value),
            ...
            <Button
              ...
              onClick={() => {
                void confirm({
                  ...
                  message: 'This resets the preview list to the sample HR letter requests.',
