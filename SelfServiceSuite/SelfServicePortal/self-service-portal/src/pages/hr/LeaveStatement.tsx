...
export function LeaveStatement() {
  ...
  const leaveTypeOptions = useMemo(() => {
    ...
    for (const type of leaveTypes) {
      options.set(type.code, type.description)
    }
    for (const row of leaveQuery.data ?? []) {
      const code = resolveTypeCode(row, leaveTypes)
      if (code) options.set(code, row.LeaveType || code)
    }
    return [...options.entries()].map(([value, label]) => ({ value, label }))
  }, [leaveTypes, leaveQuery.data])
  ...
  const balanceByType = useMemo(() => {
    ...
    typeCodes.forEach((code, index) => {
      ...
      map[code] = data ? resolveApplicableLeaveBalance(data) : 0
    })
    return map
    ...
    { id: 'status', header: 'Status', cell: (row) => <StatusBadge status={row.status} /> },
    {
      ...
      cell: (row) => (
        <Button
          ...
          variant="ghost"
          size="sm"
          onClick={() => navigate(`/hr/leave-request?application=${encodeURIComponent(row.id)}`)}
        >
  ...
  const selectedAnnualFigures =
    balanceQuery.data?.isAnnual ? resolveAnnualLeaveFigures(balanceQuery.data) : null
    ...
    try {
      ...
      const message =
        reason instanceof Error && reason.message
          ? reason.message
          : 'Leave statement could not be generated.'
      setDownloadError(message)
      toast.error(message, 'Leave statement failed')
  ...
  return (
    <PageWrapper title="Leave Statement" showPageHeading={false}>
      <PortalFormCard title="Leave Statement" allowOverflow>
        <div className="space-y-4">
          ...
          {leaveType && balanceQuery.data ? (
            balanceQuery.data.isAnnual && selectedAnnualFigures ? (
              <div className="space-y-2">
                <div className="grid gap-3 sm:grid-cols-3">
                  <BalanceFigure label="Leave Entitlement" value={selectedAnnualFigures.entitlement} />
                  <BalanceFigure label="Carry Forward" value={selectedAnnualFigures.carryForward} />
                  <BalanceFigure
                    label="Total Available Leave Balance"
                    value={selectedAnnualFigures.totalAvailable}
                  />
                  <BalanceFigure label="Leave Accrued To-Date" value={selectedAnnualFigures.accruedToDate} />
                  <BalanceFigure label="Total Leave Taken To-Date" value={selectedAnnualFigures.takenToDate} />
                  <BalanceFigure
                    label="Available Leave Balance"
                    value={selectedAnnualFigures.available}
                    emphasized
                  />
                </div>
              ...
              <div className="max-w-sm">
                <BalanceFigure
                  label="Available Leave Balance"
                  value={resolveApplicableLeaveBalance(balanceQuery.data)}
                  emphasized
                />
              </div>
            )
          ...
          <div className="flex flex-wrap justify-center gap-2">
            <Button type="button" disabled={downloading || !leaveType} onClick={() => void generate()}>
              <Download className="h-4 w-4" />
              {downloading ? 'Generating...' : 'Generate PDF'}
            </Button>
            <Button type="button" variant="outline" disabled={!rows.length} onClick={() => window.print()}>
          ...
          {downloadError ? (
            <p className="rounded-md border border-red-200 bg-red-50 px-3 py-2 text-center text-sm text-red-800" role="alert">
              {downloadError}
            </p>
          ) : null}
        </div>
      ...
      <div ref={printRef} className="leave-statement-print mt-6 print:p-4">
        <div className="mb-3 flex flex-wrap items-end justify-between gap-2">
          <h2 className="portal-page-title text-base font-semibold">
            Leave Statement {leaveType ? `— ${leaveTypeOptions.find((t) => t.value === leaveType)?.label ?? leaveType}` : ''}
          </h2>
          {selectedBalance !== undefined ? (
            <p className="text-sm text-slate-600">
              Available Leave Balance: <span className="font-semibold text-slate-900">{formatDays(selectedBalance)}</span>
            </p>
          ) : null}
        </div>
        <DataTable
