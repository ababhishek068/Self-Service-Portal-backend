Report 50010 "PR Ledger Posting"
{
    DefaultLayout = RDLC;
    ApplicationArea = All;
    ...
    dataset
    {
        ...
        {
            RequestFilterFields = "Transaction Type", "Transaction Code", "Group Code";
            ...
            column(Transaction_Charge_Code; "Transaction Charge Code") { }
            ...
            column(NhifTotal; NhifTotal) { }
            column(BasicPayTotal; BasicPayTotal) { }
            column(PAYEETotal; PAYEETotal) { }
            ...
            begin
                ...
                if PRPeriodTrans.FindFirst() then begin
                    ...
                    PRThirdPartyCharges.reset();
                    PRThirdPartyCharges.SetRange(Code, "PR Transaction Codes"."Transaction Charge Code");
                    if PRThirdPartyCharges.FindFirst() then begin
                        Commission := Cur_No_Assigned * PRThirdPartyCharges."Charge Per Transaction";
                        if PRThirdPartyCharges.code = 'INSURANCEPREMIUM' then begin
                            Commission := Amount * PRThirdPartyCharges."Charge Per Transaction";
                        end;

                        if PRThirdPartyCharges.code = 'HIREPURCHASECREDIT' then begin
                            Commission := Amount * PRThirdPartyCharges."Charge Per Transaction";
                        end;

                    end;

                    ChequeAmount := Amount - Commission;
                end;
            ...
            begin
                ...
                if PRPeriodTrans.Find('-') then begin
                    ...
                    PRPeriodTrans.CalcSums(PRPeriodTrans.Amount);
                    NetPayTotal += PRPeriodTrans.Amount;
                end;
                ...
                if PRPeriodTrans.FindFirst() then begin
                    NumberOfStaff := PRPeriodTrans.Count();
                    PRPeriodTrans.CalcSums(PRPeriodTrans.Amount);
                    NhifTotal += PRPeriodTrans.Amount;
                end;
                PRPeriodTrans.Reset;
                PRPeriodTrans.SetCurrentkey("Employee Code", "Transaction Code", "Payroll Period");
                ...
                if PRPeriodTrans.Find('-') then begin
                    NumberOfStaff := PRPeriodTrans.Count();
                    PRPeriodTrans.CalcSums(PRPeriodTrans.Amount);
                    PensionTotal += PRPeriodTrans.Amount;
                end;
                ...
                if PRPeriodTrans.Find('-') then begin
                    NumberOfStaff := PRPeriodTrans.Count();
                    PRPeriodTrans.CalcSums(PRPeriodTrans.Amount);
                    BasicPayTotal += PRPeriodTrans.Amount;
                end;
                PRPeriodTrans.Reset;
                PRPeriodTrans.SetCurrentkey("Employee Code", "Transaction Code", "Payroll Period");
                ...
                if PRPeriodTrans.Find('-') then begin
                    NumberOfStaff := PRPeriodTrans.Count();
                    PRPeriodTrans.CalcSums(PRPeriodTrans.Amount);
                    PAYEETotal += PRPeriodTrans.Amount;
                end;
    ...
    var
        ...
        Commission: Decimal;
        PRThirdPartyCharges: Record "PR Third Party Charges";
