Table 50387 "Job-Planning Line"
{
    Caption = 'Job Planning Line';
    // DrillDownPageID = "Admission Form Header Direct";
    //  LookupPageID = "Admission Form Header Direct";
    PasteIsValid = false;
    ...
    fields
    {
        ...
        {
            Caption = 'Type';
            OptionCaption = 'Resource,Item,G/L Account,Text,Fixed Asset';
            OptionMembers = Resource,Item,"G/L Account",Text,"Fixed Asset";
                ...
                case Type of
                    Type::Resource:
                        begin
                            ...
                            GetCostAllocation;
                            Quantity := 1;
                            //  "G/l Account":=Res.gl ."Resource G/L Budget Account";
                    ...
                    Type::"Fixed Asset":
                        begin
                            ...
                            FAPostingGroup.SetRange(FAPostingGroup.Code, FA."FA Posting Group");
                            if FAPostingGroup.Find('-') then begin
                                "G/l Account" := FAPostingGroup."Acquisition Cost Account";
                            end;
                            //IF Job."Language Code" <> '' THEN
                            //  GetItemTranslation;
                            "Gen. Prod. Posting Group" := FA."FA Posting Group";
                            //VALIDATE("Unit of Measure Code",FA."Base Unit of Measure");
                        end;
        ...
        field(39003921; "Budget Line Item"; Code[50])
        {
            CalcFormula = lookup("G/L Account"."Budget Line Item" where("No." = field("G/l Account")));
            FieldClass = FlowField;
        }
        field(39003922; "G/l Account"; Code[50])
        {
            FieldClass = Normal;
            TableRelation = if (Type = filter(= "G/L Account")) "G/L Account"."No." where("No." = field("No."))
            else
            if (Type = filter(= Item)) Item."Item G/L Budget Account" where("No." = field("No."));
        }
    ...
    var
        JobTask2: Record "Job-Planning Line";
    begin
