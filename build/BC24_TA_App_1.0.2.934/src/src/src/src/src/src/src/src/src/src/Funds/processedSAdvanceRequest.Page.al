page 51556 "Processed Staff Advance Req"
//51554
{
    DeleteAllowed = false;
    Editable=false;
    ...
    SourceTableView = WHERE(Posted = CONST(true),"Payroll processed" = const(true), "Send to payroll" = const(true),"Advance Closed"=const(true));
    ApplicationArea = All;
    ...
    layout
    {
        area(content)
        {
            group(Control1)
            {
                ...
                field("Staff ID"; "Staff ID") { Editable = false; }
                field("Send to payroll"; "Send to payroll") { Editable = false; }
                field("Payroll Period"; "Payroll Period") { Editable = false; }
                field("Payroll processed"; "Payroll processed") { Editable = false; }
                field("Date posted in Payroll"; "Date posted in Payroll") { Editable = false; }
                field("Date processed"; "Date processed") { Editable = false; }
                field("Date recovered"; "Date recovered") { Editable = false; }
                field(recovered; recovered) { Editable = false; }
            ...
            {
                SubPageLink = No = FIELD("No."), "Advance Type" = const('SALARY');
            }
        }
    }
