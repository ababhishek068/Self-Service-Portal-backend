page 50960 "Sales Invoice FC"
{
        ...
        area(content)
        {
            group(General)
            {
                ...
                group("Sell-to")
                {
                    ...
                    {
                        ShowCaption = false;
                        Visible = IsSellToRegionVisible;
                        field("Sell-to Region"; Rec."Sell-to county")
                        {
                            ApplicationArea = Basic, Suite;
                            Caption = 'Region';
                            Importance = Additional;
                            QuickEntry = false;
                            ToolTip = 'Specifies the state, province or Region of the address.';
                        }
                    }
                    ...
                    field("Sell-to Country/Region Code"; Rec."Sell-to Country/Region Code")
                    {
                        ...
                        trigger OnValidate()
                        begin
                            IsSellToRegionVisible := FormatAddress.UseCounty(Rec."Sell-to Country/Region Code");
                        end;
                    }
            ...
            group("Shipping and Billing")
            {
                ...
                group(Control34)
                {
                    ...
                    group(Control200)
                    {
                            ...
                            begin
                                ...
                                case ShipToOptions of
                                    ...
                                    ShipToOptions::"Alternate Shipping Address":
                                        begin
                                            ...
                                            if ShipToAddressList.RunModal = ACTION::LookupOK then begin
                                                ...
                                                IsShipToRegionVisible := FormatAddress.UseCounty(ShipToAddress."Country/Region Code");
                                            end else
                                    ...
                                    ShipToOptions::"Custom Address":
                                        begin
                                            Rec.Validate("Ship-to Code", '');
                                            IsShipToRegionVisible := FormatAddress.UseCounty(Rec."Ship-to Country/Region Code");
                                        end;
                                end;
                        ...
                        group(Control202)
                        {
                            ...
                            {
                                ShowCaption = false;
                                Visible = IsShipToRegionVisible;
                                field("Ship-to Region"; Rec."Ship-to county")
                                {
                                    ApplicationArea = Basic, Suite;
                                    Caption = 'Region';
                                    Editable = ShipToOptions = ShipToOptions::"Custom Address";
                ...
                group(Control203)
                {
                    ...
                    group(Control205)
                    {
                        ...
                        {
                            ShowCaption = false;
                            Visible = IsBillToRegionVisible;
                            field("Bill-to Region"; Rec."Bill-to county")
                            {
                                ApplicationArea = Basic, Suite;
                                Caption = 'Region';
                                Editable = (BillToOptions = BillToOptions::"Custom Address") OR (Rec."Bill-to Customer No." <> Rec."Sell-to Customer No.");
                        ...
                        field("Bill-to Country/Region Code"; Rec."Bill-to Country/Region Code")
                        {
                            ...
                            trigger OnValidate()
                            begin
                                IsBillToRegionVisible := FormatAddress.UseCounty(Rec."Bill-to Country/Region Code");
                            end;
                        }
    ...
    procedure PostPumpAttendanceClearance_Removed()
    var
    ...
    begin
        ...
        ForeCourtSetup.get;
        Attend.get(Rec."Sales Person");
        if (Attend."Sales Account No" <> Rec."Bill-to Customer No.") and (Rec."Invoice Cleared" = false) then begin
            ForeCourtSetup.TestField("Invoice Clearance Account");
            SLine.reset;
            SLine.setrange("Document No.", Rec."No.");
            if SLine.find('-') then begin
                repeat
                    Amt := Amt + SLine."Amount Including VAT";
                until SLine.Next = 0;
            end;
            GenJnl.Reset;
            ...
            GenJnl."Account No." := Attend."Sales Account No";
            GenJnl.Amount := Amt * -1;
            GenJnl.Validate(GenJnl."Account No.");
            GenJnl.Validate(GenJnl.Amount);
            GenJnl.Description := Rec."Posting Description";
            GenJnl."Bal. Account Type" := GenJnl."account type"::"G/L Account";
            GenJnl."Bal. Account No." := ForeCourtSetup."Invoice Clearance Account";
            // GenJnl.Validate(GenJnl."Bal. Account No.");
            // GenJnl."Bal. Gen. Posting Type" := 0;
            GenJnl."Shortcut Dimension 1 Code" := Rec."Shortcut Dimension 1 Code";
    ...
    procedure CheckLinesRequiredFields()
    var
        SLine: Record "Sales Line";
        PumpAttend: record "Salesperson/Purchaser";
    begin
        if PumpAttend.get(Rec."Sales Person") then begin
            if PumpAttend."Sales Account No" <> Rec."Bill-to Customer No." then begin
                SLine.reset;
                SLine.setrange("Document No.", Rec."No.");
                if SLine.find('-') then begin
                    repeat
                        if Sline."No." <> '' then begin
                            Sline.TestField("Driver No");
                            Sline.TestField("Truck No");
                        end;
                    until sline.next = 0;
                end;
    ...
    var
        ...
        IsSaaS: Boolean;
        IsBillToRegionVisible: Boolean;
        IsSellToRegionVisible: Boolean;
        IsShipToRegionVisible: Boolean;
