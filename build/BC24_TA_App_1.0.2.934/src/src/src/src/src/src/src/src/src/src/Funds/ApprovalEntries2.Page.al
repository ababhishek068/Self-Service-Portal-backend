page 51394 "Approval Entries2"
{
    ...
    begin
        AppEntry.reset;
        AppEntry.SetRange("Approver ID", Database.userid);
        // AppEntry.SetFilter(Description, '');
            ...
            repeat
                if PayHeader.get(AppEntry."Document No.") then begin
                    ...
                    if HR.get(PayHeader."Employee No") then
                        if HR."User ID" <> '' then
                            Rec."Sender ID" := HR."User ID";
                end;
                if ImpHeader.get(Rec."Document No.") then begin
                    AppEntry.Description := ImpHeader.Payee;
    ...
    procedure SetRecordFilters(TableId: Integer; DocumentType: Option Quote,"Order",Invoice,"Credit Memo","Blanket Order","Return Order"; DocumentNo: Code[20])
    begin
