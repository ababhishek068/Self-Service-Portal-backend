Page 51393 "Active-Visitor Management Card"
{
    PageType = Card;
    Editable = false;
    InsertAllowed = false;
        ...
        area(creation)
        {
            action(clear)
            {
                ...
                begin
                    ...
                    if Confirm('Clear Visitor?', true) = false then Error('Cancelled by user: ' + UserId);
            ...
            action("Report Incident")
            {
                ...
                trigger OnAction()
                var
                    Incidence: Record "Sec-Visitor Management";
                begin
                    Rec.TestField("Visitor Name");
                    Rec.TestField("ID Number");
                    Rec.TestField("Phone Number");
                    ...
                    if Confirm('Report An Incident Occurance?', true) = false then Error('Cancelled by user: ' + UserId);
                    Incidence.Reset();
                    Incidence.SetRange("Visit No.", Rec.No);
                    //if Incidence.Find('-') then
                    PAGE.RUN(Page::"Incident Details", Incidence);
                    //else
                    //PAGE.RUN(Page::"Incident Details", Incidence);
                end;
            }
