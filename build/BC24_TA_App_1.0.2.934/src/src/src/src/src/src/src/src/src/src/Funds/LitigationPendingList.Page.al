Page 50962 "Litigation Pending List"
{
    CardPageID = "Litigation Card";
    Editable = false;
    PageType = List;
    SourceTable = "Legal Management";
    SourceTableView = where("Litigation Status" = filter(Created),
                            "Send to Litigation" = const(true), Type = filter(Letigation), Status = filter(<> Approved));
