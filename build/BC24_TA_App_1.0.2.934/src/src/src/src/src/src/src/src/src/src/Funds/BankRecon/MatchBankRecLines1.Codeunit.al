Codeunit 50004 "Match Bank Rec. Lines1"
{
    ...
    var
        ...
        BankRecMatchCandidates: Query "Bank Rec. Match Candidates1";
        Window: Dialog;
        Score: Integer;
    begin
        ...
        if BankRecMatchCandidates.Open then
            while BankRecMatchCandidates.Read do begin
                ...
                IF (BankRecMatchCandidates.Rec_Line_Transaction_Date = BankRecMatchCandidates.Posting_Date) AND
                (BankRecMatchCandidates.Rec_Line_Statement_Amount = BankRecMatchCandidates.Remaining_Amount) THEN
                    Score += 13;
                //    
                IF BankRecMatchCandidates.Rec_Line_Document_no = BankRecMatchCandidates.Document_No THEN Score += 13;



                // IF BankRecMatchCandidates.Rec_Line_Statement_Amount <> BankRecMatchCandidates.Remaining_Amount THEN Score := 0
    ...
    var
        BankAccountLedgerEntry: Record "Bank Account Ledger Entry";
        BankAccReconciliationLine: Record "Bank Acc. Reconciliation Line";
        BankAccEntrySetReconNo: Codeunit "Bank Acc. Entry Set Recon.-No1";
    ...
    begin
        if SelectedBankAccReconciliationLine.FindFirst then begin
            if BankAccReconciliationLine.Get(
              SelectedBankAccReconciliationLine."Statement Type",
              SelectedBankAccReconciliationLine."Bank Account No.",
              SelectedBankAccReconciliationLine."Statement No.",
              SelectedBankAccReconciliationLine."Statement Line No.") then
                if BankAccReconciliationLine.Type <> BankAccReconciliationLine.Type::"Bank Account Ledger Entry" then
                    exit;
    ...
    procedure MatchSingle2Backup(BankAccReconciliation: Record "Bank Acc. Reconciliation"; DateRange: Integer)
    var
        TempBankStatementMatchingBuffer: Record "Bank Statement Matching Buffer" temporary;
        BankRecMatchCandidates: Query "Bank Rec. Match Candidates1";
        Window: Dialog;
    ...
    begin
        ...
        Dateset := BankAccReconciliation."Statement Date" + DateRange;//added Mutegi
        ...
        BankRecMatchCandidates.SetRange(Rec_Line_Bank_Account_No, BankAccReconciliation."Bank Account No.");
        BankRecMatchCandidates.SetRange(Rec_Line_Statement_No, BankAccReconciliation."Statement No.");
        BankRecMatchCandidates.SetFilter(Rec_Line_Transaction_Date, '..%2', Dateset);
        if BankRecMatchCandidates.Open then
            while BankRecMatchCandidates.Read do begin
                Score := 0;
                if BankRecMatchCandidates.Rec_Line_Statement_Amount = BankRecMatchCandidates.Remaining_Amount then
                    Score += 13;
                ...
                if BankRecMatchCandidates.Rec_Line_Statement_Amount <> BankRecMatchCandidates.Remaining_Amount then    //changes if check no is same, but amount is diff.
                    Score := 0;
                if BankRecMatchCandidates.Rec_Line_Check_No <> BankRecMatchCandidates.External_Document_No then
                    Score := 0;
                ...
                //added -- Mutegi
                if BankRecMatchCandidates.Bank_GL_Count = BankRecMatchCandidates.External_Document_No then begin
                    //IF BankRecMatchCandidates.Rec_Line_Document_no = BankRecMatchCandidates.External_Document_No THEN
                    Score += 12;
                end;

                if Score > 2 then
                    TempBankStatementMatchingBuffer.AddMatchCandidate(BankRecMatchCandidates.Rec_Line_Statement_Line_No, BankRecMatchCandidates.Entry_No,
                      Score, BankRecMatchCandidates.Bal_Account_Type, BankRecMatchCandidates.Bal_Account_No);
