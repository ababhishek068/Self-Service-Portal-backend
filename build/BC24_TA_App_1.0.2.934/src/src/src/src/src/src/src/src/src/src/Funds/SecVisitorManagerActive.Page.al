Page 50757 "Sec-Visitor Manager (Active)"
{
    CardPageID = "Active-Visitor Management Card";
    Editable = false;
    InsertAllowed = false;
    DeleteAllowed = false;
    PageType = List;
    SourceTable = "Sec-Visitor Management";
    SourceTableView = where(Status = filter(Entered), "Incident Reported" = filter(false));
    ApplicationArea = All;
