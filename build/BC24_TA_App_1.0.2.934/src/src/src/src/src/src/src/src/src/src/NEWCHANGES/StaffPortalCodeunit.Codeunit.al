codeunit 50049 "Staff Portal Codeunit"
{
        ...
        if action = 'create' then begin
            ...
            if (action = 'delete') or (action = 'cancel') then begin
                ...
                if TbHRLeaveRequisition.Status = TbHRLeaveRequisition.Status::"Pending Approval" then begin
                    FnPortalCancelPendingLeave(TbHRLeaveRequisition);
                    return_value := 'true';
                end else
    ...
    local procedure FnPortalCancelPendingLeave(var LeaveApp: Record "HR Leave Application")
    begin
        if TryCancelLeaveApprovalWorkflow(LeaveApp) then begin
        end;

        if not LeaveApp.Get(LeaveApp."Application Code") then
            exit;

        FnDeleteLeaveApprovalEntries(LeaveApp);
    ...
    [TryFunction]
    local procedure TryCancelLeaveApprovalWorkflow(LeaveApplication: Record "HR Leave Application")
    begin
        VarVariant := LeaveApplication;
        CuCustomApprovals.OnCancelDocApprovalRequest(VarVariant);
    end;

    local procedure FnDeleteLeaveApprovalEntries(LeaveApp: Record "HR Leave Application")
    var
        AppEntry: Record "Approval Entry";
    ...
    begin
        ...
        LeaveRecordId := LeaveRecRef.RecordId;

        AppEntry.Reset();
        AppEntry.SetRange("Record ID to Approve", LeaveRecordId);
        if not AppEntry.IsEmpty then
            AppEntry.DeleteAll(false);

        AppEntry.Reset();
        AppEntry.SetRange("Document No.", LeaveApp."Application Code");
        AppEntry.SetRange("Table ID", Database::"HR Leave Application");
        if not AppEntry.IsEmpty then
            AppEntry.DeleteAll(false);
    end;
