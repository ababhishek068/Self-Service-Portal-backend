Report 50096 "Consolidated Marksheet A4"
{
    DefaultLayout = RDLC;
    ApplicationArea = All;
    ...
    dataset
    {
        ...
        {
            DataItemTableView = sorting("Student No.") order(ascending) where(Reversed = const(false), "Cust Exist" = filter(> 0), "Graduated List Count" = filter(0), Status = filter(Current | Registration | Suspended), "Units Taken" = filter(> 0));
            RequestFilterFields = "Programme Filter", "Stage Filter", "Semester Filter", "Academic Year", "Options Filter", "Student No.", Semester;
            ...
            column(CummUnits; CummUnits) { }
            column(CummGPA; CummGPA) { }
            column(SemGPA; SemGPA) { }
            ...
            begin
                ...
                //IF (COPYSTR("Course Registration"."Student No.",STRLEN("Course Registration"."Student No.")-1,2)<'16') OR (Prog."School Code"<>'SASS')
                ...
                for K := 1 to K_Max do begin
                    ...
                    StudUnits.setfilter(StudUnits.Semester, "Course Registration".getfilter("Semester Filter"));
                    if StudUnits.Find('-') then begin
                        ...
                        if StudUnits."Total Score" = 0 then
                            ...
                            if StudUnits."Total Score" = 0 then ColumnV[K] := 'X';
                ...
                if DSummary = false then begin
                    ...
                    */
                    ...
                    "Course Registration".CalcFields("Course Registration"."Cum Units Deffered");
                    "Course Registration".CalcFields("Course Registration"."Cum Units Deffered Cat");
                    "Course Registration".CalcFields("Course Registration"."Cum Units Deffered Exam");

                    i := i + 1;
                    ColumnH[i] := 'Total Credits';
                    //ColumnV[i]:=FORMAT(UTaken);
                    ColumnV[i] := Format("Course Registration"."CF Count");
                    //IF ("Course Registration"."CF Count">MinUnits) AND (MinUnits>0) THEN
                    ...
                    //IF ("Course Registration"."Cum Units Done">MinCourses) AND (MinCourses>0) THEN
                    //ColumnV[i]:=FORMAT(MinCourses);
                    ...
                    SemGPA := 0;
                    if ("Course Registration"."CF Total Score" > 0) and ("Course Registration"."CF Count" > 0) then begin
                        SemGPA := GetGPA((ROUND("Course Registration"."CF Total Score" / "Course Registration"."CF Count", 0.01, '>')), "Course Registration".Programme, '');
                    end;
                    CummUnits := 0;
                    CummGPA := 0;
                    Creg.reset;
                    creg.setrange("Student No.", "Course Registration"."Student No.");
                    if Creg.find('-') then begin
                        creg.CalcFields("CF Total Score");
                        creg.CalcFields("CF Count");
                        CummUnits := creg."CF Count";
                        if (Creg."CF Total Score" > 0) and (Creg."CF Count" > 0) then begin
                            CummGPA := GetGPA((ROUND(Creg."CF Total Score" / Creg."CF Count", 0.01, '>')), "Course Registration".Programme, '');
                        end;
                    end;
    ...
    var
        ...
        UnitShow2: array[200] of Decimal;
        K: Integer;
        mFound: Boolean;
        K_Max: Integer;
        SemFilter: Text[200];
    ...
    procedure GetGPA(Marks: Decimal; ProG: Code[20]; UnitG: Code[20]) xGPA: Decimal
    begin
        GradeCategory := '';
        UnitsRR.Reset;
        UnitsRR.SetRange(UnitsRR."Programme Code", "Course Registration".Programme);
