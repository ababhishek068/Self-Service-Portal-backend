Report 50100 "Consolidated Grades"
{
    DefaultLayout = RDLC;
    RDLCLayout = './Layouts/ConsolidatedMarksheetA4.rdlc';
    ApplicationArea = All;
    ...
    dataset
    {
        ...
        {
            DataItemTableView = sorting("Student No.") order(ascending) where(Reversed = const(false), "Cust Exist" = filter(> 0), "Graduated List Count" = filter(0), Status = filter(Current | Registration | Suspended));
            RequestFilterFields = "Programme", Semester, Stage, "Campus Filter", "Student No.";
            ...
            column(GLabel21; GLabel2[1]) { }
            column(GLabel22; GLabel2[2]) { }
            column(GLabel23; GLabel2[3]) { }
            column(GLabel24; GLabel2[4]) { }
            column(GLabel25; GLabel2[5]) { }
            column(GLabel26; GLabel2[6]) { }
            column(GLabel27; GLabel2[7]) { }
            column(GCount1; GCount[1]) { }
            column(GCount2; GCount[2]) { }
            column(GCount3; GCount[3]) { }
            column(GCount4; GCount[4]) { }
            column(GCount5; GCount[5]) { }
            column(GCount6; GCount[6]) { }
            column(GCount7; GCount[7]) { }
            column(Programme_Category; "Programme Category") { }
            column(School_Filter; "School Filter") { }
            ...
            begin
                ...
                if Prog.Get("Course Registration".Programme) then begin
                    ...
                    Gradings.setfilter(Gradings."Semester Filter", "Course Registration".GetFilter(Semester));
                    Gradings.setfilter(Gradings."Programme Filter", "Course Registration".GetFilter("Programme"));
                    Gradings.setfilter(Gradings."Stage Filter", "Course Registration".GetFilter("Stage"));
                    if Gradings.Find('-') then begin
                        repeat
                            Gradings.CalcFields("Semester Grade Count");
                            Gradings.CalcFields("Class Grade Count");
                            GLabel2[i] := Gradings.Range;
                            GLabel[i] := Gradings.Grade;
                            GCount[i] := Gradings."Class Grade Count";
                            i := i + 1;
                ...
                SDesc := "Course Registration".GetFilter("Course Registration"."Semester");
                ...
                if DSummary = false then begin
                    ...
                    "Course Registration".CalcFields("Course Registration"."Semester GPA");
                    "Course Registration".CalcFields("Course Registration"."Semester Units");
                    "Course Registration".CalcFields("Course Registration"."Programme GPA Points");
                    "Course Registration".CalcFields("Course Registration"."Student CF");
                    "Course Registration".CalcFields("Course Registration"."Semester CF");
                    ...
                    "Course Registration".CalcFields("Course Registration"."Semester GPA");
                    "Course Registration".CalcFields("Course Registration"."Semester GPA Points");
                    "Course Registration".CalcFields("Course Registration"."Student Semester CF");


                    i := i + 1;
                    ColumnH[i] := 'Sem Credit';
                    ColumnV[i] := Format("Course Registration"."Student Semester CF");

                    i := i + 1;
                    ColumnH[i] := 'Sem GPA';
                    if ("Course Registration"."Semester GPA Points" > 0) and ("Course Registration"."Student Semester CF" > 0) then
                        ColumnV[i] := Format(Round(("Course Registration"."Semester GPA Points" / "Course Registration"."Student Semester CF"), 0.05, '>'));

                    i := i + 1;
                    ColumnH[i] := 'Cumm Credit';
                    ColumnV[i] := Format("Course Registration"."Student CF");

                    i := i + 1;
                    ColumnH[i] := 'Cumm GPA';
                    if ("Course Registration"."Programme GPA Points" > 0) and ("Course Registration"."Student CF" > 0) then
                        ColumnV[i] := Format(Round(("Course Registration"."Programme GPA Points" / "Course Registration"."Student CF"), 0.05, '>'));

                    i := i + 1;
                    ColumnH[i] := 'Cumm Grade';
                    if ("Course Registration"."CF Total Score" > 0) and ("Course Registration"."CF Count" > 0) then begin
                        ColumnV[i] := GetGrade(Round(("Course Registration"."CF Total Score" / "Course Registration"."CF Count"), 0.05, '>'), "Course Registration".Programme, '');
                        "Course Registration"."Exam Grade" := ColumnV[i];
                    ...
                    /*  i := i + 1;
                     ...
                     if ColumnV[i] = 'PASS' then begin
                         if "Course Registration"."Cum Units Passed" < MinCourses then
  ...
  */
            ...
            begin
                ...
                if ProgOptions.Find('-') then begin
                    if ProgOptions."Minmum Pass All" > 0 then MinCourses := ProgOptions."Minmum Pass All";
                ...
                if Prog.Get(GetFilter("Programme Filter")) then begin
                    PName := Prog.Description;
                    FDesc := Prog."School Code";
                    if MinCourses = 0 then MinCourses := Prog."Min No. of Courses";
                    if MinUnits = 0 then MinUnits := Prog."Minimum Units Per Year";
                                       ...
                                       // StudUnits.SetFilter(StudUnits."Creg Exists", '>%1', 0);
                                        ...
                                        if StudUnits.Find('-') then begin
                                            repeat
                                                ...
                                                if (mFound = false) and (i < 70) then begin
                                                    i := i + 1;
    ...
    var
        ...
        GLabel: array[60] of Code[20];
        GLabel2: array[60] of Code[100];
        GCount: array[60] of Integer;
        ...
        // UnitsBuffer: Record "Units Buffer";
        ShowRegUnits: Boolean;
        FailedUnits: Text[250];
        UnitShow: array[200] of Code[200];
        K: Integer;
        K_Max: Integer;
    ...
    begin
        ...
        if YearsDone = '1' then begin
            ...
            if Stages.Find('-') then begin
                ...
                CReg.SetFilter("Stage Filter", 'Y1S1..Y1S2');
                if CReg.Find('-') then begin
