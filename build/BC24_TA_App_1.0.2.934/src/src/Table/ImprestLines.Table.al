Table 50898 "Imprest Lines"
{
    ...
    fields
    {
        ...
        {
            TableRelation = if ("Imprest Type" = filter(ItemCash)) Item."No."
            else
            if ("Imprest Type" = filter(Imprest)) "G/L Account"."No.";

            trigger OnValidate()
            begin
                TestField(Committed, false);
                if "Imprest Type" = "imprest type"::Imprest then begin
        ...
        {
            TableRelation = "Receipts and Payment Types".Code where(Type = const(Imprest), Blocked = const(false));
    ...
    begin
        ...
        //Get the Emp No
        CustomerRec.Reset;
        CustomerRec.SetRange(CustomerRec."No.", CustNo);
        if CustomerRec.Find('-') then begin
            EmpNo := CustomerRec."Staff No."
        end;

        // get the grade
        HREmployee.Reset;
        HREmployee.SetRange(HREmployee."No.", EmpNo);
        if HREmployee.Find('-') then begin
            EmpGrade := HREmployee."Job Group";
        end;
        ...
        IF objDestRateEntry.FIND('-') THEN BEGIN
            "Currency Code" := objDestRateEntry.Currency;
