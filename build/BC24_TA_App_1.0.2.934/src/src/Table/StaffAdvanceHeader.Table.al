Table 50880 "Staff Advance Header"
{
    ...
    fields
    {
        ...
        field(29; "Advance Closed"; Boolean) { Editable = false; }
        ...
        field(35; Status; Option)
        {
            ...
            begin
                ...
                end else if Rec.status = Rec.Status::Approved then begin
                    hremps.Reset();
                    hremps.SetRange(hremps."No.", rec."Staff ID");
                    hremps.SetRange(hremps.Status, hremps.Status::Active);
                    if hremps.FindFirst() then begin
                        hremps."Process Advance" := true;
                        hremps.Modify();
        ...
        field(40; "Payroll Period"; date)
        {

        }
        field(41; "Staff ID"; Code[20])
        {
            ...
            begin
                HREmployee.Reset();
                HREmployee.SetRange("No.", "Staff ID");
                if HREmployee.FindFirst() then begin
                    "Customer No" := HREmployee."Customer No";
                    "Account No." := HREmployee."Customer No";
                    Validate("Account No.");
                end;
            end;
        }
        field(43; "Customer No"; code[20])
        {
            TableRelation = Customer."No." where("Staff No." = field("Staff ID"));
        }
        ...
        field(87; "Account No."; Code[20])
        {
            ...
            begin
                ...
                if Cust.Get("Account No.") then begin
                    ...
                    if (Cust."Credit Limit (LCY)" <> 0) and (Cust."Balance (LCY)" > Cust."Credit Limit (LCY)") then
        ...
        field(91; "Send to payroll"; Boolean)
        {

        }
        field(92; "Payroll processed"; Boolean) { }
        field(93; "Payroll posted"; Boolean) { }
        field(94; "Date processed"; Date) { }
        field(95; "Date posted in Payroll"; date) { }
        field(96; recovered; Boolean) { }
        field(97; "Date recovered"; date) { }
    }
    ...
    begin
        ...
        if "No." = '' then begin
            ...
            if "Payment Type" = "payment type"::Imprest then begin
                ...
                "No." := NoSeriesMgt.GetNextNo(GenLedgerSetup."Other Staff Advance No", 0D, true);
            end
        end;
        if GuiAllowed then begin
            hremps.Reset();
            hremps.SetRange(hremps."User ID", UserId);
            if hremps.Find('-') then begin
                "Staff ID" := hremps."No.";

                Cust.Reset();
                cust.SetRange(cust."Staff No.", hremps."No.");
                if Cust.Find('-') then begin
                    ...
                    Error('1.You are not allowed to access this service, contact HR');
                end;

            end else begin
                Error('2. You have not been allowed to access this service, contact HR');
            end;
        end;
        ...
        if prperiod.FindFirst() then begin
            if prperiod."Date Opened" <> 0D then begin
                "Payroll Period" := prperiod."Date Opened";

            end else begin
                Error('There is no payroll period open, please contact HR');
