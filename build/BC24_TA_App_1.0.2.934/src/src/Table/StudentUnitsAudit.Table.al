Table 50090 "Student Units Audit"
{
    DrillDownPageID = "Student Units Audit";
    LookupPageID = "Student Units Audit";
    ...
    fields
    {
            ...
            begin
                ...
                if UnitsS.Find('-') then begin
                    ...
                    if UnitsRec.find('-') then begin
                        "No. Of Units" := UnitsRec."No. Units";
                        Description := UnitsRec.Desription;
                        "No. Of Units" := UnitsRec."No. Units";
                        "Unit Stage" := UnitsRec."Stage Code";
                    end;
                end;
        ...
        field(181; "Concentration"; code[20])
        {
        ...
        field(180; "Type"; option)
        {
            OptionMembers = MAJOR,MINOR,CONCEN;
            DataClassification = ToBeClassified;
        }
        ...
        field(96; "Semester Completed"; code[20])
        {
        ...
        {
            FieldClass = FlowField;
            CalcFormula = Count("Student Units" WHERE("Student No." = FIELD("Student No."), Unit = FIELD(Unit), "Re-Taken" = CONST(true)));
        ...
        field(50193; "Progress Status"; Option)
        {
            OptionCaption = 'Future,Registered,Completed,Unfulfilled';
            OptionMembers = Future,Registered,Completed,Unfulfilled;
        }
        ...
        field(50197; "Date Registered"; date)
        {
            CalcFormula = lookup("Student Units"."Date created" WHERE(Unit = FIELD(Unit), "Student No." = field("Student No.")));
            FieldClass = FlowField;
        }
        field(50198; "Semester Registered"; code[20])
        {
            CalcFormula = lookup("Student Units".Semester WHERE(Unit = FIELD(Unit), "Student No." = field("Student No."), "Supp Taken" = filter(false)));
            FieldClass = FlowField;
        }
        field(50199; "Enrolled Programme"; code[20])
        {
            TableRelation = Programme.Code;
        }
        field(50200; "Waived Credits"; Decimal)
        {
            trigger OnValidate()
            begin
                "No. Of Units" := "No. Of Units" - "Waived Credits";
            end;
        }
        field(50201; "Substituted"; Boolean)
        {
            trigger OnValidate()
            begin
                TestField("Substitution Unit");
                "No. Of Units" := 0;
            end;
        }
        field(50202; "Substitution Unit"; code[20])
        {
            TableRelation = "Courses Master".Code;
        }
        field(50203; "Concentration Name"; text[200]) { }
        field(50204; "Unit Category Name"; text[200]) { }
        field(50205; "Unit Category Code"; code[20])
        {
            TableRelation = "Unit Category".code;
        }
        field(50206; "Unit Category Order"; Integer)
        {
            FieldClass = FlowField;
            CalcFormula = lookup("Unit Category"."Report Order" where("Unit Type" = field("Unit Type")));
        }
        field(50207; "Earned Credits"; Decimal) { }
