...
page 51543 "Limited tender Card"
{
    ApplicationArea = All;
    Caption = 'Tender - Card -Limited';
    PageType = Card;
    ...
    layout
    {
        area(content)
        {
            ...
            part(Control1102755015; "Tender-SubForm")
            {
            ...
            part(Control14; "Bidders List")
            {
                Caption = 'Bidders List';
                SubPageLink = "Tender ID" = field("No.");
            }
    ...
    actions
    {
        area(Navigation)
        {
            action("Required Documents")
            {
                ...
                Image = Vendor;
                Promoted = true;
                RunObject = page "Tender Required Documnets";
        ...
        area(processing)
        {
            ...
            {
                ApplicationArea = Basic;
                Caption = 'Assign Bidder(s)';
                Image = Vendor;
                Promoted = true;
                PromotedCategory = Process;
                ToolTip = 'Executes the Assign Bidder(s) action.';
                trigger OnAction()
                var
                ...
                begin
                    ...
                    Window.OPEN('Assigning Bidders. Please Wait');
                    ...
                    IF VendCats.FIND('-') THEN BEGIN
                        ...
                        FOR k := 1 TO CountLimit DO BEGIN
                            ...
                            IF NOT Vends.FIND('-') THEN BEGIN
                                Vends.INIT;
                                Vends."Document Type" := Vends."Document Type"::"Open Tender";
            ...
            action("Evaluation Commitee")
            {
                ...
                begin
                    ...
                    TenderCommittee.RESET;
                    TenderCommittee.SETFILTER("Tendor No", Rec."No.");
                    TenderCommittee.SETFILTER("Committee Type", 'Tender Evaluation Committee');
                    IF TenderCommittee.FIND('-') THEN BEGIN
                        PAGE.RUN(Page::"Tender Committee", TenderCommittee)
                    END;
                end;
                ...
                begin
                    ...
                    IF QuotationRequestVendors.FINDSET THEN BEGIN
                        REPEAT
                            ...
                            PurchaseHeader."Purchase Requisition No." := Rec."Internal Requisition No.";
                            PurchaseHeader."Tendor Number" := Rec."No.";
                            PurchaseHeader."Posting Description" := Rec."Posting Description";
                            PurchaseHeader."No." := '';
                            ...
                            IF PurchaseQuoteLine.FINDSET() THEN
                                REPEAT
                                    ...
                                    PurchaseLines."Buy-from Vendor No." := QuotationRequestVendors."Vendor No.";
            ...
            action(PlacedQuotes)
            {
                ...
                begin
                    ...
                    IF Rec."Opening Committee" = TRUE THEN BEGIN
                        ...
                        TenderCommittee.SETRANGE("Committee Type", TenderCommittee."Committee Type"::"Tender Opening Committee");
                        IF TenderCommittee.FIND('-') THEN BEGIN
                            //run the authentication page
                            PAGE.RUN(page::"Tender Passwords Page", TenderCommittee);
                        END ELSE
                            ERROR('The Evaluation Committee for this Tender has not been set');
                    END;
                    OpenBids();
                    //END;
                end;
            ...
            action(BidAnalysis)
            {
                ...
                begin
                    ...
                    IF Rec."Evaluation Committee" = TRUE THEN BEGIN
                        TenderCommittee.RESET;
                        TenderCommittee.SETRANGE("Tendor No", Rec."No.");
                        TenderCommittee.SETRANGE("Committee Type", TenderCommittee."Committee Type"::"Tender Evaluation Committee");
                        IF TenderCommittee.FIND('-') THEN BEGIN
                            //run the authentication page
            ...
            action("Release")
            {
                ...
                begin
                    ...
                    QuotationRequestVendors.SETRANGE(QuotationRequestVendors."Requisition Document No.", Rec."No.");
                    IF not QuotationRequestVendors.find('-') THEN ERROR('No vendors assigned for this Tender');
...
*/
    ...
    begin
        ...
        Rec."Document Type" := Rec."Document Type"::"Restricted Tender";
    end;
    ...
    begin
        ...
        BidAnalysis.SETRANGE(BidAnalysis."Tender No.", Rec."No.");
        BidAnalysis.DELETEALL;


        //insert the quotes from rfq
        PurchaseHeader.reset;
        PurchaseHeader.SETRANGE(PurchaseHeader."Tendor Number", Rec."No.");
        if PurchaseHeader.FIND('-') then begin
            REPEAT
                ...
                IF PurchaseLines.FINDSET THEN
                    REPEAT
                        BidAnalysis.INIT;
                        BidAnalysis."Tender No." := Rec."No.";
                        BidAnalysis."Tender Line No." := PurchaseLines."Line No.";
