Page 50103 "Student Hostel Rooms"
{
    PageType = List;
        ...
        area(content)
        {
            repeater(Control1102760000)
            {
                ...
                field("Student Gender"; Rec."Student Gender")
                {
                    ApplicationArea = Basic;
                    Editable = false;
                    ToolTip = 'Specifies the value of the Student Gender field.';
                }
                ...
                group(Function_Buttons)
                {
                    ...
                    action(Allocate)
                    {
                        ...
                        begin
                            ...
                            if Cust.Get(Rec.Student) then begin
                                Cust.CalcFields(Cust.Balance);
                                if Rec.Amount = 0 then Error('Please specify the Room Amount');
                                    ...
                                    if CReg."Total Billed" <> 0 then begin  // 1
                                        ...
                                        if Cust.Find('-') then
                                            if Cust."Hostel Black Listed" = false then begin
                                                ...
                                                PaidAmt1 := 0;
                                                StudCharges.Reset;
                                                StudCharges.SetRange("Student No.", Rec.Student);
                                                StudCharges.SetRange(Recognized, true);
                                                //StudCharges.SETRANGE(Semester,Semester);
                                                StudCharges.SetRange(Code, 'ACCOMODATION');
                                                StudCharges.SetFilter("Reversed LK", '%1', false);
                                                if StudCharges.Find('-') then
                                                    PaidAmt1 := StudCharges.Amount;

                                                if (Cust.Balance + Rec.Amount) - PaidAmt1 > 0 then Error('Insufficient payments');
                                                ...
                                                if Creg1.Find('-') then begin
                                                    ...
                                                    if prog.Get(Creg1.Programme) then begin
                                                        if prog."Special Programme" then
                                                            ...
                                                            if Creg1."Settlement Type" = 'GSSP' then
                    ...
                    {
                        ApplicationArea = Basic;
                        Caption = 'Allocate Room With Cafe';
                        Image = "Action";
                        Promoted = true;
                        PromotedCategory = New;
                        PromotedIsBig = true;
                        Visible = true;
                        ToolTip = 'Executes the Allocate Room With Cafe action.';
                        ...
                        begin
                            if Confirm('Allocate the Specified Room?', true) = false then Error('Cancelled by user!');
                            HostelAlloc.Reset;
                            HostelAlloc.SetRange(Student, Rec.Student);
                            HostelAlloc.SetRange(Semester, Rec.Semester);
                            HostelAlloc.SetRange(Cleared, false);
                            if HostelAlloc.Find('-') then Error('Please note that you have already booked/reserved room in the current semester');

                            Rooms_Spaces.Reset;
                            Rooms_Spaces.SetRange(Rooms_Spaces."Space Code", Rec."Space No");
                            Rooms_Spaces.SetRange(Rooms_Spaces."Hostel Code", Rec."Hostel No");
                            Rooms_Spaces.SetRange(Rooms_Spaces."Room Code", Rec."Room No");
                            Rooms_Spaces.SetRange(Rooms_Spaces.Status, Rooms_Spaces.Status::Vaccant);
                            if Rooms_Spaces.Find('-') then begin
                                HostLedger.Reset;
                                HostLedger.SetRange("Student No", Rec.Student);
                                ...
                                if Hostel_Rooms.Find('-') then begin
                                    ...
                                    Hostel_Rooms.Validate(Status);
                                    Hostel_Rooms.Modify;
                                end;

                                HostelAlloc.Init;
                                HostelAlloc.Student := Rec.Student;
                                HostelAlloc."Accomodation Fee" := Rec.Charges;
                                HostelAlloc.Semester := Rec.Semester;
                                HostelAlloc.Charges := Rec.Charges;
                                HostelAlloc.Gender := Cust.Gender;
                                HostelAlloc."Space No" := Rooms_Spaces."Space Code";
                                HostelAlloc.Amount := Rec.Charges;
                                ...
                                HostelAlloc."Allocated By" := UserId;
                                HostelAlloc."Booking Date" := Today;
                                HostelAlloc."Booking Time" := Time;

                                HostelAlloc.Insert;

                                //if Cust.Balance + HostelAmount < 1 then // Post billing if paid
                                //  WebPortal.PostHostelCharge2(Student, Semester, Charges, "Hostel No");
                                Message('The student allocated room successfully');
                                ...
                                Error('Please note that the selected room is nolonger available');
                            end;
                        end;
                    }
                    action(BookBatch)
    ...
    var
        ...
        StudCharges: Record "Student Charges";
        LastNo: Code[20];
    ...
    procedure BookRoom(var settle_m: Option " ",JAB,SSP,"Special Programme")
    var
        billAmount: Decimal;
    begin
        // --------Check If More Than One Room Has Been Selected
        Clear(billAmount);
        billAmount := Rec.Amount;
        /*
         rooms.RESET;
        ...
        */
        ...
        StudentHostel.Reset;
        NoRoom := 0;
        StudentHostel.SetRange(StudentHostel.Student, Rec.Student);
        StudentHostel.SetRange(StudentHostel.Cleared, false);
        StudentHostel.SetRange(StudentHostel."Space No", Rec."Space No");
        if StudentHostel.Find('-') then begin
...
+ StudentHostel."Hostel No");
            ...
            if Blocks.Get(Rec."Hostel No") then;
