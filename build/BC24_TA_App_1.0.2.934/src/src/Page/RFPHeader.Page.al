    ...
    layout
    {
        area(content)
        {
            group(General)
            {
                ...
                field("Has EOI"; Rec."Has EOI")
                {
                    ApplicationArea = basic;
                    ToolTip = 'Specifies the value of the Has EOI field.';
                    trigger OnValidate()
                    var
                    begin
                        CurrPage.Update();
            ...
            {
                Visible = EOI;
                Caption = 'Expression of Interest - Publishing Details';
                ...
                field("EOI Stage"; Rec."EOI Stage")
                {
                    ApplicationArea = basic;
                    ToolTip = 'Specifies the value of the EOI Stage field.';
                }
            }
            group(Termination)
            {
        ...
        area(factboxes)
        {
            part("Attached Documents"; "Document Attachment Factbox")
            {
                ApplicationArea = All;
                Caption = 'Attachments';
                SubPageLink = //"Table ID" = CONST(64901),
                              "No." = FIELD("No.");
            }
        }
...
*/
            ...
            action("PublishEOI")
            {
                ...
                PromotedCategory = Process;
                Visible = EOI;
                ToolTip = 'Executes the Publish EOI action.';
                trigger OnAction()
                var
                begin
                    Rec.TestField("Has EOI", true);
                    if Confirm('Are you sure you want to publish this EOI for responses?') = true then begin
                        Rec."EOI Stage" := Rec."EOI Stage"::"Published/Responses";
                        Rec."Published By" := UserId;
                        Rec."Date Published" := CreateDateTime(Today, Time);
                        Rec.Modify();
                        Message('Success');
                    end;
                end;
    ...
    begin
        EOI := false;
        if Rec."Has EOI" = true then EOI := true;
    end;
