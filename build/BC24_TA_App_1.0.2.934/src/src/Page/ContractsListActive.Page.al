page 50249 "Contracts List - Active"
{
    ...
    trigger OnInit()
    begin
        Window.OPEN('Refreshing Active Contracts');
        Contracts.RESET;
        Contracts.SETRANGE(Contracts.Status, Contracts.Status::Approved);
        IF Contracts.FIND('-') THEN BEGIN
            REPEAT
                IF Contracts."Expiry Date" >= TODAY THEN BEGIN
                    Contracts.Active := TRUE;
                    Contracts."Contract Status" := Contracts."Contract Status"::Active;
                    Contracts.MODIFY;
                END;// ELSE BEGIN
        ...
        Window.CLOSE;
        CurrPage.UPDATE();
        /*Contracts.FILTERGROUP := -1;
        SearchString := '>= TODAY';
        
        Contracts.SETFILTER(Contracts."Expiry Date", SearchString);
        Contracts.SETFILTER(Contracts.Status, 'APPROVED');*/
