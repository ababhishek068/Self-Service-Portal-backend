Report 52202581 "Individual Payslips mst"
{
    DefaultLayout = RDLC;
    RDLCLayout = './Layouts/Payrollreports/payslip.rdl';
    ...
    dataset
    {
        dataitem("prSalary Card"; "pR Salary Card")
        {
            ...
            column(EmployeeStatutories; EmployeeStatutories) { }
            column(PeriodName; PeriodName) { }
            column(employeeAge; employeeAge) { }
            column(strIDNumber; strIDNumber) { }
            ...
            begin
                ...
                if emp1.Find('-') then;
                if ((ColumnNo = 1) or (ColumnNo = 0)) then
                    ColumnNo := ColumnNo + 1
                else
                    if ColumnNo = 2 then
                        ColumnNo := 1;
                ...
                if objEmp.Find('-') then begin
                    if objEmp."Global Dimension 1 Code" <> '' then begin
                        Dimm.Reset();
                        // Dimm.SetRange(Dimm."Dimension Code", 'DEPARTMENT');
                        Dimm.SetRange(Dimm.Code, objEmp."Global Dimension 2 Code");
                        if Dimm.Find('-') then
                            dept := Dimm.Name;
                    end;
                    strEmpName := '[' + objEmp."No." + '] ' + ' ' + objEmp."First Name" + ' ' + objEmp."Middle Name"+ '' +objEmp."Last Name";
                    strPin := objEmp."TIN No.";
                    dtDOE := objEmp."Date Of Joining the Company";
                    ...
                    strNssfNo := objEmp."Pension No.";
                    strNhifNo := objEmp."ID Number";
                    strPin := objEmp."TIN No.";
                    EmployeeStatutories := strPin + ', ' + strNssfNo + ', ' + strNhifNo;
                ...
                *******************************************************************************/
                ...
                if PeriodTrans.Find('-') then
                    repeat
                        ...
                        //PeriodTrans.CalcFields(PeriodTrans."Total Statutories");--felix
                        if PeriodTrans."Transaction Code" = 'TOT-DED' then
                            //  error('Test');
                            Evaluate(TransAmt[ColumnNo, Index], Format(PeriodTrans.Amount));
                            //Evaluate(TransAmt[ColumnNo, Index], Format(PeriodTrans.Amount + PeriodTrans.Amount));
            ...
            begin
                ...
                if PeriodFilter = 0D then
                    Error('Specify Payroll Period');

                if not GuiAllowed then
                    Evaluate(PeriodFilter, "prSalary Card".GetFilter("prSalary Card"."Portal Period Filter"));
