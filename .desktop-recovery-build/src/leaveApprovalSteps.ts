...
function normalizedApproverKey(value: unknown) {
  return String(value ?? '').trim().toLowerCase()
}

/**
 * BC may expose only the currently active Approval Entry for a sequential
 * Workflow User Group. Merge that live entry into the discovered route so the
 * employee can still see every configured sequence member.
 */
export function mergeApprovalRouteWithCurrentSteps(
  routeSteps: ReturnType<typeof mapApprovalSteps>,
  currentSteps: ReturnType<typeof mapApprovalSteps>,
  resolvedStatus = 'Pending Approval',
) {
  const route = [...routeSteps].sort((left, right) => left.sequenceNo - right.sequenceNo)
  if (route.length === 0) return normalizeSequentialApprovalStatuses(currentSteps)

  const current = [...currentSteps]
    .filter(
      (step) =>
        Boolean(step.actorEmployeeNo) ||
        (Boolean(step.actorName) && step.actorName !== 'Awaiting approver assignment'),
    )
    .sort((left, right) => left.sequenceNo - right.sequenceNo)

  if (resolvedStatus === 'Approved') {
    return route.map((step) => ({ ...step, status: 'Approved' }))
  }

  if (current.length === 0) {
    return route.map((step, index) => ({
      ...step,
      status: index === 0 ? 'Pending Approval' : 'Waiting',
    }))
  }

  const matches = (routeStep: (typeof route)[number], currentStep: (typeof current)[number]) => {
    const routeId = normalizedApproverKey(routeStep.actorEmployeeNo)
    const currentId = normalizedApproverKey(currentStep.actorEmployeeNo)
    if (routeId && currentId && routeId === currentId) return true
    const routeName = normalizedApproverKey(routeStep.actorName)
    const currentName = normalizedApproverKey(currentStep.actorName)
    return Boolean(routeName && currentName && routeName === currentName)
  }

  const active =
    current.find((step) => step.status === 'Pending Approval') ?? current[current.length - 1]!
  let activeIndex = route.findIndex((step) => matches(step, active))
  if (activeIndex < 0) {
    activeIndex = route.findIndex((step) => step.sequenceNo === active.sequenceNo)
  }
  if (activeIndex < 0) activeIndex = 0

  return route.map((routeStep, index) => {
    const actual = current.find((step) => matches(routeStep, step))
    if (actual) {
      return {
        ...routeStep,
        ...actual,
        actorName: actual.actorName || routeStep.actorName,
        actorEmployeeNo: actual.actorEmployeeNo || routeStep.actorEmployeeNo,
        sequenceNo: routeStep.sequenceNo,
      }
    }
    if (index < activeIndex) return { ...routeStep, status: 'Approved' }
    if (index > activeIndex) {
      const nextIsActive = active.status === 'Approved' && index === activeIndex + 1
      return { ...routeStep, status: nextIsActive ? 'Pending Approval' : 'Waiting' }
    }
    return { ...routeStep, status: active.status }
  })
}
...
export function resolveLeaveApprovalSteps(
  ...
  const resolvedStatus = resolveLeaveStatus(row, applicationApprovalEntries)
  const steps = mapApprovalStepsWithSequence(
    latestLeaveApprovalCycleEntries(applicationApprovalEntries),
  )
...
export async function resolveLeaveApprovalRouteAsync(
  ...
  if (chain.length > 0) {
    const route = mapApprovalSteps(chain)
    return mergeApprovalRouteWithCurrentSteps(
      route,
      mapped,
      resolveLeaveStatus(leaveRow, approvalEntries),
    )
  }
...
export async function resolveLeaveApprovalStepsAsync(
  ...
  const currentApprovalEntries = latestLeaveApprovalCycleEntries(
    currentLeaveApplicationApprovalEntries(row, approvalEntries),
  )
  if (currentApprovalEntries.length > 0) {
    const enriched = await enrichMappedApprovalSteps(mapApprovalSteps(currentApprovalEntries))
    ...
    if (!needsBetterName && enriched.length > 1) return enriched
  }

  const fallbackSteps = resolveLeaveApprovalSteps(row, currentApprovalEntries, documentNo)
  const entriesForRoute =
    currentApprovalEntries.length > 0
      ? currentApprovalEntries
