...
function employeeResetToken(employee: BcEmployee) {
  const row = employee as BcEmployee & Record<string, unknown>
  return String(row.ResetToken ?? row.Password_Reset_Token ?? row.Reset_Token ?? '').trim()
}
...
export function buildAuthRouter() {
  ...
  router.post('/auth/reset-password', async (req, res, next) => {
    try {
      ...
      if (storedToken !== resetToken || resetTokenIsExpired(employee.TokenExpired)) {
        res.status(422).json({
